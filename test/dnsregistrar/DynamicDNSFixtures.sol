...
library DynamicDNSFixtures {
    ...
    function createValidProof(
        ...
        DNSSEC.RRSetWithSignature[]
            memory proof = new DNSSEC.RRSetWithSignature[](2);
     ...
     */
    function createProofWithAddress(
        address addr
    ) internal returns (DNSSEC.RRSetWithSignature[] memory) {
     ...
     */
    function parseHexPair(
        string memory data
    ) internal pure returns (bytes memory first, bytes memory second) {
