...
contract TestDNSClaimChecker is Test {
    ...
    function testGetOwnerAddressSuccess() public pure {
        ...
        (address foundAddr, bool found) = DNSClaimChecker.getOwnerAddress(
            dnsName,
            rdata
        );
    ...
    function testGetOwnerAddressMultipleRecords() public pure {
        ...
        assertEq(
            foundAddr,
            expectedAddr,
            "Should return correct address from valid record"
        );
    }
    ...
    function testParseStringPrefixValidation() public pure {
        ...
        (address addr, bool found) = DNSClaimChecker.parseString(
            validStr,
            0,
            validStr.length
        );
        ...
        bytes
            memory invalidStr1 = "b=0x1234567890123456789012345678901234567890"; // wrong first char
        (addr, found) = DNSClaimChecker.parseString(
            invalidStr1,
            0,
            invalidStr1.length
        );
        ...
        bytes
            memory invalidStr3 = "addr=0x1234567890123456789012345678901234567890"; // wrong prefix
        (addr, found) = DNSClaimChecker.parseString(
            invalidStr3,
            0,
            invalidStr3.length
    ...
    function createMismatchedTXTRecord() internal pure returns (bytes memory) {
        ...
        return
            DNSTestUtils.createTXTRecord(
                wrongName,
                "a=0x1234567890123456789012345678901234567890"
            );
    }

    function createInvalidFormatTXTRecord()
        internal
        pure
        returns (bytes memory)
    {
        bytes memory ensName = DNSTestUtils.encodeDNSName("_ens.test.example");
        return
            DNSTestUtils.createTXTRecord(ensName, "invalid-format-not-address");
    }
    ...
    function createMultipleTXTRecords(
        ...
        contents[1] = string(
            abi.encodePacked("a=0x", DNSTestUtils.addressToString(validAddr))
        );
