...
contract TestOffchainDNSResolverLegacy is Test {
     ...
     */
    function testCorrectlyResolvesUsingLegacyResolversWithoutResolveSupport()
        public
    {
        ...
        DNSSEC.RRSetWithSignature[] memory rrsets = _createCustomProof(
            string(
                abi.encodePacked(
                    "ENS1 ",
                    _addressToString(address(legacyResolver))
                )
            )
        );
        ...
        assertEq(
            returnedText,
            "test",
            "Should return 'test' from legacy resolver"
        );
    }
    ...
    function testCorrectlyResolvesUsingOffchainResolver() public {
        ...
        DNSSEC.RRSetWithSignature[] memory rrsets = _createCustomProof(
            string(
                abi.encodePacked(
                    "ENS1 ",
                    _addressToString(address(offchainResolver))
                )
            )
        );
     ...
     */
    function testShouldPreventOffchainLookupErrorPropagationFromNonCCIPAwareContracts()
        public
    {
        bytes32 nameHash = keccak256(
            abi.encodePacked(keccak256("test"), keccak256("test"))
        ...
        DNSSEC.RRSetWithSignature[] memory rrsets = _createCustomProof(
            string(
                abi.encodePacked(
                    "ENS1 ",
                    _addressToString(address(dummyResolver))
                )
            )
        );
     ...
     */
    function _createCustomProof(
        string memory txtContent
        ...
        bytes memory rootKeysData = DynamicDNSFixtures.hexStringToBytes(
            string(rootKeysHex)
        );
        bytes memory txtData = DynamicDNSFixtures.hexStringToBytes(
            string(txtHex)
        );

        // Create RRSetWithSignature array
        DNSSEC.RRSetWithSignature[]
            memory rrsets = new DNSSEC.RRSetWithSignature[](2);
        rrsets[0] = DNSSEC.RRSetWithSignature({
            rrset: rootKeysData,
            sig: hex"00"
        });
        rrsets[1] = DNSSEC.RRSetWithSignature({rrset: txtData, sig: hex"00"});
