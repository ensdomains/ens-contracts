...
contract TestSimplePublicSuffixList is Test {
    ...
    function testOwnerCanAddPublicSuffixes() public {
        ...
        assertTrue(suffixList.isPublicSuffix(suffixes[0]), "test should be public suffix");
        assertTrue(suffixList.isPublicSuffix(suffixes[1]), "co.nz should be public suffix");
    }
    ...
    function testIsPublicSuffixReturnsFalseForNonExistentSuffixes() public view {
        bytes memory nonExistentSuffix = DynamicDNSFixtures.dnsEncodeName("example");
        
        assertFalse(suffixList.isPublicSuffix(nonExistentSuffix), "Non-existent suffix should return false");
    }
    ...
    function testCanAddSingleSuffix() public {
        ...
        assertTrue(suffixList.isPublicSuffix(suffixes[0]), "example should be public suffix");
    }
    ...
    function testCanAddMultipleSuffixes() public {
        ...
        for (uint256 i = 0; i < suffixes.length; i++) {
            assertTrue(suffixList.isPublicSuffix(suffixes[i]), "All added suffixes should be public suffixes");
        }
    }
    ...
    function testCanAddSameSuffixMultipleTimes() public {
        ...
        assertTrue(suffixList.isPublicSuffix(suffixes[0]), "Suffix should be public");
        
        // Add same suffix again
        vm.prank(owner);
        suffixList.addPublicSuffixes(suffixes);
        
        assertTrue(suffixList.isPublicSuffix(suffixes[0]), "Suffix should still be public");
    }
    ...
    function testDifferentEncodingsOfSameName() public {
        ...
        assertTrue(suffixList.isPublicSuffix(testSuffix1), "Properly encoded test should be public suffix");
        assertFalse(suffixList.isPublicSuffix(testSuffix2), "Differently encoded test should not be public suffix");
    }
    ...
    function testOwnershipTransfer() public {
        ...
        assertTrue(suffixList.isPublicSuffix(suffixes[0]), "New owner should be able to add suffixes");
    ...
    function testEmptyNameSuffix() public {
        ...
        assertTrue(suffixList.isPublicSuffix(suffixes[0]), "Empty name should be addable as suffix");
    }
}