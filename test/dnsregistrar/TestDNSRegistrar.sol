...
contract TestDNSRegistrar is Test {
    ...
    // Trust anchors
    bytes constant TRUST_ANCHORS = hex"00002b000100000e1000244a5c080249aac11d7b6f6446702e54a1607371607a1a41855200fd2ce1cdde32f24e8fb500002b000100000e1000244f660802e06d44b80b8f1d39a95c0b0d7c65d08458e880409bbc683457104237c7f8ec8d00002b000100000e10000404fefdfd";
    ...
    function testSetsConstructorVariablesCorrectly() public view {
        assertEq(address(dnsRegistrar.oracle()), address(dnssec), "DNSSEC oracle should be set");
        assertEq(address(dnsRegistrar.ens()), address(ens), "ENS registry should be set");
    }
    
    function testAllowsAnyoneToClaimOnBehalfOfOwner() public {
        vm.startPrank(ACCOUNT1);
        
        DNSSEC.RRSetWithSignature[] memory proof = DynamicDNSFixtures.createProofForDNSRegistrar(
            "foo.test",
            ACCOUNT0,
            "valid"
        );
        ...
        bytes32 node = keccak256(abi.encodePacked(keccak256(abi.encodePacked(ROOT_NODE, keccak256("test"))), keccak256("foo")));
    ...
    function testAllowsClaimsOnNonTLDs() public {
        ...
        bytes32 nzNode = keccak256(abi.encodePacked(ROOT_NODE, keccak256("nz")));
    ...
    function testAllowsAnyoneToUpdateDNSSECReferencedName() public {
        // First claim
        DNSSEC.RRSetWithSignature[] memory proof1 = DynamicDNSFixtures.createProofForDNSRegistrar(
            "foo.test",
            ACCOUNT0,
            "valid"
        );
        ...
        DNSSEC.RRSetWithSignature[] memory proof2 = DynamicDNSFixtures.createProofForDNSRegistrar(
            "foo.test",
            ACCOUNT1,
            "valid"
        );
        ...
        assertEq(ens.owner(node), ACCOUNT1, "Owner should be updated to ACCOUNT1");
    }
    ...
    function testDoesNotAllowUpdatesWithStaleRecords() public {
        ...
        vm.expectRevert(abi.encodeWithSignature("SignatureExpired(uint32,uint32)", block.timestamp - 3600 * 24, block.timestamp));
        dnsRegistrar.proveAndClaim(
            DynamicDNSFixtures.dnsEncodeName("foo.test"),
            proof
        );
    ...
    function testDoesNotAllowAnyoneElseToClaimAndSetResolver() public {
        ...
        vm.expectRevert(abi.encodeWithSignature("PermissionDenied(address,address)", ACCOUNT0, ACCOUNT1));
        dnsRegistrar.proveAndClaimWithResolver(
            DynamicDNSFixtures.dnsEncodeName("foo.test"),
            proof,
            ACCOUNT1,
    ...
    function testSetsAddressOnResolverIfProvided() public {
        ...
        assertEq(publicResolver.addr(node), ACCOUNT0, "Address should be set on resolver");
    }
    ...
    function testCannotClaimMultipleNamesUsingSingleUnrelatedProof() public {
        // Claim alice.test
        DNSSEC.RRSetWithSignature[] memory proofForAlice = DynamicDNSFixtures.createProofForDNSRegistrar(
            "alice.test",
            ACCOUNT1,
            "valid"
        );
        ...
        bytes32 aliceNode = keccak256(abi.encodePacked(keccak256(abi.encodePacked(ROOT_NODE, keccak256("test"))), keccak256("alice")));
        assertEq(ens.owner(aliceNode), ACCOUNT1, "Alice should be owned by ACCOUNT1");
    ...
    function testCannotTakeoverClaimedDomainsUsingUnrelatedProof() public {
        ...
        // Bob's proof
        DNSSEC.RRSetWithSignature[] memory proofForBob = DynamicDNSFixtures.createProofForDNSRegistrar(
            "bob.test",
            ACCOUNT2,
            "valid"
        );
