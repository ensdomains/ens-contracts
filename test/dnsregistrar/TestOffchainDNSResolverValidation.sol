...
contract TestOffchainDNSResolverValidation is Test {
    ...
    function _doDnsResolveCallback(
        ...
        DNSSEC.RRSetWithSignature[]
            memory proof = new DNSSEC.RRSetWithSignature[](1);
        proof[0] = DNSSEC.RRSetWithSignature({rrset: hex"00", sig: hex"00"});
        ...
        try offchainDnsResolver.resolveCallback(response, extraData) returns (
            bytes memory result
        ) {
            return result;
        } catch {
     ...
     */
    function testHandlesCallsToResolveCallbackWhereValidTXTRecordIsNotFirst()
        public
    {
        ...
        texts[1] = string(
            abi.encodePacked(
                "ENS1 ",
                DynamicDNSFixtures.addressToString(address(ownedResolver))
            )
        );
        ...
        assertTrue(
            result.length == 0 || abi.decode(result, (address)) == testAddress
        );
    }
