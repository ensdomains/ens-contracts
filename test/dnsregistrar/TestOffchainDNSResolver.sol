...
contract TestOffchainDNSResolver is Test {
    ...
    function testResolveTriggersCCIPRead() public {
        ...
        bytes memory expectedData = abi.encodeWithSignature("resolve(bytes,uint16)", dnsName, uint16(16));
        
        // Expected extraData for resolveCallback
        bytes memory expectedExtraData = abi.encode(dnsName, query, bytes4(0x00000000));
    ...
    function testResolveCallbackWithValidTXTRecords() public {
        ...
        DNSSEC.RRSetWithSignature[] memory rrsets = DynamicDNSFixtures.createValidProof("standard");
        ...
        assertEq(returned, testAddr, "Should return correct address through resolver");
    }
    ...
    function testResolveCallbackWithExtraData() public {
        ...
        DNSSEC.RRSetWithSignature[] memory rrsets = DynamicDNSFixtures.createValidProof("extra");
        ...
        assertEq(returned, testAddr, "Should ignore extra data and return correct address");
    }
    ...
    function testResolveCallbackWithENSName() public {
        ...
        bytes32 dnsresolverNode = keccak256(abi.encodePacked(ethNode, keccak256("dnsresolver")));
        ...
        DNSSEC.RRSetWithSignature[] memory rrsets = DynamicDNSFixtures.createValidProof("ens");
    ...
    function testResolveCallbackRejectsInvalidTXT() public {
        ...
        DNSSEC.RRSetWithSignature[] memory rrsets = DynamicDNSFixtures.createValidProof("invalid");
        
        bytes memory response = abi.encode(rrsets);
        bytes memory query = abi.encodeWithSignature("addr(bytes32)", keccak256("test.test"));
        bytes memory extraData = abi.encode(
            DynamicDNSFixtures.dnsEncodeName("test.test"),
            query,
            bytes4(0)
        ...
        vm.expectRevert(abi.encodeWithSignature("CouldNotResolve(bytes)", DynamicDNSFixtures.dnsEncodeName("test.test")));
    ...
    function testSupportsInterface() public view {
        bool result = resolver.supportsInterface(type(IExtendedResolver).interfaceId); // IExtendedResolver interface
