...
contract TestExtendedDNSResolver is Test {
     ...
     */
    function testCorrectlyHandlesExtraStringDataInTXTRecordWhenCallingResolverThatSupportsIt()
        public
        view
    {
        ...
        assertEq(
            returnedText,
            "foobie bletch",
            "Should return exact text from t[test]= context"
        );
    }
     ...
     */
    function testCorrectlyHandlesExtraDataInTXTRecordWhenCallingResolverThatSupportsAddressResolution()
        public
        view
    {
        bytes memory name = abi.encodePacked("test.test");
        ...
        assertEq(
            returnedAddress,
            testAddress,
            "Should parse address from a[60]= format"
        );
    }
     ...
     */
    function testCorrectlyHandlesExtraDataInTXTRecordWhenCallingResolverThatSupportsAddressResolutionWithValidCointype()
        public
        view
    {
        bytes memory name = abi.encodePacked("test.test");
        ...
        assertEq(
            returnedAddress,
            testAddress,
            "Should return address for matching coin type"
        );
    }
     ...
     */
    function testHandlesExtraDataInTXTRecordWhenCallingResolverThatSupportsAddressResolutionWithInvalidCointype()
        public
        view
    {
        bytes memory name = abi.encodePacked("test.test");
        ...
        assertEq(
            result.length,
            0,
            "Should return empty bytes for mismatched coin type"
        );
    }
     ...
     */
    function testRaisesAnErrorIfExtraAddressDataInTheTXTRecordIsInvalid()
        public
    {
        bytes memory name = abi.encodePacked("test.test");
        bytes32 nameHash = keccak256(
