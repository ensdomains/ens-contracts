...
contract TestMigrationHelper is Test {
    ...
    function setUp() public {
        ...
        ensRegistry.setSubnodeOwner(ZERO_HASH, keccak256("reverse"), ownerAccount);
        ...
        ensRegistry.setSubnodeOwner(ZERO_HASH, ETH_LABEL, address(baseRegistrar));
        ...
        migrationHelper = new MigrationHelper(baseRegistrar, INameWrapper(address(nameWrapper)));
    ...
    function testShouldRefuseToMigrateUnwrappedNamesToTheZeroAddress() public {
        ...
        vm.startPrank(ownerAccount);
        for (uint256 i = 0; i < ids.length; i++) {
            baseRegistrar.register(ids[i], registrantAccount, REGISTRATION_TIME);
        }
        vm.stopPrank();
