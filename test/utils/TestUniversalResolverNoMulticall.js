...
contract('UniversalResolver', function (accounts) {
  ...
  beforeEach(async () => {
    ...
    universalResolver = await deploy(
  ...
  describe('resolve()', () => {
    ...
    it('should return a revert if the resolver reverts with OffchainLookup', async () => {
      const data = publicResolver.interface.encodeFunctionData(
        'addr(bytes32)',
        [namehash.hash('offchain.test.eth')],
      )
      ...
      try {
        ...
        expect(e.errorArgs.urls).to.deep.equal(['https://example.com/'])
