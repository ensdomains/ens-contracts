...
contract('UniversalResolver', function(accounts) {
  ...
  describe('reverse()', () => {
    it('should resolve', async () => {
      const result = await reverseResolver.name(namehash.hash(reverseNode))
      expect(result).to.equal('test.eth')
    })
    it('should resolve a reverse record', async () => {
      ...
      const [ret] = ethers.utils.defaultAbiCoder.decode(['string'], result)
      console.log(ret)
