...
contract TestNameCoderTest is BaseTest {
    ...
    function testEncodeSimple() public view {
        ...
        assertEq(result, hex"0161" hex"00", "Simple domain encoding failed");
    }
    ...
    function testEncodeMultiLevel() public view {
        ...
        assertEq(result, hex"01610262620363636300", "Multi-level domain encoding failed");
    }
    
    function testEncodeLongLabel() public view {
        // Test encoding domain with maximum label length (63 characters)
        string memory longLabel = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"; // 63 chars
        assertEq(bytes(longLabel).length, 63, "Long label should be 63 characters");
        
        bytes memory result = nameCoder.encode(longLabel);
        // Should start with 0x3f (63 in hex) followed by the label and null terminator
        bytes memory expected = abi.encodePacked(uint8(63), longLabel, uint8(0));
    ...
    function testDecodeSimple() public view {
        ...
        string memory result = nameCoder.decode(hex"0161" hex"00");
    ...
    function testNamehashWithOffset() public view {
        ...
        assertEq(result1, namehash("sub.test.eth"), "Full domain namehash failed");
        
        // Parent domain (test.eth)
        bytes32 result2 = nameCoder.namehash(encoded, 4); // Skip "sub" label (1 + 3 bytes)
        assertEq(result2, namehash("test.eth"), "Parent domain namehash failed");
    ...
    function testEncodeFailureInvalidDomains() public {
        ...
        vm.expectRevert(abi.encodeWithSignature("DNSEncodingFailed(string)", ".a"));
        nameCoder.encode(".a");
        ...
        vm.expectRevert(abi.encodeWithSignature("DNSEncodingFailed(string)", "a..b"));
        nameCoder.encode("a..b");
        
        // Just a dot
        vm.expectRevert(abi.encodeWithSignature("DNSEncodingFailed(string)", "."));
        nameCoder.encode(".");
    ...
    function testDecodeFailureInvalidData() public {
        ...
        // Empty data
        vm.expectRevert(abi.encodeWithSignature("DNSDecodingFailed(bytes)", hex""));
        nameCoder.decode(hex"");
        
        // Incomplete length prefix
        vm.expectRevert(abi.encodeWithSignature("DNSDecodingFailed(bytes)", hex"02"));
        nameCoder.decode(hex"02");
        
        // Invalid length (claims 16 bytes but only has 0)
        vm.expectRevert(abi.encodeWithSignature("DNSDecodingFailed(bytes)", hex"1000"));
        nameCoder.decode(hex"1000");
        
        // Missing null terminator
        vm.expectRevert(abi.encodeWithSignature("DNSDecodingFailed(bytes)", hex"0161"));
    ...
    function testNamehashFailureInvalidData() public {
        ...
        // Invalid offset
        vm.expectRevert(abi.encodeWithSignature("DNSDecodingFailed(bytes)", hex"016100"));
        nameCoder.namehash(hex"0161" hex"00", 5);
    }
    
    function testMaliciousLabel() public {
        // Test decoding with malicious label containing dots
        vm.expectRevert(abi.encodeWithSignature("DNSDecodingFailed(bytes)", hex"03612e6200"));
