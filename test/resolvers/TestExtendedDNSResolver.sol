...
contract TestExtendedDNSResolver is Test {
    ...
    function testResolveFunction() public view {
        ...
        bytes memory queryData = abi.encodeWithSignature("addr(bytes32)", TEST_NODE);
        bytes memory context = ""; // Empty context
        
        // With empty context, addr query should return empty bytes (no revert)
        bytes memory result = resolver.resolve(TEST_DNS_NAME, queryData, context);
        assertEq(result.length, 0, "Empty context should return empty result for addr query");
    }
    
    function testResolveWithTextQuery() public view {
        // Test resolve with text query - should return empty result with empty context
        
        bytes memory textQuery = abi.encodeWithSignature("text(bytes32,string)", TEST_NODE, "email");
        bytes memory context = "";
        
        // Text query with empty context should return empty string encoded
        bytes memory result = resolver.resolve(TEST_DNS_NAME, textQuery, context);
        string memory decodedResult = abi.decode(result, (string));
        assertEq(bytes(decodedResult).length, 0, "Empty context should return empty text");
    }
    
    function testResolveWithAddressQuery() public view {
        // Test resolve with address query - should return empty with empty context
        
        bytes memory addrQuery = abi.encodeWithSignature("addr(bytes32)", TEST_NODE);
        bytes memory context = "";
        
        // Address query with empty context should return empty bytes
        bytes memory result = resolver.resolve(TEST_DNS_NAME, addrQuery, context);
        assertEq(result.length, 0, "Empty context should return empty result for address query");
    }
    ...
    function testResolveWithContenthashQuery() public {
        ...
        bytes memory contenthashQuery = abi.encodeWithSignature("contenthash(bytes32)", TEST_NODE);
        bytes memory context = "";
    ...
    function testResolveWithMulticoinQuery() public view {
        ...
        bytes memory multicoinQuery = abi.encodeWithSignature("addr(bytes32,uint256)", TEST_NODE, uint256(0)); // Bitcoin
        bytes memory context = "";
        
        // Multicoin query with empty context should return empty bytes
        bytes memory result = resolver.resolve(TEST_DNS_NAME, multicoinQuery, context);
        assertEq(result.length, 0, "Empty context should return empty result for multicoin query");
    }
    
    function testResolveWithPubkeyQuery() public {
        // Test resolve with unsupported pubkey query - should revert with NotImplemented
        
        bytes memory pubkeyQuery = abi.encodeWithSignature("pubkey(bytes32)", TEST_NODE);
        bytes memory context = "";
    ...
    function testResolveWithABIQuery() public {
        ...
        bytes memory abiQuery = abi.encodeWithSignature("ABI(bytes32,uint256)", TEST_NODE, uint256(1));
        bytes memory context = "";
    ...
    function testResolveWithNameQuery() public {
        ...
        bytes memory nameQuery = abi.encodeWithSignature("name(bytes32)", TEST_NODE);
        bytes memory context = "";
    ...
    function testDNSNameHandling() public view {
        ...
        bytes memory result2 = resolver.resolve(simpleLabel, queryData, context);
        bytes memory result3 = resolver.resolve(multilabelName, queryData, context);
        
        assertEq(result1.length, 0, "Root name with empty context should return empty");
        assertEq(result2.length, 0, "Simple label with empty context should return empty");
        assertEq(result3.length, 0, "Multi-label name with empty context should return empty");
    }
    ...
    function testInvalidDNSNames() public view {
        ...
        bytes memory result1 = resolver.resolve(invalidName1, queryData, context);
        bytes memory result2 = resolver.resolve(invalidName2, queryData, context);
        
        assertEq(result1.length, 0, "Invalid name 1 should return empty with empty context");
        assertEq(result2.length, 0, "Invalid name 2 should return empty with empty context");
    }
    ...
    function testUnsupportedFunction() public {
        ...
        bytes memory unsupportedQuery = abi.encodeWithSignature("unsupportedFunction(bytes32)", TEST_NODE);
        bytes memory context = "";
    ...
    function testResolverBehaviorWithMockData() public view {
        ...
        bytes memory validTxtRecord = "a[60]=0x1234567890123456789012345678901234567890";
        ...
        assertTrue(result.length > 0, "Valid TXT record should return address data");
        address decodedAddr = abi.decode(result, (address));
        assertEq(decodedAddr, 0x1234567890123456789012345678901234567890, "Should decode correct address");
    }
    ...
    function testErrorHandling() public view {
        ...
        for (uint i = 0; i < malformedNames.length; i++) {
            bytes memory result = resolver.resolve(malformedNames[i], queryData, context);
            assertEq(result.length, 0, "Malformed names should return empty with empty context");
        }
    }
    ...
    function testContextParameter() public view {
        ...
        bytes memory result1 = resolver.resolve(TEST_DNS_NAME, queryData, emptyContext);
        ...
        bytes memory result2 = resolver.resolve(TEST_DNS_NAME, queryData, contextWithData);
        assertEq(result2.length, 0, "Invalid context format should return empty result");
    }
    ...
    function testTextRecordFormatParsing() public view {
        ...
        bytes memory result = resolver.resolve(TEST_DNS_NAME, queryData, context);
        assertTrue(result.length > 0, "Valid address format should return data");
        
        address decodedAddr = abi.decode(result, (address));
        assertEq(decodedAddr, 0x1234567890123456789012345678901234567890, "Should parse address correctly");
    }
    ...
    function testInvalidAddressFormat() public {
        ...
        vm.expectRevert(abi.encodeWithSignature("InvalidAddressFormat(bytes)", "0xinvalidhex"));
    ...
    function testTextQueryWithValidData() public view {
        ...
        assertEq(decodedText, "test@example.com", "Should return correct text value");
    }
    ...
    function testTextQueryNotFound() public view {
        ...
        bytes memory context = "t[email]=test@example.com";
        bytes memory textQuery = abi.encodeWithSignature("text(bytes32,string)", TEST_NODE, "nonexistent");
        
        // Should return empty string
        bytes memory result = resolver.resolve(TEST_DNS_NAME, textQuery, context);
        string memory decodedText = abi.decode(result, (string));
        assertEq(bytes(decodedText).length, 0, "Non-existent key should return empty string");
    }
    
    function testMulticoinAddress() public view {
        // Test multicoin address query with Bitcoin
        
        bytes memory context = "a[0]=0x1234567890123456789012345678901234567890";
        bytes memory multicoinQuery = abi.encodeWithSignature("addr(bytes32,uint256)", TEST_NODE, uint256(0));
        ...
        address decodedAddr = abi.decode(result, (address));
        assertEq(decodedAddr, 0x1234567890123456789012345678901234567890, "Should return correct Bitcoin address");
    }
    ...
    function testMulticoinAddressNotFound() public view {
        ...
        bytes memory context = "a[60]=0x1234567890123456789012345678901234567890";
        bytes memory multicoinQuery = abi.encodeWithSignature("addr(bytes32,uint256)", TEST_NODE, uint256(1));
        
        // Should return empty bytes
        bytes memory result = resolver.resolve(TEST_DNS_NAME, multicoinQuery, context);
        assertEq(result.length, 0, "Non-existent coin type should return empty");
    }
    
    function testComplexContext() public view {
        // Test complex context with multiple key-value pairs
        
        bytes memory context = "a[60]=0x1234567890123456789012345678901234567890 t[email]=test@example.com t[url]=https://example.com";
        
        // Test address query
        bytes memory addrQuery = abi.encodeWithSignature("addr(bytes32)", TEST_NODE);
        bytes memory addrResult = resolver.resolve(TEST_DNS_NAME, addrQuery, context);
        address decodedAddr = abi.decode(addrResult, (address));
        assertEq(decodedAddr, 0x1234567890123456789012345678901234567890, "Should parse address from complex context");
        
        // Test text query for email
        bytes memory emailQuery = abi.encodeWithSignature("text(bytes32,string)", TEST_NODE, "email");
        bytes memory emailResult = resolver.resolve(TEST_DNS_NAME, emailQuery, context);
        string memory decodedEmail = abi.decode(emailResult, (string));
        assertEq(decodedEmail, "test@example.com", "Should parse email from complex context");
        
        // Test text query for URL
        bytes memory urlQuery = abi.encodeWithSignature("text(bytes32,string)", TEST_NODE, "url");
        bytes memory urlResult = resolver.resolve(TEST_DNS_NAME, urlQuery, context);
        string memory decodedUrl = abi.decode(urlResult, (string));
        assertEq(decodedUrl, "https://example.com", "Should parse URL from complex context");
    }
}
