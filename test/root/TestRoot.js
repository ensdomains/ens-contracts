...
contract('Root', function(accounts) {
    ...
    beforeEach(async function() {
        ...
        await ens.setSubnodeOwner('0x0', '0x' + sha3('arb'), root.address, {from: accounts[0]});
    ...
    describe('setSubnodeOwner', async () => {
        ...
        it('should allow controllers to set subnodes', async () => {
            await root.setSubnodeOwner('0x' + sha3('arb'), accounts[1], {from: accounts[0]});
            assert.equal(accounts[1], await ens.owner(node));
        });

        it('should fail when non-controller tries to set subnode', async () => {
            await exceptions.expectFailure(root.setSubnodeOwner('0x' + sha3('arb'), accounts[1], {from: accounts[1]}));
        });

        it('should not allow setting a locked TLD', async () => {
            await root.lock('0x' + sha3('arb'));
            await exceptions.expectFailure(root.setSubnodeOwner('0x' + sha3('arb'), accounts[1], {from: accounts[0]}));
        });
    });
});
