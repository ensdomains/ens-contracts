...
contract TestRoot is BaseTest {
    ...
    function testLockTLD() public {
        ...
        assertFalse(
            root.locked(ETH_LABEL),
            "ETH should not be locked initially"
        );
        ...
        assertTrue(
            root.locked(ETH_LABEL),
            "ETH should be locked after calling lock"
        );
    }
    ...
    function testControllerManagement() public {
        ...
        assertFalse(
            root.controllers(USER1),
            "USER1 should not be controller initially"
        );
        ...
        assertTrue(
            root.controllers(USER1),
            "USER1 should be controller after setting"
        );
        ...
        assertEq(
            ens.owner(ETH_NODE),
            USER2,
            "USER1 as controller should be able to set subnode"
        );
        ...
        assertFalse(
            root.controllers(USER1),
            "USER1 should not be controller after removal"
        );
    ...
    function testSetResolverAsOwner() public {
        ...
        assertEq(
            currentResolver,
            testResolver,
            "Root resolver should be set to testResolver"
        );
    }
    ...
    function testSetResolverToZeroAddress() public {
        ...
        assertEq(
            currentResolver,
            address(0),
            "Root resolver should be set to zero address"
        );
    }
    ...
    function testSetResolverMultipleTimes() public {
        ...
        assertEq(
            ens.resolver(ZERO_HASH),
            resolver1,
            "Should set first resolver"
        );
    ...
    function testSupportsInterfaceMetaID() public {
        ...
        assertTrue(
            root.supportsInterface(metaID),
            "Should support EIP-165 meta interface"
        );
    }
    ...
    function testSupportsInterfaceRandomID() public {
        ...
        assertFalse(
            root.supportsInterface(randomID),
    ...
    function testSupportsInterfaceAllOnesID() public {
        ...
        assertFalse(
            root.supportsInterface(allOnesID),
            "Should not support all-ones interface"
        );
    }
    ...
    function testLockAlreadyLockedTLD() public {
        ...
        assertTrue(
            root.locked(ETH_LABEL),
            "ETH should still be locked after second lock"
        );
    }
    ...
    function testSetSubnodeOwnerWithCustomLabels() public {
        ...
        bytes32 customNode = keccak256(
            abi.encodePacked(ZERO_HASH, customLabel)
        );
        ...
        assertEq(
            ens.owner(customNode),
            USER1,
            "Custom node should be owned by USER1"
        );
    }
}
