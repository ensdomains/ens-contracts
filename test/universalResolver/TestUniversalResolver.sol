...
contract TestUniversalResolver is Test {
    ...
    address constant ANOTHER_ADDRESS =
    ...
    function takeControl(string memory name) internal {
        ...
        if (ENSTestUtils.strEqual(name, "test.eth")) {
            vm.prank(owner);
            ensRegistry.setSubnodeOwner(
                ENSTestUtils.namehash("eth"),
                ENSTestUtils.labelhash("test"),
                owner
            );
            return;
        }
        ...
        if (endsWith(name, ".addr.reverse")) {
            ...
            vm.prank(owner);
            ensRegistry.setSubnodeOwner(
                ENSTestUtils.namehash("reverse"),
                ENSTestUtils.labelhash("addr"),
                owner
            );
            ...
            if (dotPos == 0) {
                ...
                vm.prank(owner);
                ensRegistry.setSubnodeOwner(
                    ENSTestUtils.namehash("addr.reverse"),
                    ENSTestUtils.labelhash(string(prefix)),
                    owner
                );
            } else {
                // Format: {address}.{coinType}.addr.reverse
                // First set {coinType}.addr.reverse
                bytes memory coinTypeLabel = new bytes(
                    prefix.length - dotPos - 1
                );
                ...
                ensRegistry.setSubnodeOwner(
                    ENSTestUtils.namehash("addr.reverse"),
                    ENSTestUtils.labelhash(string(coinTypeLabel)),
                    owner
                );
                ...
                string memory parentName = string(
                    abi.encodePacked(coinTypeLabel, ".addr.reverse")
                );
                vm.prank(owner);
                ensRegistry.setSubnodeOwner(
                    ENSTestUtils.namehash(parentName),
                    ENSTestUtils.labelhash(string(addrLabel)),
                    owner
                );
            }
            return;
        ...
        if (firstDot > 0 && secondDot > 0) {
            ...
            ensRegistry.setSubnodeOwner(
                ENSTestUtils.namehash("eth"),
                ENSTestUtils.labelhash(string(secondLabel)),
                owner
            );

            // Then set first.second.third ownership
            string memory parentName = string(
                abi.encodePacked(secondLabel, ".eth")
            );
            vm.prank(owner);
            ensRegistry.setSubnodeOwner(
                ENSTestUtils.namehash(parentName),
                ENSTestUtils.labelhash(string(firstLabel)),
                owner
            );
        } else if (firstDot > 0) {
            ...
            ensRegistry.setSubnodeOwner(
                ENSTestUtils.namehash("eth"),
                ENSTestUtils.labelhash(string(label)),
                owner
            );
        }
    }
    ...
    function getReverseName(
        ...
        if (coinType == COIN_TYPE_ETH) {
            return
                string(
                    abi.encodePacked(toHexStringNoPrefix(addr), ".addr.reverse")
                );
        } else {
            return
                string(
                    abi.encodePacked(
                        toHexStringNoPrefix(addr),
                        ".",
                        uintToString(coinType),
                        ".addr.reverse"
                    )
                );
        }
    ...
    function testSupportsInterfaces() public view {
        ...
        assertTrue(
            universalResolver.supportsInterface(type(IERC165).interfaceId),
            "Should support IERC165"
        );

        // Should support IUniversalResolver
        assertTrue(
            universalResolver.supportsInterface(
    ...
    function testFindResolverUnset() public {
        ...
        (address resolver, bytes32 node, uint256 offset) = universalResolver
            .findResolver(dnsEncodeName(TEST_NAME));
    ...
    function testFindResolverImmediate() public {
        ...
        ensRegistry.setResolver(
            ENSTestUtils.namehash(TEST_NAME),
            address(shapeshift1)
        );
        ...
        assertEq(
            resolver,
            address(shapeshift1),
            "Resolver should be shapeshift1"
        );
        assertEq(node, ENSTestUtils.namehash(TEST_NAME), "Node should match");
        assertEq(offset, 0, "Offset should be 0");
    }
    ...
    function testFindResolverExtended() public {
        ...
        vm.startPrank(owner);
        ensRegistry.setResolver(
            ENSTestUtils.namehash(getParentName(TEST_NAME)),
            address(shapeshift1)
        );
        ...
        assertEq(
            offset,
            1 + bytes("test").length,
            "Offset should account for label length"
        );
    }
    ...
    function testFindResolverAutoEncrypted() public {
        ...
        string memory longName = string(
            abi.encodePacked(longLabel, ".", TEST_NAME)
        );
        ...
        vm.prank(owner);
        ensRegistry.setResolver(
            ENSTestUtils.namehash(longName),
            address(shapeshift1)
        );

        (address resolver, bytes32 node, uint256 offset) = universalResolver
            .findResolver(dnsEncodeName(longName));
    ...
    function testFindResolverSelfEncrypted() public {
        ...
        string memory encryptedName = string(
            abi.encodePacked(
                "[",
                toHexStringNoPrefix(address(uint160(uint256(testHash)))),
                "].eth"
            )
        );
        ...
        vm.prank(owner);
        ensRegistry.setResolver(
            ENSTestUtils.namehash(encryptedName),
            address(shapeshift1)
        );

        (address resolver, bytes32 node, uint256 offset) = universalResolver
            .findResolver(dnsEncodeName(encryptedName));
    ...
    function testResolveNotExtended() public {
        ...
        UniversalResolver testResolver = new UniversalResolver(
            ensRegistry,
            testGateways
        );
        ...
        try testResolver.requireResolver(dnsEncodeName(TEST_NAME)) {
            ...
            console.log(
                "requireResolver Low level data length:",
                lowLevelData.length
            );
            if (lowLevelData.length >= 4) {
                ...
                if (selector == IUniversalResolver.ResolverNotFound.selector) {
                    console.log(
                        "SUCCESS: Got expected ResolverNotFound error with real EOA"
                    );
                    ...
                    console.log(
                        "UNEXPECTED: Got different error selector with real EOA"
                    );
                }
            }
    ...
    function testResolveNotAContract() public {
        ...
        try testResolver.requireResolver(dnsEncodeName(TEST_NAME)) {
            ...
            if (lowLevelData.length >= 4) {
                ...
                // Check if this is the expected error
                if (
                    selector == IUniversalResolver.ResolverNotContract.selector
                ) {
                    console.log(
                        "SUCCESS: Got expected ResolverNotContract error with real EOA"
                    );
                    // Test passes - restore to expect the correct error
                    vm.expectRevert(
                        abi.encodeWithSelector(
    ...
    function testResolveResolverRevert() public {
        ...
        vm.expectRevert(
            abi.encodeWithSignature("ResolverError(bytes)", DUMMY_CALLDATA)
        );
        universalResolver.resolve(dnsEncodeName(TEST_NAME), DUMMY_CALLDATA);
    }
    ...
    function testResolveOldWithMulticallOneRevert() public {
        ...
        bytes memory nameCall = abi.encodeWithSignature(
            "name(bytes32)",
            ENSTestUtils.namehash(TEST_NAME)
        );
    ...
    function testResolveBatchGatewayRevert() public {
        ...
        testResolver.resolveWithGateways(
            dnsEncodeName(TEST_NAME),
            DUMMY_CALLDATA,
            testGateways
        );
    ...
    function testReverseUnsetReverseResolver() public {
        vm.expectRevert(
            abi.encodeWithSignature(
                "ResolverNotFound(bytes)",
                dnsEncodeName(getReverseName(owner, COIN_TYPE_ETH))
            )
        );
    ...
    function testReverseUnsetPrimaryResolver() public {
        ...
        vm.prank(owner);
        ensRegistry.setResolver(
            ENSTestUtils.namehash(reverseName),
            address(oldResolver)
        );
    ...
    function testReverseUnsetName() public {
        ...
        assertEq(
            reverseResolver,
            address(shapeshift1),
            "Reverse resolver should be shapeshift1"
        );
    }
    ...
    function testReverseOnchainImmediateNameAndAddr() public {
        ...
        assertEq(
            reverseResolver,
            address(oldResolver),
            "Reverse resolver should be oldResolver"
        );
    }
    ...
    function testReverseOnchainImmediateNameAndFallbackAddr() public {
        ...
        // Set up forward address resolution with EVM bit (multicoin addr)
        bytes memory addrCall = abi.encodeWithSignature(
        ...
        // The new universal resolver no longer falls back to addr(bytes32)
        // and throws UnsupportedResolverProfile when addr(bytes32) is not supported
        vm.expectRevert(
            abi.encodeWithSelector(
                IUniversalResolver.UnsupportedResolverProfile.selector,
                bytes4(0x3b3b57de)
            )
        );
        universalResolver.reverse(abi.encodePacked(owner), COIN_TYPE_ETH);
    ...
    function testReverseOnchainImmediateNameAndMismatchAddr() public {
        ...
        vm.expectRevert(
            abi.encodeWithSignature(
                ...
                TEST_NAME,
                abi.encodePacked(ANOTHER_ADDRESS)
            )
        );
        universalResolver.reverse(abi.encodePacked(owner), COIN_TYPE_ETH);
    }
    ...
    function testReverseOnchainImmediateNameAndOnchainImmediateUnimplementedAddr()
        public
    {
        string memory reverseName = getReverseName(owner);
        takeControl(reverseName);
    ...
    function testReverseOffchainExtendedNameAndOnchainImmediateAddr() public {
        ...
        vm.startPrank(owner);
        ensRegistry.setResolver(
            ENSTestUtils.namehash(getParentName(reverseName)),
            address(shapeshift1)
        );
        shapeshift1.setExtended(true);
        shapeshift1.setOffchain(true);
