...
contract TestBaseRegistrar is BaseTest {
    ...
    function testShouldAllowNewRegistrations() public {
        ...
        assertTrue(baseRegistrar.available(tokenId), "Name should be available for registration");
    ...
    function testShouldAllowRegistrationsWithoutUpdatingTheRegistry() public {
        ...
        baseRegistrar.registerOnly(tokenId, registrantAccount, REGISTRATION_TIME);
    ...
    function testShouldAcceptAnyTokenIdRegardlessOfLabelLength() public {
        ...
        if (baseRegistrar.available(emptyLabelId)) {
            baseRegistrar.register(emptyLabelId, registrantAccount, REGISTRATION_TIME);
        ...
        if (baseRegistrar.available(singleCharId)) {
            baseRegistrar.register(singleCharId, registrantAccount, REGISTRATION_TIME);
    ...
    function testShouldPermitRegistrationOfAnyAvailableTokenId() public {
        ...
        baseRegistrar.register(toLabelId("abc"), registrantAccount, REGISTRATION_TIME);
        assertEq(baseRegistrar.ownerOf(toLabelId("abc")), registrantAccount);
        
        // Test longer label 
        string memory longerLabel = "abcdefghijklmnopqrstuvwxyzabcdefghijklmnopqrstuvwxyzabcdefghij";
        baseRegistrar.register(toLabelId(longerLabel), registrantAccount, REGISTRATION_TIME);
        assertEq(baseRegistrar.ownerOf(toLabelId(longerLabel)), registrantAccount);
    ...
    function testShouldAllowAControllerToRegisterAPreviouslyOwnedExpiredName() public {
        // Fast-forward past grace period so names are available
        vm.warp(block.timestamp + 91 days);
    ...
    function testShouldUpdateENSRegistryWhenNFTIsTransferredAndReclaimIsCalled() public {
        // Fast-forward past grace period so names are available
        vm.warp(block.timestamp + 91 days);
    ...
    function testShouldReturnCorrectNameAndSymbol() public view {
        ...
        assertEq(
            baseRegistrar.symbol(),
            "",
            "Symbol should be empty string"
        );
    }
}
