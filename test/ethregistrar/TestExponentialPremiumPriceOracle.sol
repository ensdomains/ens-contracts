...
contract TestExponentialPremiumPriceOracle is Test {
    ...
    uint256 constant LAST_VALUE_WITH_FACTOR =
        START_PRICE_WITH_FACTOR / HALVING_DIVISOR;
    ...
    function testShouldReturnCorrectBasePrices() public {
        ...
        IPriceOracle.Price memory priceResultA = priceOracle.price(
            "foo",
            0,
            3600
        );
        ...
        IPriceOracle.Price memory priceResultB = priceOracle.price(
            "quux",
            0,
            3600
        );
        ...
        IPriceOracle.Price memory priceResultC = priceOracle.price(
            "fubar",
            0,
            3600
        );
        ...
        IPriceOracle.Price memory priceResultD = priceOracle.price(
            "foobie",
            0,
            3600
        );
    ...
    function testShouldNotSpecifyAPremiumForFirstTimeRegistrations() public {
        ...
        IPriceOracle.Price memory priceResultE = priceOracle.price(
            "foobar",
            0,
            0
        );
    ...
    function testShouldNotSpecifyAPremiumForRenewals() public {
        ...
        IPriceOracle.Price memory priceResult = priceOracle.price(
            "foobar",
            timestamp,
            0
        );
        ...
        public
    {
        ...
        uint256 expectedPrice = (START_PRICE_WITH_FACTOR -
            LAST_VALUE_WITH_FACTOR) / 2; // ETH at $2 for $1 mil in 18 decimal precision

        uint256 premium = priceOracle.premium("foobar", timestamp, 0);
        assertEq(
            premium,
            expectedPrice,
            "Premium should equal expected price at moment of expiration"
        );

        IPriceOracle.Price memory priceResult2 = priceOracle.price(
            "foobar",
            timestamp,
            0
        );
        ...
        assertEq(
            premiumFromPrice,
            expectedPrice,
            "Premium from price() should equal expected price"
        );
    }

    // TEST 5: "should specify a reasonable price after 2.5 days into decay period"
    function testShouldSpecifyTheCorrectPriceAfter2Point5DaysAnd1YearRegistration()
        public
    {
        ...
        uint256 contractPremium = priceOracle.premium(
            "foobar",
            timestamp,
            lengthOfRegistration
        );

        // At 2.5 days into decay, should have reasonable premium (less than max, more than 0)
        uint256 maxPremium = (START_PRICE_WITH_FACTOR -
            LAST_VALUE_WITH_FACTOR) / 2; // Max premium in ETH
        assertTrue(
            contractPremium > 0,
            "Premium should be greater than 0 during decay period"
        );
        assertTrue(
            contractPremium < maxPremium,
            "Premium should be less than maximum after 2.5 days"
        );

        // Test that premium decreases over time (check at 3 days)
        uint256 laterTimestamp = block.timestamp - (90 * DAY + 3 * DAY);
        uint256 laterPremium = priceOracle.premium(
            "foobar",
            laterTimestamp,
            lengthOfRegistration
        );
        assertTrue(
            laterPremium < contractPremium,
            "Premium should decrease over time"
        );

        // Also test price() function
        IPriceOracle.Price memory priceResult3 = priceOracle.price(
            "foobar",
            timestamp,
            lengthOfRegistration
        );
        ...
        assertEq(
            premiumFromPrice,
            contractPremium,
            "Premium from price() should match premium() result"
        );
    }
    ...
    function testShouldProduceA0PremiumAtTheEndOfTheDecayPeriod() public {
        ...
        uint256 premiumBeforeEnd = priceOracle.premium(
            "foobar",
            timestamp - LAST_DAY * DAY + 1,
            0
        );
        assertGt(
            premiumBeforeEnd,
            0,
            "Premium should be greater than 0 before end of decay period"
        );

        uint256 premiumAtEnd = priceOracle.premium(
            "foobar",
            timestamp - LAST_DAY * DAY,
            0
        );
    ...
    function testShouldNotBeBeyondACertainAmountOfInaccuracyFromFloatingPointCalc()
        public
    {
        vm.warp(block.timestamp + 365 * DAY); // Warp forward to have enough time
        uint256 timestamp = block.timestamp - 90 * DAY;
        ...
        assertTrue(
            initialPremium > 0,
            "Should have premium at start of decay period"
        );

        uint256 midPremium = priceOracle.premium(
            "foobar",
            timestamp - 10 * DAY,
            0
        ); // 10 days into decay
        assertTrue(
            midPremium < initialPremium,
            "Premium should decrease over time"
        );

        uint256 latePremium = priceOracle.premium(
            "foobar",
            timestamp - 20 * DAY,
            0
        ); // 20 days into decay
        assertTrue(
            latePremium < midPremium,
            "Premium should continue decreasing"
        );

        uint256 endPremium = priceOracle.premium(
            "foobar",
            timestamp - LAST_DAY * DAY,
            0
        ); // At end of decay
        ...
        uint256 oneDayPremium = priceOracle.premium(
            "foobar",
            timestamp - 1 * DAY,
            0
        );
        ...
        assertTrue(
            ratio >= 18 && ratio <= 22,
            "One day decay should roughly halve the premium"
        );
    ...
    function testCompleteFixtureSetup() public {
        ...
        // Verify oracle setup
        assertEq(
            dummyOracle.latestAnswer(),
            200000000,
            "Dummy oracle should return 2 USD per ETH"
        );
    }
    ...
    function testPriceCalculationComponents() public {
        ...
        IPriceOracle.Price memory priceResult4 = priceOracle.price(
            "foo",
            0,
            3600
        ); // 3 chars
        uint256 base3 = priceResult4.base;
        IPriceOracle.Price memory priceResult5 = priceOracle.price(
            "test",
            0,
            3600
        ); // 4 chars
        uint256 base4 = priceResult5.base;
        IPriceOracle.Price memory priceResult6 = priceOracle.price(
            "testing",
            0,
            3600
        ); // 7 chars
    ...
    function testPremiumCalculationEdgeCases() public {
        ...
        uint256 veryOldPremium = priceOracle.premium(
            "test",
            expiredTimestamp,
            0
        );
        ...
        uint256 premiumAtExpiry = priceOracle.premium(
            "test",
            currentTimestamp,
            0
        );
        ...
        assertGt(
            recentPremium,
            0,
            "Recently expired names should have premium after grace period"
        );
    }

    function testZeroDurationHandling() public {
        // Test zero duration scenarios
        IPriceOracle.Price memory priceResult7 = priceOracle.price(
            "test",
            0,
            0
        );
        ...
        uint256 premiumZeroDuration = priceOracle.premium(
            "test",
            expiredTime,
            0
        );
    ...
    function testLargeDurationHandling() public {
        ...
        IPriceOracle.Price memory priceResult8 = priceOracle.price(
            "test",
            0,
            largeDuration
        );
    ...
    function testExactDecayBoundaries() public {
        ...
        uint256 premiumAtBoundary = priceOracle.premium(
            "test",
            boundaryTime,
            0
        );
        assertEq(
            premiumAtBoundary,
            0,
            "Premium should be exactly 0 at LAST_DAY boundary"
        );

        // Test just before boundary
        uint256 premiumBeforeBoundary = priceOracle.premium(
            "test",
            boundaryTime + 1,
            0
        );
        assertGt(
            ...
            0,
            "Premium should be > 0 just before boundary"
        );
    }
    ...
    function testPremiumStartPriceConstants() public {
        ...
        uint256 expectedMaxPremium = (START_PRICE_WITH_FACTOR -
            LAST_VALUE_WITH_FACTOR) / 2;
        assertEq(
            maxPremium,
            expectedMaxPremium,
            "Premium should start at maximum expected value"
        );
    }
