...
contract('BaseRegistrar', function (accounts) {
	...
	before(async () => {
		...
		registrar = await BaseRegistrar.new(ens.address, namehash.hash('arb'), {from: ownerAccount});
	...
	it('should allow new registrations', async () => {
		...
		assert.equal(await ens.owner(namehash.hash("newname.arb")), registrantAccount);
	...
	it('should allow registrations without updating the registry', async () => {
		...
		var block = await web3.eth.getBlock(tx.receipt.blockHash);
		assert.equal(await ens.owner(namehash.hash("silentname.arb")), ZERO_ADDRESS);
	...
	it('should permit the owner to reclaim a name', async () => {
		...
		await ens.setSubnodeOwner(namehash.hash("arb"), sha3("newname"), ZERO_ADDRESS);
		assert.equal(await ens.owner(namehash.hash("newname.arb")), ZERO_ADDRESS);
		await ens.setSubnodeOwner(ZERO_HASH, sha3("arb"), registrar.address);
	...
	it('should allow the owner to set a resolver address', async () => {
		...
		assert.equal(await ens.resolver(namehash.hash('arb')), accounts[1]);
	});
});
