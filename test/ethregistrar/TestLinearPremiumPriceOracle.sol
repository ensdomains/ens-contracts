...
contract TestLinearPremiumPriceOracle is BaseTest {
    ...
    function testShouldReportTheCorrectPremiumAndDecreaseRate() public {
        ...
        assertEq(initialPremium, INITIAL_PREMIUM, "Initial premium should match constructor parameter");
        
        uint256 decreaseRate = linearPriceOracle.premiumDecreaseRate();
        assertEq(decreaseRate, PREMIUM_DECREASE_RATE, "Premium decrease rate should match constructor parameter");
    }
    
    // TEST 2: "should return correct base prices"
    function testShouldReturnCorrectBasePrices() public {
        IPriceOracle.Price memory priceResult1 = linearPriceOracle.price("foo", 0, 3600);
        uint256 base1 = priceResult1.base;
        assertEq(base1, 7200, "foo should have base price 7200");
        
        IPriceOracle.Price memory priceResult2 = linearPriceOracle.price("quux", 0, 3600);
        uint256 base2 = priceResult2.base;
        assertEq(base2, 3600, "quux should have base price 3600");
        
        IPriceOracle.Price memory priceResult3 = linearPriceOracle.price("fubar", 0, 3600);
        uint256 base3 = priceResult3.base;
        assertEq(base3, 1800, "fubar should have base price 1800");
        
        IPriceOracle.Price memory priceResult4 = linearPriceOracle.price("foobie", 0, 3600);
    ...
    function testShouldNotSpecifyAPremiumForFirstTimeRegistrations() public {
        ...
        IPriceOracle.Price memory priceResult5 = linearPriceOracle.price("foobar", 0, 0);
    ...
    function testShouldNotSpecifyAPremiumForRenewals() public {
        ...
        IPriceOracle.Price memory priceResult6 = linearPriceOracle.price("foobar", timestamp, 0);
    ...
    function testShouldSpecifyTheMaximumPremiumAtTheMomentOfExpiration() public {
        ...
        uint256 premium = linearPriceOracle.premium("foobar", timestamp, 0);
        assertEq(premium, 50000000000000000000, "Premium should be 50 ETH at moment of expiration");
        
        IPriceOracle.Price memory priceResultA = linearPriceOracle.price("foobar", timestamp, 0);
        uint256 premiumFromPrice = priceResultA.premium;
        assertEq(premiumFromPrice, 50000000000000000000, "Premium from price() should be 50 ETH");
    }
    ...
    function testShouldSpecifyHalfThePremiumAfterHalfTheInterval() public {
        ...
        uint256 premium = linearPriceOracle.premium("foobar", timestamp, 0);
        assertEq(premium, 25000000000000000000, "Premium should be 25 ETH after half the interval");
        
        IPriceOracle.Price memory priceResultB = linearPriceOracle.price("foobar", timestamp, 0);
        uint256 premiumFromPrice = priceResultB.premium;
        assertEq(premiumFromPrice, 25000000000000000000, "Premium from price() should be 25 ETH");
    }
    ...
    function testShouldReturnCorrectTimesForPriceQueries() public {
        ...
        uint256 timeUntilInitialPremium = linearPriceOracle.timeUntilPremium(0, initialPremiumWei);
        assertEq(timeUntilInitialPremium, 90 * DAY, "Time until initial premium should be 90 days");
        
        uint256 timeUntilZeroPremium = linearPriceOracle.timeUntilPremium(0, 0);
        assertEq(timeUntilZeroPremium, 90 * DAY + 100000, "Time until zero premium should be 90 days + 100000 seconds");
    }
    ...
    function testCompleteFixtureSetup() public {
        ...
        assertTrue(address(linearPriceOracle) != address(0), "Price Oracle should be deployed");
        ...
        // Verify oracle setup
        assertEq(testDummyOracle.latestAnswer(), 200000000, "Test dummy oracle should return 2 USD per ETH");
    }
    ...
    function testLinearDecayCalculation() public {
        ...
        for (uint256 i = 0; i < testOffsets.length; i++) {
            uint256 premium = linearPriceOracle.premium("test", expiredTime - testOffsets[i], 0);
            
            if (i == 2) {
                assertEq(premium, expectedPremiums[i], "Premium should match at 50k seconds");
            } else if (i == 4) {
                ...
                assertTrue(premium <= expectedPremiums[0], "Premium should not exceed initial premium");
                if (i > 0) {
                    uint256 prevPremium = linearPriceOracle.premium("test", expiredTime - testOffsets[i-1], 0);
                    assertTrue(premium <= prevPremium, "Premium should decrease over time");
                }
            }
        }
    }
    ...
    function testPriceCalculationComponents() public {
        ...
        IPriceOracle.Price memory priceResult8 = linearPriceOracle.price("foo", 0, 3600); // 3 chars
        uint256 base3 = priceResult8.base;
        IPriceOracle.Price memory priceResult9 = linearPriceOracle.price("test", 0, 3600); // 4 chars
        uint256 base4 = priceResult9.base;
        IPriceOracle.Price memory priceResult10 = linearPriceOracle.price("testing", 0, 3600); // 7 chars
    ...
    function testPremiumCalculationEdgeCases() public {
        ...
        uint256 veryOldPremium = linearPriceOracle.premium("test", expiredTimestamp, 0);
        assertEq(veryOldPremium, 0, "Very old expired names should have 0 premium");
        
        // Test premium exactly at expiry time
        uint256 currentTimestamp = block.timestamp;
        uint256 premiumAtExpiry = linearPriceOracle.premium("test", currentTimestamp, 0);
        ...
        uint256 recentPremium = linearPriceOracle.premium("test", recentExpiry, 0);
        assertEq(recentPremium, 0, "Names in grace period should have 0 premium before linear decay starts");
    }
    ...
    function testTimeUntilPremiumCalculations() public {
        ...
        uint256 timeUntilHalf = linearPriceOracle.timeUntilPremium(0, halfInitialPremium);
        
        // This should be 90 days + 50000 seconds (when premium reaches 25 ETH)
        assertEq(timeUntilHalf, 90 * DAY + 50000, "Time until half premium should be 90 days + 50000 seconds");
        
        // Test with premium equal to quarter of initial premium
        uint256 quarterInitialPremium = 12500000000000000000; // 12.5 ETH
        uint256 timeUntilQuarter = linearPriceOracle.timeUntilPremium(0, quarterInitialPremium);
        
        // This should be 90 days + 75000 seconds
        assertEq(timeUntilQuarter, 90 * DAY + 75000, "Time until quarter premium should be 90 days + 75000 seconds");
        ...
        uint256 timeUntilInitial = linearPriceOracle.timeUntilPremium(0, initialPremium);
        
        // This should be just the grace period (90 days) since this is when decay starts
        assertEq(timeUntilInitial, 90 * DAY, "Time until initial premium should be grace period");
    }
    
    function testZeroDurationHandling() public {
        // Test zero duration scenarios
        IPriceOracle.Price memory priceResult11 = linearPriceOracle.price("test", 0, 0);
        ...
        uint256 premiumZeroDuration = linearPriceOracle.premium("test", expiredTime, 0);
    ...
    function testLargeDurationHandling() public {
        ...
        IPriceOracle.Price memory priceResult12 = linearPriceOracle.price("test", 0, largeDuration);
    ...
    function testDecayPeriodBoundaries() public {
        ...
        uint256 premiumAtStart = linearPriceOracle.premium("test", expiredTime, 0);
        assertEq(premiumAtStart, 50000000000000000000, "Premium should be at maximum at start of decay");
        
        // Test at end of decay period (90 days + 100k seconds after expiry)
        uint256 premiumAtEnd = linearPriceOracle.premium("test", expiredTime - 100000, 0);
        assertEq(premiumAtEnd, 0, "Premium should be 0 at end of decay period");
        
        // Test just before end of decay period
        uint256 premiumBeforeEnd = linearPriceOracle.premium("test", expiredTime - 99999, 0);
        assertGt(premiumBeforeEnd, 0, "Premium should be > 0 just before end of decay period");
    }
    ...
    function testMultipleNameLengths() public {
        ...
        for (uint256 i = 0; i < testNames.length; i++) {
            IPriceOracle.Price memory priceResult = linearPriceOracle.price(testNames[i], 0, 3600);
    ...
    function testLinearDecayFormula() public {
        ...
        for (uint256 i = 0; i < testTimes.length; i++) {
            ...
            uint256 premium = linearPriceOracle.premium("test", nameExpiredTime, 0);
            ...
            if (timeIntoDecay * decreaseRateInWei < expectedPremium) {
                expectedPremium = expectedPremium - (timeIntoDecay * decreaseRateInWei);
            } else {
                expectedPremium = 0;
            }
            
            assertEq(premium, expectedPremium, string(abi.encodePacked("Premium should follow linear decay formula at time ", vm.toString(timeIntoDecay))));
        }
    }
}
