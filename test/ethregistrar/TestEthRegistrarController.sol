...
contract TestEthRegistrarController is BaseTest {
    ...
    function makeCommitment(
        ...
        IETHRegistrarController.Registration
            memory registration = IETHRegistrarController.Registration({
                ...
                referrer: bytes32(uint256(ownerControlledFuses))
            });
        return controller.makeCommitment(registration);
    }
    ...
    function testShouldReportLabelValidity() public view {
        ...
        assertTrue(
            controller.valid("longname12345678"),
            "longname12345678 should be valid"
        );
        ...
        // "你好吗" in UTF-8 hex
        assertTrue(
            controller.valid(unicode"你好吗"),
            "Chinese characters should be valid"
        ); // { ni } { hao } { ma } (chinese; simplified)

        assertTrue(
            controller.valid(unicode"💩💩💩"),
            "3 emoji should be valid"
    ...
    function testShouldPermitNewRegistrations() public {
        ...
        IPriceOracle.Price memory priceResult = controller.rentPrice(
            label,
            REGISTRATION_TIME
        );
        ...
        controller.register{value: totalCost}(registration);
        ...
        assertEq(
            ...
            "Balance should increase by registration cost"
        );
    ...
    function testShouldReportRegisteredNamesAsUnavailable() public {
        ...
        controller.register{value: BUFFERED_REGISTRATION_COST}(registration);

        // Should now be unavailable
        assertFalse(
            controller.available(label),
            "Registered name should be unavailable"
        );
    ...
    function testShouldPermitNewRegistrationsWithResolverAndRecords() public {
        ...
        assertEq(
            publicResolver.text(node, "url"),
            "ethereum.com",
            "Text record should be set"
        );
    ...
    function testShouldNotPermitNewRegistrationsWithDataAndZeroResolver()
        public
    {
        vm.startPrank(registrantAccount);
        ...
        vm.expectRevert(
            abi.encodeWithSignature("ResolverRequiredWhenDataSupplied()")
        );
        bytes32 commitment = makeCommitment(
            label,
            registrantAccount,
    ...
    function testShouldNotPermitNewRegistrationsWithEoAResolver() public {
        ...
        IETHRegistrarController.Registration
            memory registration = IETHRegistrarController.Registration({
                ...
                duration: REGISTRATION_TIME,
                secret: secret,
                resolver: registrantAccount, // EOA as resolver
                data: resolverData, // Non-empty data triggers validation
                reverseRecord: 0,
                referrer: 0
            });
        controller.register{value: BUFFERED_REGISTRATION_COST}(registration);
    ...
    function testCommitmentTiming() public {
        ...
        assertTrue(
            commitTime + minAge > block.timestamp,
            "Commitment should be too new immediately"
        );
        ...
        assertTrue(
            commitTime + minAge <= block.timestamp,
            "Commitment should be old enough after min age"
        );
        assertTrue(
            commitTime + maxAge > block.timestamp,
            "Commitment should not be too old yet"
        );
        ...
        assertTrue(
            commitTime + maxAge <= block.timestamp,
            "Commitment should be too old after max age"
        );
    }
    ...
    function testRentPrice() public view {
        ...
        IPriceOracle.Price memory longerPriceResult = controller.rentPrice(
            "testlonger",
            REGISTRATION_TIME
        );
        uint256 longerTotalPrice = longerPriceResult.base +
            longerPriceResult.premium;
        assertLe(
            longerTotalPrice,
            totalPrice,
            "Longer names should not be more expensive"
        );
    }
    ...
    function testSupportsInterface() public view {
        ...
        assertTrue(
            controller.supportsInterface(type(IERC165).interfaceId),
            "Should support ERC165 interface"
        );
    }

    // TEST 9: "should not permit new registrations with records updating a different name"
    function testShouldNotPermitNewRegistrationsWithRecordsUpdatingDifferentName()
        public
    {
        vm.startPrank(registrantAccount);
    ...
    function testShouldPermitRegistrationWithResolverButNoRecords() public {
        ...
        assertEq(
            ens.resolver(nodehash),
            address(publicResolver),
            "Resolver should be set"
        );
        assertEq(
            publicResolver.addr(nodehash),
            address(0),
            "Address record should be empty"
        );
        assertEq(
            address(controller).balance,
            baseCost + premium,
            "Controller should have registration fee"
        );
    ...
    function testShouldIncludeOwnerInCommitment() public {
        ...
        IETHRegistrarController.Registration
            memory registration = IETHRegistrarController.Registration({
                label: label,
                owner: registrantAccount, // Different owner than in commitment!
                duration: REGISTRATION_TIME,
                secret: secret,
                resolver: address(0),
                data: emptyData,
    ...
    function testShouldRejectDuplicateRegistrations() public {
        ...
        IPriceOracle.Price memory priceResult2 = controller.rentPrice(
            label,
            REGISTRATION_TIME
        );
        uint256 totalCost2 = priceResult2.base + priceResult2.premium;
        vm.expectRevert(
            abi.encodeWithSignature("NameNotAvailable(string)", label)
        );
        IETHRegistrarController.Registration
            memory registration2 = IETHRegistrarController.Registration({
                label: label,
                owner: registrantAccount,
                duration: REGISTRATION_TIME,
                secret: secret2,
        ...
        controller.register{value: totalCost2}(registration2);
    ...
    function testShouldRejectExpiredCommitments() public {
        ...
        IPriceOracle.Price memory priceResult3 = controller.rentPrice(
            label,
            REGISTRATION_TIME
        );
        uint256 totalCost3 = priceResult3.base + priceResult3.premium;
        // The new ETHRegistrarController throws CommitmentTooOld with additional parameters
        vm.expectRevert();
        IETHRegistrarController.Registration
            memory registration3 = IETHRegistrarController.Registration({
                label: label,
                owner: registrantAccount,
                duration: REGISTRATION_TIME,
                secret: secret,
        ...
        controller.register{value: totalCost3}(registration3);
    ...
    function testShouldAllowAnyoneToRenewName() public {
        ...
        IPriceOracle.Price memory renewalPriceResult = controller.rentPrice(
            label,
            REGISTRATION_TIME
        );
        uint256 renewalCost = renewalPriceResult.base +
            renewalPriceResult.premium;
        bytes32 labelHash = keccak256(bytes(label));
        ...
        controller.renew{value: renewalCost}(label, REGISTRATION_TIME, 0);
    ...
    function testShouldRequireSufficientValueForRenewal() public {
        ...
        vm.expectRevert(abi.encodeWithSignature("InsufficientValue()"));
        controller.renew{value: 0}(label, REGISTRATION_TIME, 0);
    ...
    function testShouldAllowAnyoneToWithdrawFunds() public {
        ...
        assertEq(
            address(controller).balance,
            0,
            "Controller should have no funds left"
        );
    }
    ...
    function testShouldSetReverseRecordOfAccount() public {
        ...
        // Register with reverse record enabled - now requires a resolver
        commitAndWait(
            label,
            registrantAccount,
            REGISTRATION_TIME,
        ...
        // Reverse record setup now requires a resolver to be set
        IETHRegistrarController.Registration
            memory registration = IETHRegistrarController.Registration({
                label: label,
                owner: registrantAccount,
                duration: REGISTRATION_TIME,
                secret: secret,
                resolver: address(publicResolver), // Resolver required for reverse record
                data: emptyData,
            ...
            controller.register{value: BUFFERED_REGISTRATION_COST}(registration)
        {
            ...
            assertTrue(
                baseRegistrar.nameExpires(tokenId) > block.timestamp,
            ...
            IETHRegistrarController.Registration
                memory newRegistration = IETHRegistrarController.Registration({
                    label: label,
                    owner: registrantAccount,
                    duration: REGISTRATION_TIME,
                    secret: newSecret,
                    resolver: address(0),
                    data: emptyData,
                    reverseRecord: 0, // Don't set reverse record
                    referrer: bytes32(0)
                });
            ...
            assertTrue(
                baseRegistrar.nameExpires(tokenId) > block.timestamp,
                "Name should be registered even without reverse record"
            );
        }
    ...
    // TEST 18: "should register name directly to owner (no auto-wrapping)"
    function testShouldAutoWrapNameAndSetERC721OwnerToWrapper() public {
        ...
        // Register without resolver (direct registration)
        commitAndWait(
            label,
            registrantAccount,
            REGISTRATION_TIME,
        ...
        // Verify name was registered directly to owner (no auto-wrapping)
        uint256 tokenId = toLabelId(label);
        assertEq(
            baseRegistrar.ownerOf(tokenId),
            registrantAccount,
            "BaseRegistrar token should be owned by registrant"
        );
    ...
    function testMakeCommitmentFunction() public view {
        ...
        bytes32 commitment1 = makeCommitment(
            label,
            owner,
            duration,
            secret,
        ...
        bytes32 commitment2 = makeCommitment(
            label,
            owner,
            duration,
            secret,
        ...
        bytes32 commitment3 = makeCommitment(
            label,
            owner,
            duration,
            keccak256("different"),
    ...
    function testBulkCommitments() public {
        ...
        for (uint256 i = 0; i < commitments.length; i++) {
            ...
            assertGt(commitTime, 0, "Commitment should be recorded");
            assertTrue(
                commitTime + controller.minCommitmentAge() <= block.timestamp,
        ...
        public
    {
        ...
        bytes32 node = keccak256(
            abi.encodePacked(ETH_NODE, keccak256(bytes(label)))
        );
        ...
        IETHRegistrarController.Registration
            memory registration = IETHRegistrarController.Registration({
                ...
                duration: REGISTRATION_TIME,
                secret: secret,
                resolver: address(controller), // Incompatible contract as resolver
                data: resolverData,
                reverseRecord: 0,
                referrer: 0
            });
    ...
    function testShouldNotPermitNewRegistrationsWithAnyRecordUpdatingDifferentName()
        public
    {
        vm.startPrank(registrantAccount);
        ...
        bytes32 differentNode = keccak256(
            abi.encodePacked(ETH_NODE, keccak256(bytes("different")))
        );
        resolverData[1] = abi.encodeWithSignature(
            "setText(bytes32,string,string)",
            differentNode,
    ...
    function testShouldNotPermitNewRegistrationsWithNonResolverFunctionCalls()
        public
    {
        vm.startPrank(registrantAccount);
    ...
    function testNonWrappedNamesCanRenew() public {
        // The ETHRegistrarController no longer auto-wraps names
        // This test verifies that directly registered names can still be renewed
        vm.startPrank(registrantAccount);
        ...
        // Names are now registered directly to the owner, not wrapped
        assertEq(
            baseRegistrar.ownerOf(tokenId),
            registrantAccount,
            "BaseRegistrar token should be owned by registrant"
        ...
        assertEq(
            baseRegistrar.ownerOf(tokenId),
            registrantAccount,
            "BaseRegistrar token should still be owned by registrant"
        );
    ...
    function testApprovalShouldReduceGasForRegistration() public {
        ...
        IETHRegistrarController.Registration
            memory registration1 = IETHRegistrarController.Registration({
                label: label,
                owner: registrantAccount,
                duration: REGISTRATION_TIME,
                secret: secret1,
        ...
        controller.register{value: BUFFERED_REGISTRATION_COST}(registration1);
        ...
        controller.register{value: BUFFERED_REGISTRATION_COST}(registration2);
        ...
        assertTrue(
            baseRegistrar.nameExpires(toLabelId(label)) > 0,
            "First registration should succeed"
        );
        assertTrue(
            baseRegistrar.nameExpires(toLabelId(label2)) > 0,
