...
contract TestBulkRenewal is BaseTest {
    ...
    function getETHRegistrarControllerInterfaceId()
        internal
        pure
        returns (bytes4)
    {
    ...
    function testShouldRaiseAnErrorTryingToRenewANonexistentName() public {
        ...
        bulkRenewal.renewAll{value: cost}(names, duration, 0); // 0 referrer
    }
    ...
    function testShouldPermitBulkRenewalOfNames() public {
        ...
        assertEq(
            newExpiry - oldExpiry,
            duration,
            "New expiry should be old expiry + duration"
        );

        // Check any excess funds are returned
        // expect(await publicClient.getBalance({ address: bulkRenewal.address })).equal(0n)
        assertEq(
            address(bulkRenewal).balance,
            0,
            "Bulk renewal contract should have no remaining balance"
        );
    }
    ...
    function testCompleteFixtureSetup() public {
        ...
        assertTrue(
            address(bulkRenewal) != address(0),
            "Bulk Renewal should be deployed"
        );
        assertTrue(
        ...
        assertTrue(
            baseRegistrar.nameExpires(toLabelId(TEST1_LABEL)) > block.timestamp,
            "test1 should be registered"
        );
        assertTrue(
            baseRegistrar.nameExpires(toLabelId(TEST2_LABEL)) > block.timestamp,
            "test2 should be registered"
        );
        assertTrue(
            baseRegistrar.nameExpires(toLabelId(TEST3_LABEL)) > block.timestamp,
            "test3 should be registered"
        );

        // Verify ownership
        assertEq(
            baseRegistrar.ownerOf(toLabelId(TEST1_LABEL)),
            account1,
            "test1 should be owned by account1"
        );
        assertEq(
            baseRegistrar.ownerOf(toLabelId(TEST2_LABEL)),
            account1,
            "test2 should be owned by account1"
        );
        assertEq(
            baseRegistrar.ownerOf(toLabelId(TEST3_LABEL)),
            account1,
            "test3 should be owned by account1"
        );

        // Verify controller setup
        assertTrue(
            baseRegistrar.controllers(address(controller)),
            "Controller should be added to base registrar"
        );
        assertTrue(
        ...
        assertTrue(
            ...
            "Name wrapper should be controller"
        );
    ...
    function testRentPriceCalculation() public {
        ...
        assertEq(
            bulkRenewal.rentPrice(singleName, 86400),
            86400,
            "Single name should cost duration"
        );
        ...
        assertEq(
            bulkRenewal.rentPrice(multipleNames, 86400),
            86400 * 3,
            "Multiple names should cost duration * count"
        );

        // Zero duration
        assertEq(
            bulkRenewal.rentPrice(singleName, 0),
            0,
        ...
        assertEq(
            bulkRenewal.rentPrice(singleName, largeDuration),
            largeDuration,
            "Large duration should work"
        );
    }
    ...
    function testRenewalFunctionality() public {
        ...
        bulkRenewal.renewAll{value: cost}(names, duration, 0);
        ...
        assertEq(
            newExpiry1 - oldExpiry1,
            duration,
            "First name should be renewed by duration"
        );
        assertEq(
            newExpiry2 - oldExpiry2,
            duration,
            "Second name should be renewed by duration"
        );
        ...
        assertEq(
            ...
            0,
            "Contract should start with no funds"
        );
    }
    ...
    function testEdgeCases() public {
        ...
        assertEq(
            bulkRenewal.rentPrice(emptyNames, 86400),
            0,
            "Empty array should cost zero"
        );

        // This should not revert but also not do anything
        bulkRenewal.renewAll{value: 0}(emptyNames, 86400, 0);
        ...
        bulkRenewal.renewAll{value: cost}(manyNames, duration, 0);

        // Verify all were renewed
        assertEq(
            baseRegistrar.nameExpires(toLabelId(TEST1_LABEL)) - oldExpiries[0],
            duration,
            "Name 1 renewed"
        );
        assertEq(
            baseRegistrar.nameExpires(toLabelId(TEST2_LABEL)) - oldExpiries[1],
            duration,
            "Name 2 renewed"
        );
        assertEq(
            baseRegistrar.nameExpires(toLabelId(TEST3_LABEL)) - oldExpiries[2],
            duration,
            "Name 3 renewed"
        );
    }
    ...
    function testExcessPaymentHandling() public {
        ...
        bulkRenewal.renewAll{value: excessPayment}(names, duration, 0);
    ...
    function testInsufficientPayment() public {
        ...
        bulkRenewal.renewAll{value: insufficientPayment}(names, duration, 0);
    }
