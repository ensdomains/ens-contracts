...
contract('ExponentialPricePremiumOracle', function(accounts) {
  ...
  it('should return correct base prices', async () => {
    assert.equal(parseInt(await priceOracle.price('foo', 0, 3600)), 7200)

    assert.equal(parseInt(await priceOracle.price('quux', 0, 3600)), 3600)
    assert.equal(parseInt(await priceOracle.price('fubar', 0, 3600)), 1800)
    assert.equal(parseInt(await priceOracle.price('foobie', 0, 3600)), 1800)
  })
  ...
  it('should not specify a premium for first-time registrations', async () => {
    ...
    assert.equal(parseInt(await priceOracle.price('foobar', 0, 0)), 0)
  })
  ...
  it('should not specify a premium for renewals', async () => {
    ...
    assert.equal(parseInt(await priceOracle.price('foobar', ts, 0)), 0)
  })
