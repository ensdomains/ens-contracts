...
contract TestCCIPBatcher is Test {
    ...
    function testCCIPBatchBasicFunctionality() public {
        ...
        assertTrue((result.lookups[0].flags & FLAG_EIP140_AFTER) != 0, "Should detect EIP-140 support");
        assertTrue((result.lookups[0].flags & FLAG_DONE) != 0, "Should be marked as done");
        assertEq(result.lookups[0].data, abi.encode(uint256(42)), "Should return correct data");
    }
    ...
    function testEIP140DetectionPreEIP140() public {
        ...
        assertTrue((result.lookups[0].flags & FLAG_EIP140_BEFORE) != 0, "Should detect no EIP-140 support");
        assertTrue((result.lookups[0].flags & FLAG_EIP140_AFTER) == 0, "Should not have EIP-140 after flag");
        assertTrue((result.lookups[0].flags & FLAG_DONE) != 0, "Should be marked as done");
    }
    ...
    function testEIP140DetectionPostEIP140() public {
        ...
        assertTrue((result.lookups[0].flags & FLAG_EIP140_AFTER) != 0, "Should detect EIP-140 support");
        assertTrue((result.lookups[0].flags & FLAG_EIP140_BEFORE) == 0, "Should not have EIP-140 before flag");
        assertTrue((result.lookups[0].flags & FLAG_DONE) != 0, "Should be marked as done");
    }
    ...
    function testCallErrorHandling() public {
        ...
        assertTrue((result.lookups[0].flags & FLAG_CALL_ERROR) != 0, "Should have call error flag");
        assertTrue((result.lookups[0].flags & FLAG_DONE) != 0, "Should be marked as done");
    }
    ...
    function testEmptyResponseHandling() public {
        ...
        if ((result.lookups[0].flags & FLAG_EMPTY_RESPONSE) != 0) {
            assertEq(result.lookups[0].data, abi.encodePacked(bytes4(lookups[0].call)), "Should encode function selector");
        }
    }
    ...
    function testOffchainLookupDetection() public {
        ...
        try batcher.ccipBatch(batch) {
            ...
            assertEq(bytes4(reason), OffchainLookup.selector, "Should revert with OffchainLookup");
        }
    }
    ...
    function testMultipleLookupsSameTarget() public {
        ...
        for (uint256 i = 0; i < 3; i++) {
            assertTrue((result.lookups[i].flags & FLAG_EIP140_AFTER) != 0, "All should detect EIP-140 support");
            assertTrue((result.lookups[i].flags & FLAG_DONE) != 0, "All should be marked as done");
        }
    }
    ...
    function testMultipleLookupsDifferentTargets() public {
        ...
        assertTrue((result.lookups[0].flags & FLAG_EIP140_BEFORE) != 0, "First should not support EIP-140");
        assertTrue((result.lookups[1].flags & FLAG_EIP140_AFTER) != 0, "Second should support EIP-140");
        
        // Both should be done
        assertTrue((result.lookups[0].flags & FLAG_DONE) != 0, "First should be done");
        assertTrue((result.lookups[1].flags & FLAG_DONE) != 0, "Second should be done");
    }
    ...
    function testCCIPBatchCallbackValidResponse() public {
        ...
        CCIPBatcher.Batch memory result = batcher.ccipBatchCallback(response, extraData);
        
        assertTrue((result.lookups[0].flags & FLAG_DONE) != 0, "First lookup should be done");
        assertTrue((result.lookups[1].flags & FLAG_DONE) != 0, "Second lookup should be done");
    }
    ...
    function testCCIPBatchCallbackFailures() public {
        ...
        assertTrue((result.lookups[0].flags & FLAG_BATCH_ERROR) != 0, "First should have batch error");
        assertTrue((result.lookups[0].flags & FLAG_DONE) != 0, "First should be done");
        assertTrue((result.lookups[1].flags & FLAG_DONE) != 0, "Second should be done");
        assertTrue((result.lookups[1].flags & FLAG_BATCH_ERROR) == 0, "Second should not have batch error");
    }
    ...
    function testInvalidBatchGatewayResponse() public {
        ...
        bytes memory response = abi.encode(failures, responses);
        bytes memory extraData = abi.encode(CCIPBatcher.Batch({
    ...
    function mockCallback(bytes calldata response, bytes calldata extraData) external pure returns (bytes memory) {
        return response;
    }
}
...
contract TestCCIPBatcherDNSIntegration is Test {
    ...
    function testRealDNSIntegrationWithBatcher() public {
        ...
        for (uint i = 0; i < testDomains.length; i++) {
            DNSTestResult memory result = _testDomainWithBatcher(testDomains[i]);
            
            if (result.success) {
                assertTrue(bytes(result.encoded).length > 0, string(abi.encodePacked("Should encode domain: ", testDomains[i])));
                console.log(string(abi.encodePacked("Successfully processed domain: ", testDomains[i])));
                console.log(string(abi.encodePacked("Encoded as: ", result.encoded)));
            } else {
                console.log(string(abi.encodePacked("Domain processing failed for: ", testDomains[i], " Error: ", result.error)));
            }
        }
    }
    ...
    function testBatchGatewayIntegration() public {
        ...
        if (result.success) {
            ...
            lookups[0] = CCIPBatcher.Lookup({
                ...
                call: abi.encodeWithSignature("resolve(bytes,uint16)", _hexToBytes(result.encoded), uint16(16)),
                data: "",
                flags: 0
            });
            ...
            console.log("Skipping batch integration test - DNS resolution not available");
        }
    }
    ...
    function testDNSEncodingConsistency() public {
        ...
        for (uint i = 0; i < testNames.length; i++) {
            ...
            if (bytes(encoded).length > 0) {
                assertTrue(_isValidHexString(encoded), string(abi.encodePacked("Should be valid hex: ", encoded)));
                console.log(string(abi.encodePacked("Encoded ", testNames[i], " as ", encoded)));
            } else {
                console.log(string(abi.encodePacked("Failed to encode: ", testNames[i])));
            }
        }
    }
    ...
    function testOffchainDNSOracleCompatibility() public {
        ...
        if (oracleAvailable) {
            ...
            for (uint i = 0; i < typeScriptDomains.length; i++) {
                DNSTestResult memory result = _resolveDNSRecord(typeScriptDomains[i], 16);
                if (result.success) {
                    successCount++;
                    console.log("Oracle successfully resolved:", typeScriptDomains[i]);
                }
            }
            
            console.log(string(abi.encodePacked("Oracle resolved ", vm.toString(successCount), " out of ", vm.toString(typeScriptDomains.length), " domains")));
        } else {
    ...
    function _testDomainWithBatcher(string memory domain) internal returns (DNSTestResult memory) {
        ...
        return DNSTestResult({
            ...
            domain: domain,
            encoded: encoded,
            error: dnsResult.success ? "" : "DNS resolution failed but encoding succeeded"
        });
    }
    ...
    function _resolveDNSRecord(string memory domain, uint16 qtype) internal returns (DNSTestResult memory) {
    ...
    function _extractEncodedValue(string memory json) internal pure returns (string memory) {
...
contract MockDNSResolver {
    ...
    function resolve(bytes memory name, uint16 qtype) external view returns (bytes memory) {
        ...
        bytes memory callData = abi.encodeWithSignature("resolveCallback(bytes,bytes)", "", abi.encode(name, qtype));
