...
contract TestLocalBatchGateway is Test {
    ...
    function testBatcherExists() public {
        ...
        assertTrue(
            address(batcher) != address(0),
            "CCIPBatcher should be deployed"
        );
    }
    ...
    function testDNSNameEncoding() public {
        ...
        assertGt(
            BRANTLY_ROCKS.length,
            0,
            "Brantly.rocks DNS encoding should not be empty"
        );
        assertGt(
            RAFFY_XYZ.length,
            0,
            "Raffy.xyz DNS encoding should not be empty"
        );

        // Check that the names end with null byte (proper DNS encoding)
        require(
            uint8(BRANTLY_ROCKS[BRANTLY_ROCKS.length - 1]) == 0x00,
            "DNS name should end with null byte"
        );
        require(
            uint8(RAFFY_XYZ[RAFFY_XYZ.length - 1]) == 0x00,
            "DNS name should end with null byte"
        );
    }
    ...
    function testDNSNameLengths() public {
        ...
        assertEq(
            BRANTLY_ROCKS.length,
            15,
            "brantly.rocks should be 15 bytes when DNS encoded"
        );
        assertEq(
            RAFFY_XYZ.length,
            11,
            "raffy.xyz should be 11 bytes when DNS encoded"
        );
    }
    ...
    function testBatchRequestStructure() public {
        ...
        request.data = abi.encodeWithSignature(
    ...
    function testFunctionSignatureEncoding() public {
        ...
        assertEq(
            encodedCall.length,
            expectedLength,
            "Encoded call should have expected length structure"
        );
    }
    ...
    function testMultipleDomainBatch() public {
        ...
        assertEq(
            domains.length,
            qtypes.length,
            "Domains and qtypes should have same length"
        );
    ...
    function testCCIPBatcherInterface() public {
        ...
        assertTrue(
            address(batcher).code.length > 0,
            "CCIPBatcher should have code"
        );
    ...
    function testDNSEncodingEdgeCases() public {
        ...
        assertEq(
            single.length,
            5,
            "Single label without null should be 5 bytes"
        );
        ...
        assertEq(
            singleProper.length,
            6,
            "Single label with null should be 6 bytes"
        );
        require(
            uint8(singleProper[singleProper.length - 1]) == 0x00,
