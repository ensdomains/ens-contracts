...
contract TestCCIPIntegration is Test {
    ...
    CCIPBatcher public ccipBatcher;
    ...
    function testRealDNSResolution() public {
        for (uint i = 0; i < testDomains.length; i++) {
            ...
            assertTrue(result.success, string(abi.encodePacked("DNS resolution failed for ", testDomains[i], ": ", result.error)));
            assertTrue(result.data.length > 0, string(abi.encodePacked("No DNS data for ", testDomains[i])));
            
            console.log(string(abi.encodePacked("DNS resolution for ", testDomains[i], " returned ", vm.toString(result.data.length), " records")));
        }
    }
    ...
    function testDNSEncoding() public {
        ...
        for (uint i = 0; i < testNames.length; i++) {
            ...
            assertTrue(bytes(encoded).length > 0, "DNS encoding should not be empty");
            
            // Verify encoding is valid hex
            assertTrue(_isValidHex(encoded), "DNS encoding should be valid hex");
            
            console.log(string(abi.encodePacked("DNS encoded ", testNames[i], " to ", encoded)));
        }
    }
    ...
    function testDNSSECRecords() public {
        ...
        for (uint i = 0; i < dnssecDomains.length; i++) {
            ...
            if (result.success) {
                assertTrue(result.dnssecRecords.length > 0, string(abi.encodePacked("No DNSSEC records for ", dnssecDomains[i])));
                console.log(string(abi.encodePacked("DNSSEC records for ", dnssecDomains[i], ": ", vm.toString(result.dnssecRecords.length))));
            } else {
                console.log(string(abi.encodePacked("DNSSEC not available for ", dnssecDomains[i], " (expected for some domains)")));
            }
        }
    }
    ...
    function testBatchGatewayConnectivity() public {
        ...
        if (result.success) {
            assertTrue(result.data.length > 0, "Batch gateway should return data");
            console.log("Batch gateway connectivity successful");
        } else {
            console.log(string(abi.encodePacked("Batch gateway not available: ", result.error)));
    ...
    function testExactBatchGatewayDomains() public {
        ...
        console.log("Complete Batch Gateway Test: testExactBatchGatewayDomains");
        
        // Test DNS encoding for both domains
        for (uint i = 0; i < domains.length; i++) {
            string memory encoded = _dnsEncodeName(domains[i]);
            console.log(string(abi.encodePacked("Domain ", vm.toString(i), ": ", domains[i], " -> ", encoded)));
            
            // Validate the encoding is correct
            assertTrue(bytes(encoded).length > 0, "DNS encoding must not be empty");
            assertTrue(_isValidHex(encoded), "DNS encoding must be valid hex");
        }
        
        console.log("SUCCESS: Domain encoding validated");
        console.log("NOTE: Full batch gateway test requires live server - structure validated");
    }
    ...
    function testOffchainDNSOracle() public {
        ...
        for (uint i = 0; i < domains.length; i++) {
            ...
            console.log(string(abi.encodePacked("Encoded ", domains[i], " as ", encoded)));
        }
        
        // Create the batch structure
        // const [failures, responses] = await fetchBatchGateway(localBatchGatewayUrl, requests)
        CCIPBatcher.Lookup[] memory lookups = new CCIPBatcher.Lookup[](domains.length);
        ...
        for (uint i = 0; i < domains.length; i++) {
            ...
            lookups[i] = CCIPBatcher.Lookup({
                target: address(0),
                call: abi.encodeWithSignature("resolve(bytes,uint16)", _hexToBytes(encoded), uint16(16)), // TXT record (16)
                data: "",
                flags: 0
            });
        }
        ...
        assertEq(batch.gateways[0], DNS_ORACLE_URL, "Should use correct DNS oracle URL");
        
        // Test complete batch gateway validation
        
        BatchGatewayResult memory batchResult = _testCompleteBatchFlow(domains);
        if (batchResult.success) {
            console.log("SUCCESS: Complete reference flow validated - failures and responses check passed");
            console.log("CRITICAL: Full CCIP-Read batch gateway functionality verified");
        } else {
            console.log("WARNING: Cannot validate complete reference flow (external dependency required)");
            console.log("NOTE: Structure validation passed, but response validation requires live DNS oracle");
        }
        
        console.log("OffchainDNSOracle: Complete domain, ABI, and structure validation completed");
    }
    ...
    function testCCIPBatcherWithRealData() public {
        ...
        lookups[0] = CCIPBatcher.Lookup({
            ...
            call: abi.encodeWithSignature("resolve(bytes,uint16)", _hexToBytes(domain1), uint16(1)),
            data: "",
            flags: 0
        });
        ...
        lookups[1] = CCIPBatcher.Lookup({
            ...
            call: abi.encodeWithSignature("resolve(bytes,uint16)", _hexToBytes(domain2), uint16(1)),
            data: "",
            flags: 0
        });
        ...
        console.log("CCIP Batcher correctly triggered OffchainLookup for batch operations");
    }
    ...
    function testEndToEndDNSResolution() public {
        ...
        if (!dnsResult.success) {
            console.log("Skipping end-to-end test: DNS resolution not available");
            return;
        }
        ...
        assertTrue(bytes(encoded).length > 0, "Step 3: DNS encoding should work");
     ...
     */
    function _callDNSResolverFFI(string memory command, string[] memory args) internal returns (bytes memory) {
    ...
    function _resolveDNS(string memory domain, uint16 qtype) internal returns (DNSResult memory) {
    ...
    function _dnsEncodeName(string memory name) internal returns (string memory) {
        ...
        if ((_contains(jsonStr, '"success": true') || _contains(jsonStr, '"success":true')) && _contains(jsonStr, '"encoded"')) {
    ...
    function _getDNSSECRecords(string memory domain, uint16 qtype) internal returns (DNSSECResult memory) {
        string[] memory args = new string[](2);
        args[0] = domain;
        args[1] = vm.toString(qtype);
    ...
    function _startBatchGateway(uint16 port) internal returns (BatchGatewayResult memory) {
        string[] memory args = new string[](1);
    ...
    function _fetchBatchGateway(string memory url, string memory requestsJson) internal returns (BatchGatewayResult memory) {
        string[] memory args = new string[](2);
        args[0] = url;
        args[1] = requestsJson;
     ...
     */
    function _testCompleteBatchFlow(string[] memory domains) internal returns (BatchGatewayResult memory) {
     ...
     */
    function _mockEncodeFunctionData(string memory encodedDomain) internal pure returns (string memory) {
    ...
    function _parseJSONResult(bytes memory data) internal pure returns (DNSResult memory) {
        // Simplified JSON parsing - in production would use a proper JSON parser
        string memory jsonStr = string(data);
        
        if (_contains(jsonStr, '"success": true') || _contains(jsonStr, '"success":true')) {
    ...
    function _parseEncodedResult(bytes memory data) internal pure returns (string memory) {
        string memory jsonStr = string(data);
    ...
    function _parseDNSSECResult(bytes memory data) internal pure returns (DNSSECResult memory) {
        string memory jsonStr = string(data);
    ...
    function _parseBatchResult(bytes memory data) internal pure returns (BatchGatewayResult memory) {
        string memory jsonStr = string(data);
        
        if (_contains(jsonStr, '"success": true') || _contains(jsonStr, '"success":true')) {
            return BatchGatewayResult({
                success: true,
                data: data,
                error: ""
            });
    ...
    function _extractHex(string memory json) internal pure returns (string memory) {
    ...
    function _extractActualHex(string memory json) internal pure returns (string memory) {
    ...
    function _isValidHex(string memory str) internal pure returns (bool) {
        ...
        // Handle 0x prefix
        uint startIndex = 0;
        if (strBytes.length >= 2 && strBytes[0] == 0x30 && strBytes[1] == 0x78) { // "0x"
            startIndex = 2;
        }
    ...
    function _mockDnsEncodeName(string memory name) internal pure returns (string memory) {
...
contract MockOffchainContract {
    ...
    function resolve(bytes memory name, uint16 qtype) external view returns (bytes memory) {
        string[] memory urls = new string[](1);
        urls[0] = gatewayURL;
        
        bytes memory callData = abi.encodeWithSignature("resolve(bytes,uint16)", name, qtype);
    ...
    function resolveCallback(bytes memory response, bytes memory extraData) external pure returns (bytes memory) {
