...
import { concat, zeroHash, keccak256, pad } from 'viem'
...
export async function getUniversalSigValidatorAddress() {
  const usvArtifact = await hre.artifacts.readArtifact('UniversalSigValidator')
  const usvBytecode = usvArtifact.bytecode
  const deterministicAddress = `0x${keccak256(
    concat([
      '0xff',
      pad(ddpAddress, { size: 20 }),
      zeroHash,
