...
contract TestSolidityTests is Test {
    ...
    function testSolidityTestContractsExist() public {
        ...
        assertTrue(address(testBytesUtils) != address(0), "TestBytesUtils should be deployed");
        assertTrue(address(testRRUtils) != address(0), "TestRRUtils should be deployed");
    }
    ...
    function testBytesUtilsManual() public {
        ...
        assertTrue(bytes("hello").equals("hello"), "String equality should work");
        assertFalse(bytes("hello").equals("world"), "String inequality should work");
    ...
    function testBytesUtilsEdgeCases() public {
        ...
        assertEq(empty.substring(0, 0).length, 0, "Empty substring should have length 0");
        
        // Test single character operations
        bytes memory singleChar = "a";
        assertTrue(singleChar.equals("a"), "Single char equality");
        assertEq(singleChar.readUint8(0), 0x61, "Single char should read correctly");
        
        // Test long string operations
        bytes memory longString = "abcdefghijklmnopqrstuvwxyz";
        assertTrue(longString.equals("abcdefghijklmnopqrstuvwxyz"), "Long string equality");
        assertEq(longString.substring(0, 3).length, 3, "Long string substring length");
    }
    ...
    function testRRUtilsEdgeCases() public {
        ...
        assertEq(multiLabel.labelCount(0), 2, "Multi-label should have correct count");
        
        // Test serial number edge cases
        assertTrue(RRUtils.serialNumberGte(0, 0xFFFFFFFF), "0 >= 0xFFFFFFFF (wraparound)");
        assertFalse(RRUtils.serialNumberGte(0xFFFFFFFF, 0), "0xFFFFFFFF should not be >= 0");
    }
}