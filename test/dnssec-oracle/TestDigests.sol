...
contract TestDigests is Test {
    ...
    function testSHA1DigestValidHash() public view {
        ...
        bytes
            memory expectedHash = hex"2aae6c35c94fcfb415dbe95f408b9ce91ee846ed";

        assertTrue(
            sha1Digest.verify(data, expectedHash),
            "SHA1 digest should verify correctly"
        );
    }
    ...
    function testSHA1DigestInvalidHash() public view {
        ...
        assertFalse(
            sha1Digest.verify(data, wrongHash),
            "SHA1 digest should reject incorrect hash"
        );
    }
    ...
    function testSHA256DigestValidHash() public view {
        ...
        bytes
            memory expectedHash = hex"b94d27b9934d3e08a52e52d7da7dabfac484efe37a5380ee9088f7ace2efcde9";

        assertTrue(
            sha256Digest.verify(data, expectedHash),
            "SHA256 digest should verify correctly"
        );
    }
    ...
    function testSHA256DigestInvalidHash() public view {
        ...
        bytes memory data = "hello world";
        bytes
            memory wrongHash = hex"deadbeefcafebabe123456789abcdef0123456789abcdef0deadbeefcafebabe";

        assertFalse(
            sha256Digest.verify(data, wrongHash),
            "SHA256 digest should reject incorrect hash"
        );
    }
    ...
    function testEmptyDataSHA1() public view {
        ...
        bytes
            memory expectedHash = hex"da39a3ee5e6b4b0d3255bfef95601890afd80709";

        assertTrue(
            sha1Digest.verify(emptyData, expectedHash),
            "SHA1 of empty data should verify"
        );
    }
    ...
    function testEmptyDataSHA256() public view {
        ...
        bytes
            memory expectedHash = hex"e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855";

        assertTrue(
            sha256Digest.verify(emptyData, expectedHash),
            "SHA256 of empty data should verify"
        );
    }
    ...
    function testLargeDataSHA256() public view {
        ...
        bytes
            memory expectedHash = hex"d7a8fbb307d7809469ca9abcb0082e4f8d5651e46d3cdb762d02d0bf37c9e592";

        assertTrue(
            sha256Digest.verify(largeData, expectedHash),
            "SHA256 of large data should verify"
        );
    }
    ...
    function testDNSKeyDataSHA256() public view {
        ...
        bytes
            memory dnskeyData = hex"03666f6f03636f6d000100010803010001a8b5a4c8b2e75c8e5f1234567890abcdef";
        ...
        assertTrue(
            sha256Digest.verify(dnskeyData, expectedHash),
            "DNS key data SHA256 should verify"
        );
    }
}
