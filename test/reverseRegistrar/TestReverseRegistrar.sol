...
contract TestReverseRegistrar is BaseTest {
    ...
    function testShouldCalculateNodeHashCorrectly() public view {
        ...
        assertEq(actualHash, expectedHash, "Node hash calculation should be correct");
    }
    ...
    function testAllowsAuthorisedAccountToClaimDifferentAddress() public {
        ...
        assertEq(owner, USER2, "Authorised account should be able to claim for USER1");
    }
    ...
    function testAllowsControllerToClaimDifferentAddress() public {
        ...
        assertEq(owner, USER2, "Controller should be able to claim for any address");
    }
    ...
    function testAllowsOwnerOfContractToClaimReverseNode() public {
        ...
        reverseRegistrar.claimForAddr(address(dummyOwnable), USER1, address(mockResolver));
        
        bytes32 reverseNode = _getReverseNodeHash(address(dummyOwnable));
        address owner = ens.owner(reverseNode);
        
        assertEq(owner, USER1, "Contract owner should be able to claim reverse record");
    }
    ...
    function testAllowsControllerToSetNameRecordsForOtherAccounts() public {
        ...
        reverseRegistrar.setNameForAddr(USER1, USER2, address(mockResolver), name);
    ...
    function testSetNameForAddrEmitsReverseClaimedEvent() public {
        ...
        reverseRegistrar.setNameForAddr(USER1, USER2, address(mockResolver), "test.eth");
    }
    
    // Test 14: forbids non-controller if address is different from sender and not authorised
    function testForbidsNonControllerIfAddressIsDifferentFromSenderAndNotAuthorised() public {
        vm.prank(USER1);
        vm.expectRevert(bytes(""));
        reverseRegistrar.setNameForAddr(USER2, USER3, address(mockResolver), "test.eth");
    }
    ...
    function testAllowsNameToBeSetForAddressIfSenderIsTheAddress() public {
        ...
        assertEq(owner, USER2, "USER1 should be able to set their own reverse record");
    }
    ...
    function testAllowsNameToBeSetForAddressIfSenderIsAuthorised() public {
        ...
        vm.prank(USER2);
        reverseRegistrar.setNameForAddr(USER1, USER3, address(mockResolver), "test.eth");
        
        bytes32 reverseNode = _getReverseNodeHash(USER1);
        address owner = ens.owner(reverseNode);
        
        assertEq(owner, USER3, "Authorised user should be able to set reverse record");
    }
    ...
    function testAllowsOwnerOfContractToClaimWithResolverForAddr() public {
        ...
        assertEq(owner, USER1, "Contract reverse record should be owned by USER1");
    ...
    function testAllowsOwnerToSetController() public {
        vm.prank(TestAccounts.owner());
        reverseRegistrar.setController(USER1, true);
        assertTrue(reverseRegistrar.controllers(USER1), "USER1 should be a controller");
        
        vm.prank(TestAccounts.owner());
        reverseRegistrar.setController(USER1, false);
        assertFalse(reverseRegistrar.controllers(USER1), "USER1 should no longer be a controller");
    }
