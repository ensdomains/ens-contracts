...
contract UnwrapETH2LD is BaseWrapperTest {
    ...
    function testUnwrapETH2LDByOwner() public {
        ...
        assertFalse(
            nameWrapper.isWrapped(TEST_NODE),
            "Domain should be unwrapped"
        );
        assertEq(
            nameWrapper.ownerOf(TEST_NODE_ID),
            address(0),
            "Wrapper should not own domain"
        );
        assertEq(
            ens.owner(TEST_NODE),
            CONTROLLER,
            "ENS should show CONTROLLER as owner"
        );
        assertEq(
            baseRegistrar.ownerOf(TEST_LABEL_ID),
            REGISTRANT,
            "Registrar should show REGISTRANT as owner"
        );
    ...
    function testUnwrapETH2LDByOperator() public {
        ...
        assertFalse(
            nameWrapper.isWrapped(TEST_NODE),
            "Domain should be unwrapped by operator"
        );
        assertEq(
            ens.owner(TEST_NODE),
            CONTROLLER,
    ...
    function testUnwrapETH2LDSameAddresses() public {
        ...
        assertEq(
            ens.owner(TEST_NODE),
            OWNER,
            "ENS should show OWNER as controller"
        );
        assertEq(
            baseRegistrar.ownerOf(TEST_LABEL_ID),
            OWNER,
            "Registrar should show OWNER as registrant"
        );
    ...
    function testUnwrapETH2LDReclaim() public {
        ...
        assertEq(
            ens.owner(TEST_NODE),
            CONTROLLER,
            "ENS should show CONTROLLER after reclaim"
        );
        assertEq(
            baseRegistrar.ownerOf(TEST_LABEL_ID),
            REGISTRANT,
            "Registrar should show REGISTRANT"
        );
    ...
    function testUnwrapETH2LDDifferentAddresses() public {
        ...
        assertEq(
            ens.owner(TEST_NODE),
            controller,
            "ENS should show different controller"
        );
        assertEq(
            baseRegistrar.ownerOf(TEST_LABEL_ID),
            registrant,
            "Registrar should show different registrant"
        );
        assertNotEq(
            controller,
            registrant,
            "Controller and registrant should be different"
        );
    ...
    function testUnwrapETH2LDMaintainsRegistrarState() public {
        ...
        assertEq(
            finalExpiry,
            initialExpiry,
            "Registrar expiry should be maintained"
        );
        assertTrue(
            baseRegistrar.available(TEST_LABEL_ID) == false,
    ...
    function testUnwrapETH2LDResetsWrapperState() public {
        ...
        assertFalse(
            nameWrapper.isWrapped(TEST_NODE),
            "Should not be wrapped after unwrap"
        );

        // Check with both isWrapped overloads
        assertFalse(
            nameWrapper.isWrapped(TEST_NODE),
            "Node-based isWrapped should return false"
        );
        assertFalse(
            nameWrapper.isWrapped(ETH_NODE, TEST_LABEL_HASH),
            "Parent/label-based isWrapped should return false"
        );
    ...
    function testUnwrapETH2LDClearsIndividualApprovals() public {
        ...
        assertEq(
            nameWrapper.getApproved(TEST_NODE_ID),
            address(0),
            "Individual approval should be cleared"
        );
    ...
    function testUnwrapETH2LDFailsAfterOperatorApprovalRevoked() public {
        ...
        bytes32 testNode2 = keccak256(
            abi.encodePacked(ETH_NODE, testLabelHash2)
        );
