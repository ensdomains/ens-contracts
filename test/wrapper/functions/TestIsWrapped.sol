...
contract IsWrapped is BaseWrapperTest {
    ...
    function testIsWrappedForUnregisteredDomain() public view {
        ...
        bytes32 unregisteredNode = keccak256(
            abi.encodePacked(ETH_NODE, unregisteredLabel)
        );

        // Unregistered domain should not be wrapped
        assertFalse(
            nameWrapper.isWrapped(unregisteredNode),
            "Unregistered domain should not be wrapped"
        );
        assertFalse(
            nameWrapper.isWrapped(ETH_NODE, unregisteredLabel),
            "Unregistered domain should not be wrapped"
        );
    }
    ...
    function testIsWrappedForRegisteredButNotWrappedDomain() public {
        ...
        assertFalse(
            nameWrapper.isWrapped(TEST_NODE),
            "Registered but not wrapped domain should not be wrapped"
        );
        assertFalse(
            nameWrapper.isWrapped(ETH_NODE, TEST_LABEL_HASH),
            "Registered but not wrapped domain should not be wrapped"
    ...
    function testIsWrappedForSubdomain() public {
        ...
        assertFalse(
            ...
            "Subdomain should not be wrapped initially"
        );
        assertFalse(
            nameWrapper.isWrapped(TEST_NODE, SUB_LABEL_HASH),
            "Subdomain should not be wrapped initially"
        );
        ...
        assertTrue(
            nameWrapper.isWrapped(SUB_NODE),
            "Subdomain should be wrapped after creation"
        );
        assertTrue(
            nameWrapper.isWrapped(TEST_NODE, SUB_LABEL_HASH),
            "Subdomain should be wrapped after creation"
    ...
    function testIsWrappedForSubdomainWithExpiredParent() public {
        ...
        assertTrue(
            nameWrapper.isWrapped(SUB_NODE),
            "Subdomain should be wrapped initially"
        );
        assertTrue(
            nameWrapper.isWrapped(TEST_NODE, SUB_LABEL_HASH),
            "Subdomain should be wrapped initially"
        ...
        assertFalse(
            nameWrapper.isWrapped(SUB_NODE),
            "Subdomain should not be wrapped when parent expired"
        );
        assertFalse(
            nameWrapper.isWrapped(TEST_NODE, SUB_LABEL_HASH),
            "Subdomain should not be wrapped when parent expired"
    ...
    function testIsWrappedForUnknownTLD() public view {
        ...
        bytes32 unknownNode = keccak256(
            abi.encodePacked(ROOT_NODE, unknownTLD)
        );

        // Unknown TLD should not be wrapped
        assertFalse(
            nameWrapper.isWrapped(unknownNode),
            "Unknown TLD should not be wrapped"
        );
        assertFalse(
            nameWrapper.isWrapped(ROOT_NODE, unknownTLD),
            "Unknown TLD should not be wrapped"
        );
    }
    ...
    function testIsWrappedConsistencyBetweenOverloads() public {
        ...
        assertEq(
            result1a,
            result1b,
            "Results should be consistent before wrapping"
        );
        ...
        assertEq(
            result2a,
            result2b,
            "Results should be consistent after wrapping"
        );
        ...
        assertEq(
            result3a,
            result3b,
            "Results should be consistent after unwrapping"
        );
