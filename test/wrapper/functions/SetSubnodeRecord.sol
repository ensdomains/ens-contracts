...
contract SetSubnodeRecord is BaseWrapperTest {
    ...
    function testSetSubnodeRecordByOwner() public {
        ...
        assertEq(nameWrapper.ownerOf(CHILD_NODE_ID), address(0), "Child should not exist initially");
        assertEq(ens.owner(CHILD_NODE), address(0), "Child should not exist in ENS initially");
        ...
        assertTrue(nameWrapper.isWrapped(CHILD_NODE), "Child should be wrapped");
        assertEq(nameWrapper.ownerOf(CHILD_NODE_ID), NEW_OWNER, "Child should be owned by NEW_OWNER");
        assertEq(ens.owner(CHILD_NODE), address(nameWrapper), "ENS should show wrapper as owner");
    ...
    function testCannotSetSubnodeRecordByUnauthorized() public {
        ...
        bytes32 childNode = keccak256(abi.encodePacked(TEST_NODE, keccak256(bytes(CHILD_LABEL))));
    ...
    function testSetSubnodeRecordChangesOwner() public {
        ...
        assertEq(nameWrapper.ownerOf(CHILD_NODE_ID), OWNER, "Should be owned by OWNER initially");
        ...
        assertEq(nameWrapper.ownerOf(CHILD_NODE_ID), NEW_OWNER, "Should be owned by NEW_OWNER after change");
    ...
    function testSetSubnodeRecordBurnAndUnwrap() public {
        ...
        assertTrue(nameWrapper.isWrapped(CHILD_NODE), "Should be wrapped initially");
        assertEq(nameWrapper.ownerOf(CHILD_NODE_ID), NEW_OWNER, "Should be owned by NEW_OWNER");
        ...
        assertEq(nameWrapper.ownerOf(CHILD_NODE_ID), address(0), "Should not be owned in wrapper");
    ...
    function testSetSubnodeRecordSetsENSValues() public {
        ...
        assertEq(ens.owner(CHILD_NODE), address(nameWrapper), "ENS owner should be wrapper");
        assertEq(ens.resolver(CHILD_NODE), RESOLVER, "ENS resolver should be set");
    ...
    function testSetSubnodeRecordWithZeroResolver() public {
        ...
        assertEq(ens.resolver(CHILD_NODE), address(0), "Resolver should be zero");
    ...
    function testSetSubnodeRecordChangesBalances() public {
        ...
        assertEq(nameWrapper.balanceOf(NEW_OWNER, CHILD_NODE_ID), 0, "NEW_OWNER should not have token initially");
        ...
        assertEq(nameWrapper.balanceOf(NEW_OWNER, CHILD_NODE_ID), 1, "NEW_OWNER should have token");
    ...
    function testRewrappingNameWithPCCBurnedButExpiredIsPossible() public {
        ...
        (, uint32 fusesAfterExpiry, uint64 expiryAfterExpiry) = nameWrapper.getData(CHILD_NODE_ID);
        assertEq(expiryAfterExpiry, childExpiry, "Expiry should remain the same");
        assertEq(fusesAfterExpiry, 0, "Fuses should be reset to 0 after expiry");
        ...
        assertEq(nameWrapper.ownerOf(CHILD_NODE_ID), NEW_OWNER, "Subdomain should be rewrapped");
        
        vm.stopPrank();
    }
    
    function testDoesNotAllowUnauthorizedUserEvenWithENSRegistryApproval() public {
        _wrapTestDomain();
    ...
    function testUnwrappingPreviouslyWrappedUnexpiredNameRetainsPCCAndRevertsSetSubnodeRecord() public {
        _wrapTestDomain();
        
        vm.startPrank(OWNER);
        
        uint256 parentExpiry = baseRegistrar.nameExpires(TEST_LABEL_ID) + baseRegistrar.GRACE_PERIOD();
        
        // Create subdomain with PCC
        nameWrapper.setSubnodeOwner(TEST_NODE, CHILD_LABEL, NEW_OWNER, PARENT_CANNOT_CONTROL, MAX_EXPIRY);
        
        // Verify fuses are set
        (, uint32 fusesBefore, uint64 expiryBefore) = nameWrapper.getData(CHILD_NODE_ID);
        ...
        (address owner, uint32 fusesAfter, uint64 expiryAfter) = nameWrapper.getData(CHILD_NODE_ID);
    ...
    function testExpiredSubnamesStillProtectedByCannotCreateSubdomainOnParent() public {
        _wrapTestDomain();
        ...
        bytes32 subnode2 = keccak256(abi.encodePacked(TEST_NODE, sublabel2Hash));
        ...
        vm.expectRevert(abi.encodeWithSignature("OperationProhibited(bytes32)", subnode2));
        nameWrapper.setSubnodeRecord(
            TEST_NODE,
            sublabel2,
            NEW_OWNER,
