...
contract Wrap is Test {
    ...
    function testWrapsNameIfYouAreTheOwner() public {
        ...
        assertEq(
            nameWrapper.ownerOf(toNameId(label)),
            account0,
            "Should be owned by account0 after wrap"
        );
    ...
    function testAllowsSpecifyingResolver() public {
        ...
        assertEq(
            ensRegistry.resolver(namehash(label)),
            account1,
    ...
    function testAllowsApprovedAccountOnENSRegistryToWrapName() public {
        ...
        assertEq(
            ...
            account1,
            "Should be owned by account1 in ENS"
        );
        ...
        assertEq(
            nameWrapper.ownerOf(toNameId(label)),
            account1,
            "Should be owned by account1 in NameWrapper"
        );
    ...
    function testCanReWrapNameThatWasReassignedByUnwrappedParent() public {
        ...
        assertEq(
            nameWrapper.ownerOf(toNameId(parentLabel)),
            ZERO_ACCOUNT,
            "Parent should start with zero owner"
        );

        _setRegistryApprovalForWrapper();
        ensRegistry.setSubnodeOwner(
            namehash(parentLabel),
        ...
        assertEq(
            ensRegistry.owner(namehash(childName)),
            address(nameWrapper),
            "Should be owned by NameWrapper in ENS"
        );
    }
    ...
    function testWillNotWrapNameWithJunkAtEnd() public {
        ...
        bytes memory invalidName = abi.encodePacked(
            dnsEncodeName("xyz"),
            hex"123456"
        );
    ...
    function testDoesNotAllowWrappingNameYouDoNotOwn() public {
        ...
        assertEq(
            nameWrapper.ownerOf(toNameId(label)),
            account0,
            "Should still be owned by account0"
        );
    ...
    function testRewrappingPreviouslyWrappedUnexpiredNameRetainsPCC() public {
        ...
        assertEq(
            fusesBefore,
            PARENT_CANNOT_CONTROL,
            "PCC should be set before unwrap"
        );
        ...
        assertEq(
            fusesAfter,
            PARENT_CANNOT_CONTROL,
            "PCC should be retained after re-wrap"
        );
        assertEq(
            expiryAfter,
    ...
    function testCompleteFixtureSetup() public view {
        ...
        assertTrue(
            address(nameGriefer) != address(0),
            "Name Griefer should be deployed"
        );
    ...
    function testDNSEncoding() public pure {
        ...
        assertEq(
            encoded,
            expected,
            "DNS encoding should match expected format"
        );
        ...
        assertEq(
            singleLabel,
            expectedSingle,
            "Single label DNS encoding should work"
        );
