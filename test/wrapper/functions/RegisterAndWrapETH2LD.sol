...
contract RegisterAndWrapETH2LD is Test {
    ...
    function testRegisterAndWrapETH2LD() public {
        ...
        assertFalse(
            nameWrapper.isWrapped(TEST_NODE),
            "Should not be wrapped initially"
        );
        assertEq(
            nameWrapper.ownerOf(TEST_NODE_ID),
            address(0),
            "Should not own token initially"
        );
        assertTrue(
            baseRegistrar.available(TEST_LABEL_ID),
    ...
    function testRegisterAndWrapETH2LDToNewOwner() public {
        ...
        assertEq(
            nameWrapper.ownerOf(TEST_NODE_ID),
            NEW_OWNER,
            "NEW_OWNER should own wrapped token"
        );
        assertTrue(nameWrapper.isWrapped(TEST_NODE), "Should be wrapped");
    ...
    function testRegisterAndWrapETH2LDWithFuses() public {
        ...
        assertTrue(
            actualFuses & CANNOT_UNWRAP != 0,
            "Should have CANNOT_UNWRAP fuse"
        );
        assertTrue(
            actualFuses & CANNOT_SET_RESOLVER != 0,
            "Should have CANNOT_SET_RESOLVER fuse"
        );
        assertTrue(
            actualFuses & PARENT_CANNOT_CONTROL != 0,
            "Should have PARENT_CANNOT_CONTROL fuse"
        );
        assertTrue(
            actualFuses & IS_DOT_ETH != 0,
            "Should have IS_DOT_ETH fuse"
        );

        vm.stopPrank();
    }

    function testCannotRegisterAndWrapETH2LDWithFusesWithoutCannotUnwrap()
        public
    {
        vm.startPrank(OWNER);
    ...
    function testRegisterAndWrapETH2LDAutomaticFuses() public {
        ...
        assertTrue(
            ...
            "Should automatically have PARENT_CANNOT_CONTROL"
        );
        assertTrue(
            fuses & IS_DOT_ETH != 0,
            "Should automatically have IS_DOT_ETH"
        );
    ...
    function testCannotRegisterAndWrapETH2LDLongLabel() public {
        ...
        string
            memory longLabel = "";

        // Try to register long label - should fail
        vm.expectRevert(
            abi.encodeWithSignature("LabelTooLong(string)", longLabel)
        ); // LabelTooLong
        nameWrapper.registerAndWrapETH2LD(
            longLabel,
            OWNER,
    ...
    function testRegisterAndWrapETH2LDEmitsEvents() public {
        ...
        emit NameWrapped(
            ...
            uint64(block.timestamp + 86400 + baseRegistrar.GRACE_PERIOD())
        );
    ...
    function testRegisterAndWrapETH2LDChangesBalances() public {
        ...
        assertEq(
            ...
            0,
            "OWNER should not have token initially"
        );
    ...
    function testRegisterAndWrapETH2LDSetsCorrectExpiry() public {
        ...
        uint256 expectedWrapperExpiry = registrarExpiry +
            baseRegistrar.GRACE_PERIOD();

        assertEq(
            registrarExpiry,
            expectedRegistrarExpiry,
            "Registrar expiry should match expected"
        );
        assertEq(
            wrapExpiry,
            registrarExpiry,
            "Function should return registrar expiry"
        );
        // The actual wrapper expiry is registrar expiry + grace period
        assertEq(
            wrapExpiry + baseRegistrar.GRACE_PERIOD(),
        ...
        assertEq(
            storedExpiry,
            expectedWrapperExpiry,
            "Stored expiry should be registrar + grace period"
        );
    ...
    function testRegisterAndWrapETH2LDMultipleDomains() public {
        ...
        assertEq(
            nameWrapper.ownerOf(TEST_NODE_ID),
            OWNER,
            "First domain should be owned by OWNER"
        );
        assertEq(
            nameWrapper.ownerOf(node2Id),
            NEW_OWNER,
            "Second domain should be owned by NEW_OWNER"
        );
        assertEq(
            ens.resolver(node2),
            RESOLVER,
            "Second domain should have resolver"
        );
        ...
        assertEq(
            fuses1 & CANNOT_UNWRAP,
            0,
            "First domain should not have CANNOT_UNWRAP"
        );
        assertTrue(
            fuses2 & CANNOT_UNWRAP != 0,
            "Second domain should have CANNOT_UNWRAP"
        );
    ...
    function testRegisterAndWrapETH2LDWithDifferentDurations() public {
        ...
        assertTrue(
            expiry2 > expiry1,
            "Longer duration should have later expiry"
        );
        assertEq(
            expiry1 - block.timestamp,
            duration1,
            "First domain should have correct duration"
        );
        assertEq(
            expiry2 - block.timestamp,
            duration2,
            "Second domain should have correct duration"
        );
    ...
    function testRegistrationThroughControllerAndWrapperComparison() public {
        ...
        controller.register{value: totalPrice}(registration);
        ...
        // Compare results
        bytes32 controllerNode = keccak256(
            abi.encodePacked(ETH_NODE, keccak256(bytes(controllerLabel)))
        );
        bytes32 wrapperNode = keccak256(
            abi.encodePacked(ETH_NODE, keccak256(bytes(wrapperLabel)))
        );

        // The new controller no longer auto-wraps, so controller and wrapper registrations behave differently
        // Controller registration: owned by user, not wrapped
        assertEq(
            ens.owner(controllerNode),
            OWNER,
            "Controller registration should be owned by user"
        );
        assertFalse(
            nameWrapper.isWrapped(controllerNode),
            "Controller registration should not be wrapped"
        );

        // Direct wrapper registration: owned by wrapper, wrapped
        assertEq(
            ens.owner(wrapperNode),
            address(nameWrapper),
            "Wrapper registration should be owned by wrapper"
        );
        assertTrue(
            nameWrapper.isWrapped(wrapperNode),
            "Wrapper registration should be wrapped"
        );
        assertEq(
            nameWrapper.ownerOf(uint256(wrapperNode)),
            OWNER,
            "User should own wrapper-registered wrapped token"
        );

        // Base registrar NFT ownership differs between the two approaches
        // Controller registration: user owns the BaseRegistrar NFT directly
        assertEq(
            baseRegistrar.ownerOf(uint256(keccak256(bytes(controllerLabel)))),
            OWNER,
            "User should own controller NFT"
        );
        // Wrapper registration: wrapper owns the BaseRegistrar NFT
        assertEq(
            baseRegistrar.ownerOf(uint256(keccak256(bytes(wrapperLabel)))),
            address(nameWrapper),
            "Wrapper should own wrapper NFT"
        );
    ...
    function testControllerCannotRegisterToWrapper() public {
        ...
        // However, the new controller behavior allows this since it no longer auto-wraps
        string memory testLabel = "conflict";
        uint256 duration = 365 days;

        bytes32 secret = keccak256("secret");
        IETHRegistrarController.Registration
            memory wrapperRegistration = IETHRegistrarController.Registration({
                label: testLabel,
                owner: address(nameWrapper), // Try to register to wrapper address
                duration: duration,
                secret: secret,
                resolver: address(0),
                data: new bytes[](0),
        ...
        bytes32 commitment = controller.makeCommitment(wrapperRegistration);
        ...
        // The new controller no longer auto-wraps, so registration to wrapper address now succeeds
        // The BaseRegistrar will directly assign the NFT to the wrapper address
        controller.register{value: price}(wrapperRegistration);

        // Verify that the registration succeeded and is owned by the wrapper
        bytes32 nodeHash = keccak256(
            abi.encodePacked(ETH_NODE, keccak256(bytes(testLabel)))
        );
        assertEq(
            ens.owner(nodeHash),
            address(nameWrapper),
            "Registration should be owned by wrapper"
        );
    ...
    function testPriceConsistencyBetweenControllerAndWrapper() public {
        ...
        assertTrue(
            controllerPrice > 0,
            "Controller should return non-zero price"
        );

        // Both registrations should succeed with same cost expectations
        assertTrue(
            controllerPrice > 0,
            "Should have non-zero price from oracle"
        );
    }
