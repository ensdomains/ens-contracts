...
import {
    ...
    USER_SETTABLE_FUSES
} from "../../../contracts/wrapper/INameWrapper.sol";
...
contract WrapETH2LD is Test {
    ...
    function _register(string memory label, address owner, uint256 duration) internal {
        vm.startPrank(account0);
    ...
    function _registerNoPrank(string memory label, address owner, uint256 duration) internal {
        vm.warp(block.timestamp + baseRegistrar.GRACE_PERIOD() + DAY + 1); // Past grace period
        baseRegistrar.register(toLabelId(label), owner, duration);
    }
    
    function _register(string memory label, address owner, uint256 duration, uint256 accountIndex) internal {
    ...
    function _setBaseRegistrarApprovalForWrapper(uint256 accountIndex) internal {
        vm.startPrank(accounts[accountIndex]);
        baseRegistrar.setApprovalForAll(address(nameWrapper), true);
        vm.stopPrank();
    }
    
    function _wrapEth2ld(string memory label, address owner, uint32 fuses, address resolver) internal {
        // This function assumes the caller is already in the correct prank context
        nameWrapper.wrapETH2LD(label, owner, uint16(fuses), resolver);
    }
    
    function _wrapEth2ld(string memory label, address owner, uint32 fuses, address resolver, uint256 accountIndex) internal {
        vm.startPrank(accounts[accountIndex]);
        nameWrapper.wrapETH2LD(label, owner, uint16(fuses), resolver);
        vm.stopPrank();
    }
    ...
    function _registerSetupAndWrapName(string memory label, uint256 duration, uint256 accountIndex, uint32 fuses) internal {
        // Add the account as a controller if not already added
        vm.startPrank(account0);
        baseRegistrar.addController(accounts[accountIndex]);
        vm.stopPrank();
        ...
        // Move past grace period and register domain
        vm.warp(block.timestamp + baseRegistrar.GRACE_PERIOD() + DAY + 1); // Past grace period
        baseRegistrar.register(toLabelId(label), accounts[accountIndex], duration);
        baseRegistrar.setApprovalForAll(address(nameWrapper), true);
        
        // Wrap domain with specified fuses
        nameWrapper.wrapETH2LD(label, accounts[accountIndex], uint16(fuses), address(0));
        
        vm.stopPrank();
    }
    
    function _registerSetupAndWrapNameNoWarp(string memory label, uint256 duration, uint256 accountIndex, uint32 fuses) internal {
        // Add the account as a controller if not already added
        vm.startPrank(account0);
        baseRegistrar.addController(accounts[accountIndex]);
        vm.stopPrank();
    ...
    function _unwrapEth2ld(string memory label, address controller, address registrant) internal {
        // This function assumes the caller is already in the correct prank context
        nameWrapper.unwrapETH2LD(keccak256(bytes(label)), controller, registrant);
    }
    ...
    function testWrapsNameIfSenderIsOwner() public {
        ...
        assertEq(nameWrapper.ownerOf(toNameId(NAME)), ZERO_ACCOUNT, "Should start with zero owner");
        
        _wrapEth2ld(LABEL, account0, CAN_DO_EVERYTHING, address(0));
        
        // make sure reclaim claimed ownership for the wrapper in registry
        assertEq(ensRegistry.owner(namehash(NAME)), address(nameWrapper), "Registry should be owned by NameWrapper");
        
        // make sure owner in the wrapper is the user
        assertEq(nameWrapper.ownerOf(toNameId(NAME)), account0, "NameWrapper should be owned by account0");
        
        // make sure registrar ERC721 is owned by Wrapper
        assertEq(baseRegistrar.ownerOf(toLabelId(LABEL)), address(nameWrapper), "BaseRegistrar should be owned by NameWrapper");
    ...
    function testCannotWrapNameIfOwnerHasNotAuthorisedWrapper() public {
        ...
        nameWrapper.wrapETH2LD(LABEL, account0, uint16(CAN_DO_EVERYTHING), address(0));
    ...
    function testAllowsSpecifyingResolver() public {
        ...
        assertEq(ensRegistry.resolver(namehash(NAME)), account1, "Resolver should be set to account1");
        
        vm.stopPrank();
    }
    
    // TEST 4: "Can re-wrap a name that was wrapped has already expired on the .eth registrar"
    function testCanReWrapNameThatWasWrappedHasAlreadyExpiredOnEthRegistrar() public {
        vm.startPrank(account0);
        ...
        assertTrue(baseRegistrar.available(toLabelId(LABEL)), "Should be available");
        
        vm.stopPrank();
        
        // Register from another address
        _register(LABEL, account1, 1 * DAY, 1);
        assertEq(baseRegistrar.ownerOf(toLabelId(LABEL)), account1, "Should be owned by account1");
        ...
        vm.expectEmit(true, false, false, true);
        emit NameWrapped(expectedNode, expectedName, account1, PARENT_CANNOT_CONTROL | IS_DOT_ETH, uint64(expectedExpiry + baseRegistrar.GRACE_PERIOD()));
        
        _wrapEth2ld(LABEL, account1, CAN_DO_EVERYTHING, address(0), 1);
        
        assertEq(nameWrapper.ownerOf(toNameId(NAME)), account1, "NameWrapper should be owned by account1");
        assertEq(baseRegistrar.ownerOf(toLabelId(LABEL)), address(nameWrapper), "BaseRegistrar should be owned by NameWrapper");
    }
    
    // TEST 5: "Can re-wrap a name that was wrapped has already expired even if CANNOT_TRANSFER was burned"
    function testCanReWrapNameThatWasWrappedHasAlreadyExpiredEvenIfCannotTransferWasBurned() public {
        vm.startPrank(account0);
        
        _registerNoPrank(LABEL, account0, 1 * DAY);
        _setBaseRegistrarApprovalForWrapper();
        _wrapEth2ld(LABEL, account0, CANNOT_UNWRAP | CANNOT_TRANSFER, address(0));
    ...
    function testCorrectlyReportsFusesForNameThatHasExpiredAndBeenRewrappedMorePermissively() public {
        ...
        assertEq(initialFuses, expectedInitialFuses, "Initial fuses should match");
        
        // Create a subdomain that can't be unwrapped
        bytes32 parentNode = namehash(NAME);
        _setSubnodeOwner(parentNode, "sub", account0, PARENT_CANNOT_CONTROL | CANNOT_UNWRAP, MAX_EXPIRY);
        
        (, uint32 subFuses,) = nameWrapper.getData(toNameId("sub.wrapped2.eth"));
        assertEq(subFuses, PARENT_CANNOT_CONTROL | CANNOT_UNWRAP, "Sub fuses should match");
        ...
        uint256 expectedExpiry = baseRegistrar.nameExpires(toLabelId(LABEL)) + baseRegistrar.GRACE_PERIOD();
        (, uint32 newFuses, uint64 newExpiry) = nameWrapper.getData(toNameId(NAME));
        assertEq(newFuses, PARENT_CANNOT_CONTROL | IS_DOT_ETH, "New fuses should match");
        assertEq(newExpiry, expectedExpiry, "New expiry should match");
        
        // subdomain fuses get reset
        (, uint32 newSubFuses,) = nameWrapper.getData(toNameId("sub.wrapped2.eth"));
    ...
    function testCorrectlyReportsFusesWithRegisterAndWrap() public {
        ...
        nameWrapper.registerAndWrapETH2LD(LABEL, account1, 1 * DAY, address(0), 0);
    ...
    function testEmitsWrapEvent() public {
        ...
        vm.expectEmit(true, false, false, true);
        emit NameWrapped(expectedNode, expectedName, account0, PARENT_CANNOT_CONTROL | IS_DOT_ETH, uint64(expiry + baseRegistrar.GRACE_PERIOD()));
    ...
    function testTransfersWrappedTokenToTargetAddress() public {
        ...
        assertEq(nameWrapper.ownerOf(toNameId(NAME)), account1, "Should be owned by account1");
    ...
    function testDoesNotAllowWrappingWithTargetAddressZero() public {
        ...
        nameWrapper.wrapETH2LD(LABEL, ZERO_ACCOUNT, uint16(CAN_DO_EVERYTHING), address(0));
    ...
    function testDoesNotAllowWrappingWithWrapperContractAddress() public {
        ...
        nameWrapper.wrapETH2LD(LABEL, address(nameWrapper), uint16(CAN_DO_EVERYTHING), address(0));
    ...
    function testDoesNotAllowAnyoneElseToWrapNameEvenIfOwnerAuthorisedWrapper() public {
        ...
        vm.expectRevert(abi.encodeWithSignature("Unauthorised(bytes32,address)", expectedNode, account1));
    ...
    function testCanWrapNameEvenIfControllerAddressDifferentToRegistrantAddress() public {
        vm.startPrank(account0);
        ...
        assertEq(nameWrapper.ownerOf(toNameId(NAME)), account0, "Should be owned by account0");
    ...
    function testDoesNotAllowFuseToBeBurnedIfCannotUnwrapNotBurned() public {
        ...
        nameWrapper.wrapETH2LD(LABEL, account0, uint16(CANNOT_SET_RESOLVER), address(0));
    ...
    function testCannotBurnAnyParentControlledFuse() public {
        ...
        for (uint256 i = 0; i < 7; i++) {
            string memory testLabel = string(abi.encodePacked("test", vm.toString(i)));
            ...
            vm.expectRevert();
            nameWrapper.wrapETH2LD(testLabel, account0, uint16(undefinedFuse), address(0));
        }
    ...
    function testAllowsFuseToBeBurnedIfCannotUnwrapHasBeenBurned() public {
        ...
        uint32 expectedFuses = initialFuses | PARENT_CANNOT_CONTROL | IS_DOT_ETH;
    ...
    function testWillNotWrapEmptyName() public {
        ...
        nameWrapper.wrapETH2LD("", account0, uint16(CAN_DO_EVERYTHING), address(0));
    ...
    function testWillNotWrapLabelGreaterThan255Characters() public {
        ...
        string memory longString = "";
        assertEq(bytes(longString).length, 256, "String should be 256 characters");
        ...
        vm.expectRevert(abi.encodeWithSignature("LabelTooLong(string)", longString));
        nameWrapper.wrapETH2LD(longString, account0, uint16(CAN_DO_EVERYTHING), address(0));
        
        vm.stopPrank();
    }
    
    // TEST 23: "Rewrapping a previously wrapped unexpired name retains PCC and expiry"
    function testRewrappingPreviouslyWrappedUnexpiredNameRetainsPCCAndExpiry() public {
        ...
        assertEq(fuses, PARENT_CANNOT_CONTROL | IS_DOT_ETH, "PCC should be retained");
        assertEq(expiry, parentExpiry + baseRegistrar.GRACE_PERIOD(), "Expiry should be parent expiry + grace period");
