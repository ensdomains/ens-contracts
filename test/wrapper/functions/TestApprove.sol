...
contract Approve is BaseWrapperTest {
    ...
    function testApprovalByOwner() public {
        ...
        assertEq(nameWrapper.getApproved(TEST_NODE_ID), address(0), "Should have no approval initially");
    ...
    function testApprovalByOperator() public {
        ...
        nameWrapper.approve(APPROVED, TEST_NODE_ID);
        assertEq(nameWrapper.getApproved(TEST_NODE_ID), APPROVED, "Should be approved by operator");
        vm.stopPrank();
    }
    ...
    function testApprovedCannotSetSubnodeOwner() public {
        ...
        vm.expectRevert(abi.encodeWithSignature("Unauthorised(bytes32,address)", TEST_NODE, APPROVED));
        nameWrapper.setSubnodeOwner(TEST_NODE, SUB_LABEL, OTHER, CAN_DO_EVERYTHING, MAX_EXPIRY);
        vm.stopPrank();
    }
    ...
    function testApprovedCannotSetSubnodeRecord() public {
        ...
        vm.startPrank(APPROVED);
        vm.expectRevert(abi.encodeWithSignature("Unauthorised(bytes32,address)", TEST_NODE, APPROVED));
        nameWrapper.setSubnodeRecord(TEST_NODE, SUB_LABEL, OTHER, address(0), 0, CAN_DO_EVERYTHING, MAX_EXPIRY);
        vm.stopPrank();
    }
    ...
    function testApprovalReplacement() public {
        ...
        assertEq(nameWrapper.getApproved(TEST_NODE_ID), OTHER, "Approval should be replaced");
    ...
    function testAllowsApprovedAddressToCallExtendExpiry() public {
        ...
        nameWrapper.setSubnodeOwner(TEST_NODE, SUB_LABEL, ACCOUNT, CAN_DO_EVERYTHING, 0);
        nameWrapper.approve(APPROVED, TEST_NODE_ID);
        
        vm.stopPrank();
        
        // Verify subdomain is owned by ACCOUNT
        assertEq(nameWrapper.ownerOf(SUB_NODE_ID), ACCOUNT, "Subdomain should be owned by ACCOUNT");
    ...
    function testApprovedAddressCannotExtendExpiryWhenExpired() public {
        ...
        vm.expectRevert(abi.encodeWithSignature("Unauthorised(bytes32,address)", SUB_NODE, APPROVED));
        nameWrapper.extendExpiry(TEST_NODE, SUB_LABEL_HASH, 1000);
        
        vm.stopPrank();
    }
    
    function testApprovedAddressCanBeReplacedAndPreviousApprovedIsRemoved() public {
        _wrapTestDomain();
        ...
        nameWrapper.extendExpiry(TEST_NODE, SUB_LABEL_HASH, uint64(parentExpiry - 500));
        vm.stopPrank();
        
        // First approved address should no longer work
        vm.startPrank(ACCOUNT);
        vm.expectRevert(abi.encodeWithSignature("Unauthorised(bytes32,address)", SUB_NODE, ACCOUNT));
        nameWrapper.extendExpiry(TEST_NODE, SUB_LABEL_HASH, uint64(parentExpiry));
        vm.stopPrank();
        
        // Verify expiry was set by second approved address
        (, , uint64 expiry) = nameWrapper.getData(SUB_NODE_ID);
        assertEq(expiry, uint64(parentExpiry - 500), "Expiry should be set by second approved address");
    }
    ...
    function testApprovedAddressCanCallSetSubnodeRecord() public {
        ...
        vm.expectRevert(abi.encodeWithSignature("Unauthorised(bytes32,address)", TEST_NODE, APPROVED));
        nameWrapper.setSubnodeRecord(TEST_NODE, SUB_LABEL, ACCOUNT, address(0), 0, CAN_DO_EVERYTHING, 10000);
    ...
    function testApprovalIsClearedOnReRegistrationAndWrapOfExpiredName() public {
        ...
        nameWrapper.approve(APPROVED, TEST_NODE_ID);
        nameWrapper.setFuses(TEST_NODE, uint16(CANNOT_UNWRAP | CANNOT_APPROVE | PARENT_CANNOT_CONTROL));
        ...
        assertEq(nameWrapper.getApproved(TEST_NODE_ID), address(0), "Approval should appear cleared when expired");
        ...
        assertEq(nameWrapper.getApproved(TEST_NODE_ID), address(0), "Approval should remain cleared after re-wrap");
    ...
    function testApprovalIsNotClearedOnTransferIfCannotApproveIsBurnt() public {
        ...
        assertEq(nameWrapper.getApproved(TEST_NODE_ID), APPROVED, "Approval should NOT be cleared when CANNOT_APPROVE is burnt");
    ...
    function testApprovalIsClearedOnUnwrap() public {
        ...
        // Create subdomain first
        nameWrapper.setSubnodeOwner(TEST_NODE, SUB_LABEL, OWNER, CAN_DO_EVERYTHING, 0);
        
        // Set approval on subdomain
        nameWrapper.approve(APPROVED, SUB_NODE_ID);
        assertEq(nameWrapper.getApproved(SUB_NODE_ID), APPROVED, "Should be approved");
        
        // Unwrap subdomain
        nameWrapper.unwrap(TEST_NODE, SUB_LABEL_HASH, OWNER);
        
        // Verify approval is cleared
        assertEq(nameWrapper.getApproved(SUB_NODE_ID), address(0), "Approval should be cleared after unwrap");
        ...
        assertEq(nameWrapper.getApproved(SUB_NODE_ID), address(0), "Approval should remain cleared after re-wrap");
        
        // Re-approve to show approval can be reinstated
        nameWrapper.approve(APPROVED, SUB_NODE_ID);
        assertEq(nameWrapper.getApproved(SUB_NODE_ID), APPROVED, "Approval should be reinstated");
