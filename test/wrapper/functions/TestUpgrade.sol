...
contract Upgrade is BaseWrapperTest {
    ...
    function testUpgradeETH2LDByOwner() public {
        ...
        nameWrapper.setUpgradeContract(
            INameWrapperUpgrade(address(upgradeContract))
        );
        ...
        assertEq(
            nameWrapper.ownerOf(TEST_NODE_ID),
            address(0),
            "Should not be owned after upgrade"
        );

        // Check upgrade contract was called with correct parameters
        assertEq(
            upgradeContract.lastUpgradedName(),
            string(dnsName),
            "DNS name should match"
        );
        assertEq(
            upgradeContract.lastUpgradedOwner(),
            OWNER,
            "Owner should match"
        );
        assertEq(
            upgradeContract.lastUpgradedFuses(),
            fuses,
            "Fuses should match"
        );
        assertEq(
            upgradeContract.lastUpgradedExpiry(),
            expiry,
            "Expiry should match"
        );
        assertEq(
            upgradeContract.lastUpgradedApproved(),
            address(0),
            "Approved should be zero"
        );
        assertEq(
            upgradeContract.lastUpgradedExtraData(),
            "",
            "Extra data should be empty"
        );
    ...
    function testUpgradeByOperator() public {
        ...
        assertEq(
            upgradeContract.lastUpgradedOwner(),
            OWNER,
            "Original owner should be passed"
        );
    ...
    function testUpgradeWithFuses() public {
        ...
        assertEq(
            upgradeContract.lastUpgradedFuses(),
            expectedFuses,
            "Fuses should include set fuses plus automatic ones"
        );
        assertTrue(
            upgradeContract.lastUpgradedFuses() & CANNOT_UNWRAP != 0,
            "Should have CANNOT_UNWRAP"
        );
        assertTrue(
            upgradeContract.lastUpgradedFuses() & CANNOT_SET_RESOLVER != 0,
            "Should have CANNOT_SET_RESOLVER"
        );
        assertTrue(
            upgradeContract.lastUpgradedFuses() & PARENT_CANNOT_CONTROL != 0,
            "Should have PARENT_CANNOT_CONTROL"
        );
        assertTrue(
            upgradeContract.lastUpgradedFuses() & IS_DOT_ETH != 0,
            "Should have IS_DOT_ETH"
        );
    ...
    function testUpgradeWithApproval() public {
        ...
        assertEq(
            upgradeContract.lastUpgradedApproved(),
            NEW_OWNER,
            "Approved address should be passed"
        );
    ...
    function testUpgradeSubdomain() public {
        ...
        // Create subdomain
        nameWrapper.setSubnodeOwner(tldNode, CHILD_LABEL, OWNER, 0, 0);

        bytes32 subNode = keccak256(
            abi.encodePacked(tldNode, CHILD_LABEL_HASH)
        );
        ...
        bytes memory subDnsName = abi.encodePacked(
            uint8(3),
            CHILD_LABEL,
            uint8(3),
            tldLabel,
        ...
        assertEq(
            nameWrapper.ownerOf(subNodeId),
            address(0),
            "Subdomain should not be owned after upgrade"
        );
        assertEq(
            upgradeContract.lastUpgradedOwner(),
            OWNER,
    ...
    function testUpgradeKeepsFusesAndExpiryInStorage() public {
        ...
        (, uint32 storedFuses, uint64 storedExpiry) = nameWrapper.getData(
            TEST_NODE_ID
        );
        assertEq(
            storedFuses,
            initialFuses,
            "Fuses should be preserved in storage"
        );
        assertEq(
            storedExpiry,
            initialExpiry,
            "Expiry should be preserved in storage"
        );
    ...
    function testUpgradeWithSubdomainFuses() public {
        ...
        // Get expected values
        (, uint32 expectedFuses, uint64 expectedExpiry) = nameWrapper.getData(
            CHILD_NODE_ID
        );

        bytes memory childDnsName = abi.encodePacked(
            uint8(3),
            CHILD_LABEL,
            uint8(8),
            TEST_LABEL,
        ...
        assertEq(
            upgradeContract.lastUpgradedFuses(),
            expectedFuses,
            "Subdomain fuses should be passed"
        );
        assertEq(
            upgradeContract.lastUpgradedExpiry(),
            expectedExpiry,
            "Subdomain expiry should be passed"
        );
    ...
    function testUpgradeCorrectExpiry() public {
        ...
        uint64 expectedExpiry = uint64(
            registrarExpiry + baseRegistrar.GRACE_PERIOD()
        );
        ...
        assertEq(
            upgradeContract.lastUpgradedExpiry(),
            expectedExpiry,
            "Expiry should match registrar + grace period"
        );
