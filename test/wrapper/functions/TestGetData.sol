...
contract GetData is BaseWrapperTest {
    ...
    function testGetDataAfterUnwrap() public {
        ...
        assertTrue(
            ...
            "Domain should be wrapped initially"
        );
        ...
        assertFalse(
            nameWrapper.isWrapped(defaultNode),
            "Domain should not be wrapped after unwrap"
        );
        ...
        assertEq(
            owner,
            address(0),
            "Owner should be zero address after unwrap"
        );
    ...
    function testGetDataForSubdomainOfExpiredParent() public {
        ...
        assertEq(
            owner,
            address(0),
            "Subdomain owner should be zero when parent expired"
        );
        assertEq(
            fuses,
            0,
            "Subdomain fuses should be zero when parent expired"
        );
    ...
    function testGetDataConsistency() public {
        ...
        (address owner1, uint32 fuses1, uint64 expiry1) = nameWrapper.getData(
            defaultNodeId
        );
        (address owner2, uint32 fuses2, uint64 expiry2) = nameWrapper.getData(
            defaultNodeId
        );
