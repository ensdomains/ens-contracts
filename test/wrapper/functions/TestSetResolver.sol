...
contract SetResolver is BaseWrapperTest {
    ...
    function testSetResolverByOwner() public {
        ...
        assertEq(
            ...
            address(0),
            "Should have no resolver initially"
        );
    ...
    function testSetResolverAfterBurningOtherFuses() public {
        ...
        nameWrapper.setFuses(
            TEST_NODE,
            uint16(CANNOT_TRANSFER | CANNOT_SET_TTL)
        );
        ...
        assertEq(
            ens.resolver(TEST_NODE),
            RESOLVER,
            "Should be able to set resolver with other fuses burned"
        );
    ...
    function testSetResolverForSubdomain() public {
        ...
        assertEq(
            ens.resolver(subNode),
            RESOLVER,
            "Subdomain resolver should be set"
        );
    ...
    function testSetResolverUpdatesDNSRecord() public {
        ...
        assertEq(
            initialResolver,
            address(0),
            "Initial resolver should be zero"
        );
        ...
        assertEq(
            newResolver,
            RESOLVER,
            "Resolver should be updated in ENS registry"
        );
        assertNotEq(
            newResolver,
            initialResolver,
            "Resolver should have changed"
        );
