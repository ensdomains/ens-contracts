...
contract SetSubnodeOwner is Test {
    ...
    function _dnsEncodeName(string memory name) internal pure returns (bytes memory) {
    ...
    function _setSubnodeOwnerFixture() internal {
        ...
        nameWrapper.wrapETH2LD(LABEL, account0, uint16(CANNOT_UNWRAP), address(0));
    ...
    function _setSubnodeOwner(bytes32 parentNode, string memory label, address owner, uint32 fuses, uint64 expiry, uint256 accountIndex) internal {
    ...
    function testCanBeCalledByOwnerAndSetsContractAsOwnerOnENSRegistry() public {
        _setSubnodeOwnerFixture();
        
        vm.startPrank(account0);
        
        // await expectOwnerOf(name).on(nameWrapper).toBe(accounts[0])
        assertEq(nameWrapper.ownerOf(toNameId(NAME)), account0, "Parent should be owned by account0");
        ...
        assertEq(ensRegistry.owner(namehash(SUBNAME)), address(nameWrapper), "ENS should be owned by NameWrapper");
        
        // await expectOwnerOf(subname).on(nameWrapper).toBe(accounts[0])
        assertEq(nameWrapper.ownerOf(toNameId(SUBNAME)), account0, "NameWrapper should be owned by account0");
    ...
    function testTransfersWrappedTokenToTargetAddress() public {
        ...
        assertEq(nameWrapper.ownerOf(toNameId(SUBNAME)), account1, "NameWrapper should be owned by account1");
    ...
    function testWillNotAllowWrappingWithTargetAddressZero() public {
        ...
        nameWrapper.setSubnodeOwner(parentNode, SUBLABEL, ZERO_ACCOUNT, CAN_DO_EVERYTHING, 0);
    ...
    function testWillNotAllowWrappingWithWrapperContractAddress() public {
        ...
        nameWrapper.setSubnodeOwner(parentNode, SUBLABEL, address(nameWrapper), CAN_DO_EVERYTHING, 0);
    ...
    function testDoesNotAllowAnyoneElseToWrapNameEvenIfOwnerAuthorisedWrapper() public {
        ...
        vm.expectRevert(abi.encodeWithSignature("Unauthorised(bytes32,address)", expectedParentNode, account1));
        nameWrapper.setSubnodeOwner(parentNode, SUBLABEL, account0, CAN_DO_EVERYTHING, 0);
        
        vm.stopPrank();
    }
    
    // TEST 7: "Fuses cannot be burned if the name does not have PARENT_CANNOT_CONTROL burned"
    function testFusesCannotBeBurnedIfNameDoesNotHaveParentCannotControlBurned() public {
        ...
        vm.expectRevert(abi.encodeWithSignature("OperationProhibited(bytes32)", expectedSubnode));
        nameWrapper.setSubnodeOwner(parentNode, SUBLABEL, account0, CANNOT_UNWRAP | CANNOT_TRANSFER, MAX_EXPIRY);
    ...
    function testDoesNotAllowFusesToBeBurnedIfCannotUnwrapNotBurned() public {
        ...
        // await expect(nameWrapper).write('setSubnodeOwner', [...]).toBeRevertedWithCustomError('OperationProhibited')
        vm.expectRevert(abi.encodeWithSignature("OperationProhibited(bytes32)", expectedSubnode));
        nameWrapper.setSubnodeOwner(parentNode, SUBLABEL, account0, PARENT_CANNOT_CONTROL | CANNOT_TRANSFER, 0);
        
        vm.stopPrank();
    }
    
    // TEST 9: "Allows fuses to be burned if CANNOT_UNWRAP and PARENT_CANNOT_CONTROL is burned and is not expired"
    function testAllowsFusesToBeBurnedIfCannotUnwrapAndParentCannotControlBurnedAndNotExpired() public {
        // note: not using suite specific fixture here
        ...
        bytes32 parentNode = namehash(NAME);
        nameWrapper.setSubnodeOwner(parentNode, SUBLABEL, account0, CANNOT_UNWRAP | PARENT_CANNOT_CONTROL | CANNOT_SET_RESOLVER, MAX_EXPIRY);
        
        bytes32 subnodeHash = namehash(SUBNAME);
        uint32 expectedFuses = CANNOT_UNWRAP | PARENT_CANNOT_CONTROL | CANNOT_SET_RESOLVER;
        ...
        assertTrue(nameWrapper.allFusesBurned(subnodeHash, expectedFuses), "All expected fuses should be burned");
    ...
    function testDoesNotAllowIsDotEthToBeBurned() public {
        ...
        bytes32 parentNode = namehash(NAME);
        bytes32 expectedSubnode = namehash(SUBNAME);
        uint32 invalidFuses = CANNOT_UNWRAP | PARENT_CANNOT_CONTROL | CANNOT_SET_RESOLVER | IS_DOT_ETH;
        
        // Should revert when trying to burn IS_DOT_ETH on subdomain
        vm.expectRevert(abi.encodeWithSignature("OperationProhibited(bytes32)", expectedSubnode));
        nameWrapper.setSubnodeOwner(parentNode, SUBLABEL, account0, invalidFuses, MAX_EXPIRY);
    ...
    function testCompleteFixtureSetup() public view {
        ...
        assertEq(SUBNAME, "sub.ownerandwrap.eth", "Subname constant should match");
    }
    ...
    function testSetSubnodeOwnerFixtureSetup() public {
        ...
        assertEq(nameWrapper.ownerOf(toNameId(NAME)), account0, "Parent domain should be wrapped and owned by account0");
        assertEq(ensRegistry.owner(namehash(NAME)), address(nameWrapper), "Parent domain should be owned by NameWrapper in ENS");
        ...
        assertEq(fuses, expectedFuses, "Parent domain should have expected fuses");
    }
    ...
    function testSubnodeCreationWithDifferentFuses() public {
        ...
        for (uint256 i = 0; i < testFuses.length; i++) {
            if (i >= 2) { // Only test with proper fuse combinations for CANNOT_UNWRAP
                nameWrapper.setSubnodeOwner(parentNode, testLabels[i], account0, testFuses[i], MAX_EXPIRY);
                
                bytes32 subnodeHash = keccak256(abi.encodePacked(parentNode, keccak256(bytes(testLabels[i]))));
                assertTrue(nameWrapper.allFusesBurned(subnodeHash, testFuses[i]), 
                    string(abi.encodePacked("Fuses should be burned for ", testLabels[i])));
            }
        }
    ...
    function testSubnodeOwnershipTransfer() public {
        ...
        nameWrapper.setSubnodeOwner(parentNode, SUBLABEL, account1, CAN_DO_EVERYTHING, 0);
        
        assertEq(nameWrapper.ownerOf(toNameId(SUBNAME)), account1, "Subdomain should be owned by account1");
        ...
        nameWrapper.safeTransferFrom(account1, account2, toNameId(SUBNAME), 1, "");
        
        assertEq(nameWrapper.ownerOf(toNameId(SUBNAME)), account2, "Subdomain should be owned by account2 after transfer");
    ...
    function testSubnodeExpiryHandling() public {
        ...
        nameWrapper.setSubnodeOwner(parentNode, SUBLABEL, account0, PARENT_CANNOT_CONTROL, shortExpiry);
    ...
    function testCannotCreateSubdomainWithoutCannotCreateSubdomainFuse() public {
        _registerSetupAndWrapName(LABEL, CANNOT_UNWRAP);
    ...
    function testDoesNotAllowFusesToBeBurnedIfCannotUnwrapAndParentCannotControlBurnedButExpired() public {
        // note: not using suite specific fixture here
        ...
        uint32 expectedParentFuses = PARENT_CANNOT_CONTROL | CANNOT_UNWRAP | IS_DOT_ETH;
        assertEq(parentFuses, expectedParentFuses, "Parent should have expected fuses");
        
        nameWrapper.setSubnodeOwner(parentNode, SUBLABEL, account0, CANNOT_UNWRAP | PARENT_CANNOT_CONTROL, 0); // set expiry to 0
        
        assertFalse(nameWrapper.allFusesBurned(namehash(SUBNAME), PARENT_CANNOT_CONTROL), "PARENT_CANNOT_CONTROL should not be burned when expired");
    ...
    function testNormalisesMaxExpiryOfSubdomainToParentExpiry() public {
        ...
        bytes32 parentNode = namehash(NAME);
        nameWrapper.setSubnodeOwner(parentNode, SUBLABEL, account0, CANNOT_UNWRAP | PARENT_CANNOT_CONTROL, MAX_EXPIRY);
        
        (, , uint64 expiry) = nameWrapper.getData(toNameId(SUBNAME));
        
        assertEq(uint256(expiry), expectedExpiry + baseRegistrar.GRACE_PERIOD(), "Expiry should be normalised to parent's expiry");
    ...
    function testEmitsTransferSingleEvent() public {
        ...
        emit TransferSingle(account0, address(0), account1, toNameId(SUBNAME), 1);
    ...
    function testWillNotCreateSubdomainWithEmptyLabel() public {
        ...
        nameWrapper.setSubnodeOwner(parentNode, "", account0, CAN_DO_EVERYTHING, 0);
    ...
    function testShouldBeAbleToCallTwiceAndChangeOwner() public {
        ...
        assertEq(nameWrapper.ownerOf(toNameId(SUBNAME)), account0, "Subdomain should be owned by account0");
        
        nameWrapper.setSubnodeOwner(parentNode, SUBLABEL, account1, 0, 0);
        
        assertEq(nameWrapper.ownerOf(toNameId(SUBNAME)), account1, "Subdomain should be owned by account1 after second call");
    ...
    function testSettingOwnerToZeroBurnsAndUnwraps() public {
        ...
        assertEq(nameWrapper.ownerOf(toNameId(NAME)), account0, "Name should be wrapped and owned by account0");
        
        // NameWrapper.setSubnodeOwner to account1
        bytes32 parentNode = namehash(NAME);
        nameWrapper.setSubnodeOwner(parentNode, SUBLABEL, account1, 0, MAX_EXPIRY);
        
        vm.expectEmit(true, false, false, true);
        emit NameUnwrapped(namehash(SUBNAME), ZERO_ACCOUNT);
        
        nameWrapper.setSubnodeOwner(parentNode, SUBLABEL, ZERO_ACCOUNT, PARENT_CANNOT_CONTROL, MAX_EXPIRY);
        
        require(nameWrapper.ownerOf(toNameId(SUBNAME)) == address(0), "Subdomain should be burned (owner 0)");
        
        vm.stopPrank();
    }
    
    // TEST 18: "Unwrapping within an external contract does not create any state inconsistencies"
    function testUnwrappingWithinExternalContractDoesNotCreateStateInconsistencies() public {
        _registerSetupAndWrapName(LABEL, CAN_DO_EVERYTHING);
        ...
        bytes32 parentNode = namehash(NAME);
        nameWrapper.setSubnodeOwner(parentNode, SUBLABEL, account0, CAN_DO_EVERYTHING, MAX_EXPIRY);
        
        // attempt to move owner to testReentrancy, which unwraps domain itself to account while keeping ERC1155 to testReentrancy
        bytes32 expectedSubnode = namehash(SUBNAME);
        vm.expectRevert(abi.encodeWithSignature("OperationProhibited(bytes32)", expectedSubnode));
        nameWrapper.setSubnodeOwner(parentNode, SUBLABEL, address(testReentrancy), CANNOT_UNWRAP | PARENT_CANNOT_CONTROL, MAX_EXPIRY);
    ...
    function testUnwrappingPreviouslyWrappedUnexpiredNameRetainsPCC() public {
        ...
        // NameWrapper.setSubnodeOwner to account1
        bytes32 parentNode = namehash(NAME);
        nameWrapper.setSubnodeOwner(parentNode, SUBLABEL, account1, PARENT_CANNOT_CONTROL, MAX_EXPIRY);
        
        // Confirm fuses are set
        (, uint32 fusesBefore,) = nameWrapper.getData(toNameId(SUBNAME));
        assertEq(fusesBefore, PARENT_CANNOT_CONTROL, "PARENT_CANNOT_CONTROL should be set");
        ...
        (address owner, uint32 fuses, uint64 expiry) = nameWrapper.getData(toNameId(SUBNAME));
        
        assertEq(owner, address(0), "Owner should be zero after unwrap");
        assertEq(uint256(expiry), parentExpiry + baseRegistrar.GRACE_PERIOD(), "Expiry should be parent expiry + grace period");
        assertEq(fuses, PARENT_CANNOT_CONTROL, "PARENT_CANNOT_CONTROL should remain set");
    ...
    function testRewrappingExpiredNameWithPCCBurnedResetsFuses() public {
        ...
        nameWrapper.setSubnodeOwner(parentNode, SUBLABEL, account1, PARENT_CANNOT_CONTROL, shortExpiry);
        ...
        assertEq(owner, address(0), "Owner should be zero after unwrap");
        assertEq(uint256(expiry), uint256(shortExpiry), "Expiry should match short expiry");
        assertEq(fuses, PARENT_CANNOT_CONTROL, "PARENT_CANNOT_CONTROL should remain set");
        ...
        assertEq(uint256(expiryAfter), uint256(shortExpiry), "Expiry should remain the same");
        ...
        assertEq(nameWrapper.ownerOf(toNameId(SUBNAME)), account1, "Subdomain should be owned by account1 after rewrap");
        
        (address rawOwner, uint32 rawFuses, uint64 expiry2) = nameWrapper.getData(toNameId(SUBNAME));
        assertEq(rawFuses, 0, "Raw fuses should be 0");
        assertEq(rawOwner, account1, "Raw owner should be account1");
        
        // Verify expiry behavior
        assertTrue(uint256(expiry2) < timestamp, "New expiry should be less than current timestamp");
        
        vm.stopPrank();
    }
    
    // TEST 21: "Expired subnames should still be protected by CANNOT_CREATE_SUBDOMAIN on the parent"
    function testExpiredSubnamesProtectedByCannotCreateSubdomainOnParent() public {
        // note: not using suite specific fixture here
        _registerSetupAndWrapName(LABEL, CANNOT_UNWRAP);
        ...
        bytes32 expectedSubnode2 = keccak256(abi.encodePacked(parentNode, keccak256(bytes(sublabel2))));
        vm.expectRevert(abi.encodeWithSignature("OperationProhibited(bytes32)", expectedSubnode2));
        nameWrapper.setSubnodeOwner(parentNode, sublabel2, account1, 0, shortExpiry);
        ...
        assertEq(uint256(expiry), uint256(shortExpiry), "Expiry should match shortExpiry");
        assertTrue(uint256(expiry) < timestamp, "Expiry should be less than current timestamp");
        assertEq(fuses, 0, "Fuses should be reset after expiry");
        
        bytes32 expectedSubnode = namehash(SUBNAME);
        vm.expectRevert(abi.encodeWithSignature("OperationProhibited(bytes32)", expectedSubnode));
        nameWrapper.setSubnodeOwner(parentNode, SUBLABEL, account1, 0, shortExpiry);
        
        vm.stopPrank();
    }
    
    // TEST 22: "Burning a name still protects it from the parent as long as it is unexpired and has PCC burnt"
    function testBurningNameStillProtectsFromParentWhenUnexpiredWithPCC() public {
        // note: not using suite specific fixture here
        _registerSetupAndWrapName(LABEL, CANNOT_UNWRAP);
        ...
        assertEq(fuses, PARENT_CANNOT_CONTROL, "PARENT_CANNOT_CONTROL should remain set");
        assertEq(ensRegistry.owner(namehash(SUBNAME)), address(0), "ENS owner should be zero");
        
        // attempt to take back the name
        bytes32 expectedSubnode = namehash(SUBNAME);
        vm.expectRevert(abi.encodeWithSignature("OperationProhibited(bytes32)", expectedSubnode));
        nameWrapper.setSubnodeOwner(parentNode, SUBLABEL, account0, PARENT_CANNOT_CONTROL, MAX_EXPIRY);
...
contract TestNameWrapperReentrancy {
    ...
    constructor(address _account, address _nameWrapper, bytes32 _testNode, bytes32 _subLabel) {
        account = _account;
