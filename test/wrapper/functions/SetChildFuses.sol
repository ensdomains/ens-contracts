...
contract SetChildFuses is Test {
    ...
    function _wrapParentWithChild() internal {
        ...
        nameWrapper.setSubnodeOwner(TEST_NODE, CHILD_LABEL, CHILD_OWNER, 0, 0);
    ...
    function testSetChildFusesByParentOwner() public {
        ...
        assertTrue(
            newFuses & PARENT_CANNOT_CONTROL != 0,
        ...
        assertEq(
            ...
            "Child expiry should be parent expiry + grace period"
        );
    ...
    function testSetChildFusesParentControlledFusesWithoutPCC() public {
        ...
        (, uint32 newFuses, ) = nameWrapper.getData(CHILD_NODE_ID);
        assertTrue(
            newFuses & parentControlledFuse != 0,
            "Should have parent controlled fuse"
        );
    ...
    function testSetChildFusesForTLD() public {
        ...
        // Wrap TLD
        bytes memory tldDnsName = abi.encodePacked(
            uint8(7),
            tldLabel,
            uint8(0)
        );
        ...
        assertTrue(
            ...
            "TLD should have CANNOT_UNWRAP fuse"
        );
        assertTrue(
            fuses & PARENT_CANNOT_CONTROL != 0,
            "TLD should have PARENT_CANNOT_CONTROL fuse"
        );
    ...
    function testCannotSetChildFusesWithPCCAlreadyBurnedEvenIfPCCIncluded()
        public
    {
        _wrapParentWithChild();
    ...
    function testCannotSetChildFusesWithoutCannotUnwrapEvenWithOtherFuses()
        public
    {
        _wrapParentWithChild();
    ...
    function testSetChildFusesParentControlledOnExpiredWithExpiryExtension()
        public
    {
        _wrapParentWithChild();
        ...
        (, uint32 newFuses, uint64 finalExpiry) = nameWrapper.getData(
            CHILD_NODE_ID
        );
        assertTrue(
            newFuses & parentControlledFuse != 0,
    ...
    function testSetChildFusesStateTransitions() public {
        ...
        assertTrue(
            stateTwoFuses & parentControlledFuse != 0,
            "Should have parent controlled fuse"
        );
        assertTrue(
            stateTwoFuses & PARENT_CANNOT_CONTROL == 0,
            "Should not have PCC yet"
        );
        ...
        assertTrue(
            stateThreeFuses & CANNOT_UNWRAP != 0,
            "Should have CANNOT_UNWRAP"
        );
        assertTrue(
            stateThreeFuses & PARENT_CANNOT_CONTROL != 0,
            "Should have PCC"
        );
        assertTrue(
            stateThreeFuses & parentControlledFuse != 0,
            "Should retain parent controlled fuse"
        );
