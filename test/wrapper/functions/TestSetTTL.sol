...
contract SetTTL is BaseWrapperTest {
    ...
    function testSetTTLAfterBurningOtherFuses() public {
        ...
        nameWrapper.setFuses(TEST_NODE, uint16(CANNOT_TRANSFER | CANNOT_SET_RESOLVER));
        
        // Should still be able to set TTL
        uint64 newTTL = 3600;
        nameWrapper.setTTL(TEST_NODE, newTTL);
        assertEq(ens.ttl(TEST_NODE), newTTL, "Should be able to set TTL with other fuses burned");
    ...
    function testSetTTLPreservesOtherRecord() public {
        ...
        // Verify both are set
        assertEq(ens.resolver(TEST_NODE), resolver, "Resolver should be preserved");
        assertEq(ens.ttl(TEST_NODE), 3600, "TTL should be set");
        
        // Change TTL, verify resolver is preserved
        nameWrapper.setTTL(TEST_NODE, 7200);
        assertEq(ens.resolver(TEST_NODE), resolver, "Resolver should still be preserved");
