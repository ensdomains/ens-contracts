...
contract SetFuses is Test {
    ...
    bytes32 constant NAME_NODE =
        keccak256(abi.encodePacked(ETH_NODE, LABEL_HASH));
    ...
    bytes32 constant SUB_NAME_NODE =
        keccak256(abi.encodePacked(NAME_NODE, SUB_LABEL_HASH));
    ...
    function testCannotBurnParentCannotControl() public {
        ...
        assertFalse(
            success,
            "Setting PARENT_CANNOT_CONTROL directly should fail"
        );
    ...
    function testWillNotAllowBurningFusesIfParentCannotControlNotBurned()
        public
    {
        _registerSetupAndWrapName(CANNOT_UNWRAP);
    ...
    function testWillNotAllowBurningFusesOfSubdomainsIfCannotUnwrapNotBurned()
        public
    {
        _registerSetupAndWrapName(CANNOT_UNWRAP);
    ...
    function testWillNotAllowBurningFusesOfEthNamesUnlessCannotUnwrapBurned()
        public
    {
        ...
        // expect(await nameWrapper).write('setFuses', [...]).toBeRevertedWithCustomError('OperationProhibited')
        vm.expectRevert(
            abi.encodeWithSignature("OperationProhibited(bytes32)", NAME_NODE)
        );
    ...
    function testCanBeCalledByTheOwner() public {
        ...
        assertEq(
            ...
            "Initial fuses should match expected"
        );
        ...
        uint32 expectedNewFuses = CANNOT_UNWRAP |
            CANNOT_TRANSFER |
            PARENT_CANNOT_CONTROL |
            IS_DOT_ETH;
    ...
    function testEmitsFusesSetEvent() public {
        ...
        uint256 expectedExpiry = baseRegistrar.nameExpires(LABEL_ID) +
    ...
    function testReturnsTheCorrectFuses() public {
        ...
        uint32 fusesReturned = nameWrapper.setFuses(
            NAME_NODE,
        ...
        assertEq(
            fusesReturned,
            expectedFuses,
            "Returned fuses should match expected (old fuses)"
        );
    ...
    function testCanBeCalledByAuthorisedAccount() public {
        ...
        assertEq(
            fuses,
            expectedFuses,
            "Authorized account should be able to set fuses"
        );
    ...
    function testCanSetFusesAndThenBurnAbilityToBurnFuses() public {
        ...
        assertEq(
            nameWrapper.ownerOf(NAME_NODE_ID),
            account0,
            "Owner should still be account0"
        );

        // check flag in the wrapper
        assertTrue(
            nameWrapper.allFusesBurned(NAME_NODE, CANNOT_BURN_FUSES),
            "CANNOT_BURN_FUSES should be burned"
        );
    ...
    function testCanSetFusesAndBurnTransfer() public {
        ...
        // check flag in the wrapper
        assertTrue(
            nameWrapper.allFusesBurned(NAME_NODE, CANNOT_TRANSFER),
            "CANNOT_TRANSFER should be burned"
        );
    ...
    function testCanSetFusesAndBurnCanSetResolverAndCanSetTTL() public {
        ...
        nameWrapper.setFuses(
            NAME_NODE,
            uint16(CANNOT_SET_RESOLVER | CANNOT_SET_TTL)
        );
        ...
        assertTrue(
            ...
            "Resolver and TTL fuses should be burned"
        );
    ...
    function testCanSetFusesAndBurnCanCreateSubdomains() public {
        ...
        assertFalse(
            nameWrapper.allFusesBurned(NAME_NODE, CANNOT_CREATE_SUBDOMAIN),
            "CANNOT_CREATE_SUBDOMAIN should not be burned initially"
        );
        ...
        assertEq(
            ens.owner(
                keccak256(abi.encodePacked(NAME_NODE, keccak256("creatable")))
            ),
            address(nameWrapper),
            "Subdomain should be owned by NameWrapper in ENS"
        );

        // await expectOwnerOf(`creatable.${name}`).on(nameWrapper).toBe(accounts[0])
        uint256 creatableNodeId = uint256(
            keccak256(abi.encodePacked(NAME_NODE, keccak256("creatable")))
        );
        assertEq(
            nameWrapper.ownerOf(creatableNodeId),
            account0,
            "Subdomain should be owned by account0 in NameWrapper"
        );

        nameWrapper.setFuses(
            NAME_NODE,
            uint16(CAN_DO_EVERYTHING | CANNOT_CREATE_SUBDOMAIN)
        );
        ...
        assertEq(
            nameWrapper.ownerOf(NAME_NODE_ID),
            account0,
            "Parent domain should still be owned by account0"
        );

        assertTrue(
            nameWrapper.allFusesBurned(NAME_NODE, CANNOT_CREATE_SUBDOMAIN),
            "CANNOT_CREATE_SUBDOMAIN should be burned"
        );

        // try to create a subdomain
        bytes32 uncreatableNode = keccak256(
            abi.encodePacked(NAME_NODE, keccak256("uncreatable"))
        );
        vm.expectRevert(
            abi.encodeWithSignature(
                "OperationProhibited(bytes32)",
    ...
    function testCompleteFixtureSetup() public view {
        ...
        assertTrue(
            address(publicResolver) != address(0),
            "Public Resolver should be deployed"
        );
        ...
        assertTrue(
            baseRegistrar.controllers(address(nameWrapper)),
            "NameWrapper should be controller"
        );
        assertTrue(
    ...
    function testAllFuseTypes() public {
        ...
        for (uint256 i = 0; i < individualFuses.length; i++) {
            ...
            string memory testLabel = string(
                abi.encodePacked("fuse", vm.toString(i))
            );
            bytes32 testLabelHash = keccak256(bytes(testLabel));
            uint256 testLabelId = uint256(testLabelHash);
            bytes32 testNameNode = keccak256(
                abi.encodePacked(ETH_NODE, testLabelHash)
            );
            ...
            assertTrue(
                nameWrapper.allFusesBurned(testNameNode, individualFuses[i]),
                string(
                    abi.encodePacked(
                        "Fuse ",
                        vm.toString(individualFuses[i]),
                        " should be burned"
                    )
                )
            );
    ...
    function testFuseEnforcement() public {
        ...
        bytes32 transferNode = keccak256(
            abi.encodePacked(ETH_NODE, keccak256("transfer"))
        );
        ...
        bytes32 resolverNode = keccak256(
            abi.encodePacked(ETH_NODE, keccak256("resolver"))
        );
        vm.expectRevert(
            abi.encodeWithSignature(
                "OperationProhibited(bytes32)",
        ...
        bytes32 ttlNode = keccak256(
            abi.encodePacked(ETH_NODE, keccak256("ttltest"))
        );
        vm.expectRevert(
            abi.encodeWithSignature("OperationProhibited(bytes32)", ttlNode)
        );
