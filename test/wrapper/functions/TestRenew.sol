...
contract Renew is BaseWrapperTest {
    ...
    function _registerAndWrapTestDomain() internal returns (uint256 initialExpiry) {
        vm.startPrank(OWNER);
    ...
    function testRenewName() public {
        ...
        assertEq(newExpiry, initialExpiry + extension, "Registrar expiry should be extended");
    ...
    function testRenewExtendsWrapperExpiry() public {
        ...
        uint256 expectedWrapperExpiry = initialExpiry + extension + baseRegistrar.GRACE_PERIOD();
        
        assertEq(newWrapperExpiry, expectedWrapperExpiry, "Wrapper expiry should be extended");
        assertEq(newWrapperExpiry, initialWrapperExpiry + extension, "Wrapper expiry should increase by extension");
    ...
    function testRenewMaintainsOwnership() public {
        ...
        assertEq(nameWrapper.ownerOf(TEST_NODE_ID), OWNER, "Should be owned by OWNER initially");
        ...
        assertEq(nameWrapper.ownerOf(TEST_NODE_ID), OWNER, "ownerOf should return OWNER after renewal");
    ...
    function testRenewWithFuses() public {
        ...
        assertEq(newFuses, initialFuses, "Fuses should be maintained after renewal");
        assertTrue(newFuses & CANNOT_UNWRAP != 0, "Should maintain CANNOT_UNWRAP fuse");
        assertTrue(newFuses & CANNOT_SET_RESOLVER != 0, "Should maintain CANNOT_SET_RESOLVER fuse");
    ...
    function testRenewExpiredName() public {
        ...
        // Get initial state
        (, uint32 initialFuses, uint64 initialWrapperExpiry) = nameWrapper.getData(TEST_NODE_ID);
        uint256 initialRegistrarExpiry = baseRegistrar.nameExpires(TEST_LABEL_ID);
        
        // Wrapper expiry should be registrar expiry + grace period
        assertEq(initialWrapperExpiry, initialRegistrarExpiry + baseRegistrar.GRACE_PERIOD(), "Initial wrapper expiry check");
        ...
        assertEq(nameWrapper.ownerOf(TEST_NODE_ID), OWNER, "Domain should still show owner");
        
        // But registrar should still be renewable (within grace)
        assertTrue(initialRegistrarExpiry + baseRegistrar.GRACE_PERIOD() >= block.timestamp, "Should be within registrar grace period");
        ...
        assertEq(newRegistrarExpiry, initialRegistrarExpiry + DAY, "Registrar should be extended by 1 day");
        
        // Check wrapper expiry
        (, uint32 newFuses, uint64 newWrapperExpiry) = nameWrapper.getData(TEST_NODE_ID);
        assertEq(newWrapperExpiry, newRegistrarExpiry + baseRegistrar.GRACE_PERIOD(), "Wrapper expiry should be registrar + grace period");
        ...
        if (newWrapperExpiry <= block.timestamp) {
            assertEq(nameWrapper.ownerOf(TEST_NODE_ID), address(0), "Domain should still appear expired");
        } else {
            assertEq(nameWrapper.ownerOf(TEST_NODE_ID), OWNER, "Domain should be unexpired");
        }
    ...
    function testRenewUnexpiresDomain() public {
        ...
        assertEq(nameWrapper.ownerOf(TEST_NODE_ID), OWNER, "Domain should still be owned during grace period");
        
        // Get current wrapper expiry before renewal
        (, , uint64 currentWrapperExpiry) = nameWrapper.getData(TEST_NODE_ID);
        assertEq(currentWrapperExpiry, initialWrapperExpiry, "Wrapper expiry should not have changed yet");
        ...
        assertEq(newRegistrarExpiry, initialRegistrarExpiry + renewalDuration, "Registrar expiry should be extended");
        
        // Verify wrapper expiry was extended
        assertEq(newWrapperExpiry, newRegistrarExpiry + baseRegistrar.GRACE_PERIOD(), "Wrapper expiry should be extended");
        assertTrue(newWrapperExpiry > initialWrapperExpiry, "New wrapper expiry should be later than initial");
        
        // Domain should still be owned
        assertEq(nameWrapper.ownerOf(TEST_NODE_ID), OWNER, "Domain should remain owned after renewal");
    ...
    function testRenewMultipleTimes() public {
        ...
        assertEq(firstRenewalExpiry, initialExpiry + 86400, "First renewal should extend by 1 day");
        
        // Second renewal
        nameWrapper.renew(TEST_LABEL_ID, 2 * 86400);
        uint256 secondRenewalExpiry = baseRegistrar.nameExpires(TEST_LABEL_ID);
        assertEq(secondRenewalExpiry, firstRenewalExpiry + 2 * 86400, "Second renewal should extend by 2 days");
        
        // Check wrapper expiry follows
        (, , uint64 wrapperExpiry) = nameWrapper.getData(TEST_NODE_ID);
        assertEq(wrapperExpiry, secondRenewalExpiry + baseRegistrar.GRACE_PERIOD(), "Wrapper expiry should follow registrar");
    ...
    function testRenewZeroDuration() public {
        ...
        uint256 newExpiry = baseRegistrar.nameExpires(TEST_LABEL_ID);
        assertEq(newExpiry, initialExpiry, "Zero duration renewal should not change expiry");
    ...
    function testRenewLongDuration() public {
        ...
        uint256 newExpiry = baseRegistrar.nameExpires(TEST_LABEL_ID);
        assertEq(newExpiry, initialExpiry + longDuration, "Long duration renewal should extend correctly");
        
        // Check wrapper expiry
        (, , uint64 wrapperExpiry) = nameWrapper.getData(TEST_NODE_ID);
        assertEq(wrapperExpiry, newExpiry + baseRegistrar.GRACE_PERIOD(), "Wrapper expiry should be registrar expiry + grace period");
    ...
    function testRenewMaintainsRegistrarOwnership() public {
        ...
        assertEq(baseRegistrar.ownerOf(TEST_LABEL_ID), address(nameWrapper), "Registrar should still show wrapper as owner");
    ...
    function testRenewWithDifferentController() public {
        ...
        uint256 newExpiry = baseRegistrar.nameExpires(TEST_LABEL_ID);
        assertEq(newExpiry, initialExpiry + 86400, "Renewal should work with different controller");
        
        vm.stopPrank();
    }
    
    // Helper function to register through controller
    function _registerThroughController(string memory label, uint256 duration) internal returns (uint256) {
        ...
        IPriceOracle.Price memory priceStruct = controller.rentPrice(label, duration);
    ...
    function _renewThroughController(string memory label, uint256 duration) internal {
        IPriceOracle.Price memory priceStruct = controller.rentPrice(label, duration);
        uint256 price = priceStruct.base + priceStruct.premium;
        controller.renew{value: price}(label, duration);
    }
    ...
    function testRenewalThroughControllerVsWrapper() public {
        ...
        uint256 controllerLabelId = _registerThroughController("controller", initialDuration);
        ...
        uint256 controllerInitialExpiry = baseRegistrar.nameExpires(controllerLabelId);
        uint256 wrapperInitialExpiry = baseRegistrar.nameExpires(wrapperLabelId);
        ...
        uint256 controllerNewExpiry = baseRegistrar.nameExpires(controllerLabelId);
        uint256 wrapperNewExpiry = baseRegistrar.nameExpires(wrapperLabelId);
        
        assertEq(controllerNewExpiry, controllerInitialExpiry + renewalDuration, "Controller renewal should work");
        assertEq(wrapperNewExpiry, wrapperInitialExpiry + renewalDuration, "Wrapper renewal should work");
    ...
    function testControllerCanRenewWrappedName() public {
        ...
        IPriceOracle.Price memory renewalPriceStruct = controller.rentPrice(TEST_LABEL, 180 days);
        uint256 renewalPrice = renewalPriceStruct.base + renewalPriceStruct.premium;
        
        uint256 expiryBefore = baseRegistrar.nameExpires(TEST_LABEL_ID);
        controller.renew{value: renewalPrice}(TEST_LABEL, 180 days);
        uint256 expiryAfter = baseRegistrar.nameExpires(TEST_LABEL_ID);
        
        assertEq(expiryAfter, expiryBefore + 180 days, "Controller should be able to renew wrapped names");
    ...
    function testRenewalPriceConsistency() public {
        ...
        assertTrue(controllerPrice > 0, "Controller should return non-zero price for renewal");
        
        // Wrapper renewal doesn't require payment (only gas)
        // But should still respect the same duration constraints
        assertTrue(duration >= 86400, "Duration should meet minimum requirements");
    }
}
