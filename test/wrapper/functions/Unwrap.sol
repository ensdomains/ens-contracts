...
contract Unwrap is Test {
    ...
    function _splitName(string memory name) internal pure returns (string[] memory) {
    ...
    function _registerSetupAndWrapNameNoPrank(string memory label, uint32 fuses) internal {
        // Move past grace period and register domain
        vm.warp(block.timestamp + baseRegistrar.GRACE_PERIOD() + 1);
        baseRegistrar.register(toLabelId(label), account0, 365 days);
        baseRegistrar.setApprovalForAll(address(nameWrapper), true);
    ...
    function testAllowsOwnerToUnwrapName() public {
        ...
        _setSubnodeOwner(parentNode, childLabel, account0, CAN_DO_EVERYTHING, 0);
        
        // await expectOwnerOf(childName).on(nameWrapper).equal(accounts[0])
        assertEq(nameWrapper.ownerOf(toNameId(childName)), account0, "NameWrapper should own child before unwrap");
        ...
        assertEq(ensRegistry.owner(namehash(childName)), account0, "ENS Registry should own child after unwrap");
    ...
    function testWillNotAllowPreviousOwnerToUnwrapNameWhenNameExpires() public {
        ...
        baseRegistrar.setApprovalForAll(address(nameWrapper), true);
        nameWrapper.wrapETH2LD(parentLabel, account0, uint16(CANNOT_UNWRAP), address(0));
        
        bytes32 parentNode = namehash(parentName);
        // Get parent expiry and use it for child to match normalization behavior
        uint256 parentExpiry = baseRegistrar.nameExpires(toLabelId(parentLabel));
        uint64 childExpiry = uint64(parentExpiry + baseRegistrar.GRACE_PERIOD());
        _setSubnodeOwner(parentNode, childLabel, account0, PARENT_CANNOT_CONTROL, childExpiry);
        ...
        vm.expectRevert(abi.encodeWithSignature("Unauthorised(bytes32,address)", childNode, account0));
    ...
    function testEmitsTransferSingleEvent() public {
        ...
        emit TransferSingle(account0, account0, ZERO_ACCOUNT, expectedTokenId, 1);
    ...
    function testAllowsAuthorisedAccountOnNFTWrapperToUnwrapName() public {
        ...
        assertEq(nameWrapper.ownerOf(toNameId(label)), account0, "NameWrapper should own before unwrap");
        ...
        assertEq(ensRegistry.owner(namehash(label)), account1, "ENS Registry should be owned by account1");
        
        // await expectOwnerOf(label).on(nameWrapper).equal(zeroAccount)
        assertEq(nameWrapper.ownerOf(toNameId(label)), ZERO_ACCOUNT, "NameWrapper should have zero owner");
    ...
    function testDoesNotAllowAnyoneElseToUnwrapName() public {
        ...
        // await expectOwnerOf(label).on(nameWrapper).equal(accounts[0])
        assertEq(nameWrapper.ownerOf(toNameId(label)), account0, "NameWrapper should own before unwrap attempt");
    ...
    function testDoesNotAllowAccountAuthorisedByOwnerOnENSRegistryToUnwrapName() public {
        string memory label = "abc";
        ...
        assertEq(ensRegistry.owner(namehash(label)), account1, "ENS Registry should be owned by account1");
        assertTrue(ensRegistry.isApprovedForAll(account1, account0), "account0 should be approved for all account1's names");
        ...
        assertEq(nameWrapper.ownerOf(toNameId(label)), account1, "NameWrapper should be owned by account1");
    ...
    function testWillNotUnwrapEth2LDs() public {
        ...
        assertEq(nameWrapper.ownerOf(toNameId(ethName)), account0, "NameWrapper should own .eth domain");
    ...
    function testWillNotAllowTargetAddressZeroOrWrapperContract() public {
        ...
        vm.expectRevert(abi.encodeWithSignature("IncorrectTargetOwner(address)", ZERO_ACCOUNT));
        nameWrapper.unwrap(ROOT_NODE, keccak256(bytes(label)), ZERO_ACCOUNT);
        
        vm.expectRevert(abi.encodeWithSignature("IncorrectTargetOwner(address)", address(nameWrapper)));
        nameWrapper.unwrap(ROOT_NODE, keccak256(bytes(label)), address(nameWrapper));
    ...
    function testWillNotAllowToUnwrapWithPCCCUBurnedIfExpired() public {
        ...
        // Create subdomain
        ensRegistry.setSubnodeOwner(namehash(parentName), keccak256(bytes(childLabel)), account0);
        
        baseRegistrar.setApprovalForAll(address(nameWrapper), true);
        nameWrapper.wrapETH2LD(parentLabel, account0, uint16(CANNOT_UNWRAP), address(0));
        
        bytes32 parentNode = namehash(parentName);
        _setSubnodeOwner(parentNode, childLabel, account0, PARENT_CANNOT_CONTROL | CANNOT_UNWRAP, 0);
        
        // await expectOwnerOf(childName).on(ensRegistry).equal(nameWrapper)
        assertEq(ensRegistry.owner(namehash(childName)), address(nameWrapper), "ENS should be owned by NameWrapper");
    ...
    function testWillAllowToUnwrapWithPCCCUBurnedIfExpiredAndThenExtendedWithoutPCCCU() public {
        string memory parentLabel = "awesome";
        string memory parentName = "awesome.eth";
        string memory childLabel = "sub";
        string memory childName = "sub.awesome.eth";
        ...
        bytes32 parentNode = namehash(parentName);
        _setSubnodeOwner(parentNode, childLabel, account0, PARENT_CANNOT_CONTROL | CANNOT_UNWRAP, uint64(timestamp + DAY));
        ...
        assertEq(ensRegistry.owner(namehash(childName)), account0, "ENS should be owned by account0 after unwrap");
        
        vm.stopPrank();
    }
    
    // TEST 11: "Will not allow to unwrap a name with the CANNOT_UNWRAP fuse burned if not expired"
    function testWillNotAllowToUnwrapNameWithCannotUnwrapFuseBurnedIfNotExpired() public {
        ...
        bytes32 parentNode = namehash(parentName);
        _setSubnodeOwner(parentNode, childLabel, account0, PARENT_CANNOT_CONTROL | CANNOT_UNWRAP, MAX_EXPIRY);
    ...
    function testUnwrappingPreviouslyWrappedUnexpiredNameRetainsPCCAndExpiry() public {
        ...
        assertEq(nameWrapper.ownerOf(toNameId(parentName)), account0, "Parent should be owned by account0");
        ...
        vm.stopPrank();
        vm.startPrank(account0);
        _setSubnodeOwner(parentNode, childLabel, account1, PARENT_CANNOT_CONTROL, MAX_EXPIRY);
        vm.stopPrank();
        
        // Confirm fuses are set
        (, uint32 fusesBefore,) = nameWrapper.getData(toNameId(childName));
        assertEq(fusesBefore, PARENT_CANNOT_CONTROL, "PCC fuse should be set before unwrap");
        ...
        (, uint32 fusesAfter, uint64 expiryAfter) = nameWrapper.getData(toNameId(childName));
        assertEq(fusesAfter, PARENT_CANNOT_CONTROL, "PCC fuse should be retained after unwrap");
    ...
    function testCompleteFixtureSetup() public view {
        ...
        assertTrue(ensRegistry.isApprovedForAll(account0, address(nameWrapper)), "Registry approval should be set");
    }
    ...
    function testUnwrapDifferentNameTypes() public {
        ...
        for (uint256 i = 0; i < testLabels.length; i++) {
            ...
            // Verify wrapped
            assertEq(nameWrapper.ownerOf(toNameId(label)), account0, "Should be wrapped");
            
            // Unwrap
            nameWrapper.unwrap(ROOT_NODE, keccak256(bytes(label)), account0);
            
            // Verify unwrapped
            assertEq(ensRegistry.owner(namehash(label)), account0, "Should be unwrapped");
            assertEq(nameWrapper.ownerOf(toNameId(label)), ZERO_ACCOUNT, "Should have zero owner in wrapper");
        }
    ...
    function testUnwrapWithDifferentTargetAddresses() public {
        ...
        assertEq(ensRegistry.owner(namehash(label)), account1, "Should be owned by target address");
