...
contract TestNameWrapper is BaseWrapperTest {
    ...
    function testSupportsInterface() public view {
        ...
        assertTrue(nameWrapper.supportsInterface(type(IERC721Receiver).interfaceId), "Should support IERC721Receiver");
    }
    ...
    function testWrapETH2LD() public {
        ...
        // Test wrapping
        uint64 expiry = nameWrapper.wrapETH2LD(defaultLabel, OWNER, uint16(CAN_DO_EVERYTHING), address(0));
        
        // Check state
        assertEq(nameWrapper.ownerOf(defaultNodeId), OWNER, "Owner should be set");
        assertTrue(nameWrapper.isWrapped(defaultNode), "Domain should be wrapped");
    ...
    function testUnwrapETH2LD() public {
        ...
        baseRegistrar.register(defaultLabelId, OWNER, 365 days);
        baseRegistrar.setApprovalForAll(address(nameWrapper), true);
        nameWrapper.wrapETH2LD(defaultLabel, OWNER, uint16(CAN_DO_EVERYTHING), address(0));
        
        // Test unwrapping  
        nameWrapper.unwrapETH2LD(defaultLabelHash, OWNER, OWNER);
        
        // Check state
        assertFalse(nameWrapper.isWrapped(defaultNode), "Domain should not be wrapped");
        assertEq(baseRegistrar.ownerOf(defaultLabelId), OWNER, "Should own base registrar token");
    ...
    function testSetSubnodeOwner() public {
        ...
        assertEq(nameWrapper.ownerOf(SUB_NODE_ID), ACCOUNT, "Subdomain owner should be set");
        assertTrue(nameWrapper.isWrapped(SUB_NODE), "Subdomain should be wrapped");
    ...
    function testSetFuses() public {
        ...
        uint32 newFuses = nameWrapper.setFuses(defaultNode, uint16(CANNOT_TRANSFER));
        
        // Check fuses were set
        (, uint32 fuses,) = nameWrapper.getData(defaultNodeId);
        assertTrue(fuses & CANNOT_TRANSFER != 0, "CANNOT_TRANSFER should be set");
        assertTrue(newFuses & CANNOT_UNWRAP != 0, "Should include CANNOT_UNWRAP fuse");
        assertTrue(newFuses & IS_DOT_ETH != 0, "Should include IS_DOT_ETH fuse");
    ...
    function testIsWrapped() public {
        ...
        assertFalse(nameWrapper.isWrapped(defaultNode), "Should not be wrapped initially");
        ...
        // Should be wrapped
        assertTrue(nameWrapper.isWrapped(defaultNode), "Should be wrapped after wrapping");
        
        // Test the overloaded isWrapped function with parentNode and labelhash
        // Both functions should return the same result
        assertTrue(nameWrapper.isWrapped(ETH_NODE, defaultLabelHash), "Should be wrapped using labelhash version");
    ...
    function testSetResolver() public {
        ...
        assertEq(ens.resolver(defaultNode), newResolver, "Resolver should be set");
    ...
    function testSetRecord() public {
        ...
        assertEq(nameWrapper.ownerOf(defaultNodeId), newOwner, "Owner should be set");
        assertEq(ens.resolver(defaultNode), newResolver, "Resolver should be set");
        assertEq(ens.ttl(defaultNode), newTTL, "TTL should be set");
    ...
    function testApprove() public {
        ...
        // Check approval
        assertEq(nameWrapper.getApproved(defaultNodeId), APPROVED, "Should be approved");
    ...
    function testERC1155Integration() public {
        ...
        assertEq(nameWrapper.balanceOf(OWNER, defaultNodeId), 1, "Should have balance of 1");
        assertEq(nameWrapper.balanceOf(ACCOUNT, defaultNodeId), 0, "Should have balance of 0");
    ...
    function testMetadataService() public view {
        ...
        assertEq(uri, "https://ens.domains", "URI should return the static ENS domains URL");
        
        // Test with different token ID - should return same URL (static)
        string memory uri2 = nameWrapper.uri(123);
        assertEq(uri2, "https://ens.domains", "URI should return same static URL for any token ID");
    }
}