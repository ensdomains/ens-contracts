...
contract TestStaticMetadataService is BaseWrapperTest {
    ...
    function testUriReturnsUrl() public view {
        ...
        assertEq(
            uri,
            "https://ens.domains",
            "URI should return the static URL"
        );
        ...
        assertEq(
            uri2,
            "https://ens.domains",
            "URI should return the same static URL for any token ID"
        );
        ...
        assertEq(
            uri3,
            "https://ens.domains",
            "URI should return the same static URL for zero token ID"
        );
    }
    ...
    function testOwnerCanSetNewMetadataService() public {
        ...
        StaticMetadataService newMetadataService = new StaticMetadataService(
            "https://new.example.com"
        );

        vm.prank(OWNER);
        nameWrapper.setMetadataService(
            IMetadataService(address(newMetadataService))
        );

        // Verify the metadata service was changed
        assertEq(
            address(nameWrapper.metadataService()),
            address(newMetadataService),
            "Metadata service should be updated"
        );
        ...
        assertEq(
            newUri,
            "https://new.example.com",
            "URI should return the new metadata service URL"
        );
    }
    ...
    function testNonOwnerCannotSetNewMetadataService() public {
        ...
        assertEq(
            ...
            "Metadata service should remain unchanged"
        );
        ...
        assertEq(
            originalUri,
            "https://ens.domains",
            "URI should still return the original URL"
        );
    }
    ...
    function testStaticMetadataServiceDirectly() public {
        ...
        assertEq(
            directUri1,
            "https://ens.domains",
            "Direct call should return static URI"
        );
        assertEq(
            directUri2,
            "https://ens.domains",
            "Direct call should return same static URI"
        );
        assertEq(
            directUri3,
            "https://ens.domains",
            "Direct call should return same static URI for max uint"
        );
        ...
        assertEq(
            directUri1,
            directUri2,
            "All direct calls should return identical URIs"
        );
        assertEq(
            directUri2,
            directUri3,
    ...
    function testConstructorWithDifferentUris() public {
        ...
        assertEq(
            emptyService.uri(123),
            "",
            "Empty URI constructor should work"
        );

        // Test with very long URI
        string
            memory longUri = "https://very-long-domain-name-that-might-be-used-for-testing-purposes.example.com/with/many/path/segments/and/parameters?param1=value1&param2=value2";
        StaticMetadataService longService = new StaticMetadataService(longUri);
        assertEq(
            longService.uri(456),
            longUri,
            "Long URI constructor should work"
        );

        // Test with special characters
        string
            memory specialUri = "https://example.com/path?query=value&other=test#fragment";
        StaticMetadataService specialService = new StaticMetadataService(
            specialUri
        );
        assertEq(
            specialService.uri(789),
            specialUri,
            "Special character URI should work"
        );
    }
    ...
    function testGasEfficiency() public view {
        ...
        assertTrue(
            gasUsed < 10000,
            "StaticMetadataService should be gas efficient"
        );
    }
}
