...
contract TestTestUnwrap is Test {
    ...
    function testSetWrapperApproval() public {
        ...
        assertFalse(
            testUnwrap.approvedWrapper(address(nameWrapper)),
    ...
    function testWrapETH2LDAuthorized() public {
        ...
        assertEq(
            ...
            ACCOUNT,
            "Token should be transferred to ACCOUNT"
        );
        assertEq(
            ens.owner(TEST_NODE),
            ACCOUNT,
            "ENS ownership should be set to ACCOUNT"
        );
    ...
    function testSetSubnodeRecordAuthorized() public {
        ...
        assertEq(
            ens.owner(SUB_NODE),
            OWNER,
            "Subdomain should be owned by OWNER initially"
        );
        ...
        assertEq(
            ens.owner(SUB_NODE),
            ACCOUNT,
            "Subdomain owner should be transferred to ACCOUNT"
        );
    ...
    function testWrapFromUpgradeSubdomain() public {
        ...
        assertEq(
            ens.owner(SUB_NODE),
            ACCOUNT,
            "Subdomain owner should be set to ACCOUNT"
        );
    ...
    function testContractReferences() public view {
        ...
        assertEq(
            address(testUnwrap.ens()),
            address(ens),
            "ENS reference should be correct"
        );
        assertEq(
            address(testUnwrap.registrar()),
            address(baseRegistrar),
            "Registrar reference should be correct"
        );
    }
    ...
    function testCompleteWorkflow() public {
        ...
        assertEq(
            ens.owner(SUB_NODE),
            OTHER,
            "Subdomain should be transferred to OTHER"
        );
