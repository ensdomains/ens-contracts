...
contract ConstraintsBehavior is Test {
    ...
    function _getUniqueParentLabel() internal returns (string memory) {
        testCounter++;
        string memory parentLabel = string(
            abi.encodePacked("test", vm.toString(testCounter))
        );
    ...
    function _parentCanExtend(
        ...
        if (isNotExpired) {
            ...
            uint64 parentExpiry = uint64(
                baseRegistrar.nameExpires(state.parentLabelId)
            );
            assertLt(
                childExpiry,
                parentExpiry,
                "Child expiry should be less than parent"
            );
            assertGt(
                childExpiry,
                block.timestamp,
                "Child should not be expired"
            );
        } else {
            ...
            assertEq(
                expiryBefore,
                0,
                "Child should have 0 expiry before extending"
            );
        }
        ...
        assertEq(
            expiry1,
            state.parentExpiry + GRACE_PERIOD,
            "Expiry should be parent + grace period"
        );
    ...
    function _parentCannotBurnFusesOrPCC(TestState memory state) internal {
        ...
        nameWrapper.setChildFuses(
            state.parentNode,
            state.childLabelHash,
            uint32(CANNOT_UNWRAP | PARENT_CANNOT_CONTROL),
            0
        );
    ...
    function _parentCanExtendWithSetChildFusesOnly(
        TestState memory state,
        bool isNotExpired
    ) internal {
        if (isNotExpired) {
    ...
    function _parentCanReplaceOwner(TestState memory state) internal {
        ...
        assertEq(
            nameWrapper.ownerOf(state.childNodeId),
            state.owner,
            "Child should now be owned by parent"
        );
    ...
    function _parentCanUnwrapChild(TestState memory state) internal {
        ...
        assertEq(
            nameWrapper.ownerOf(state.childNodeId),
            address(0),
            "Child should no longer be wrapped"
        );
        assertEq(
            ens.owner(state.childNode),
            state.owner,
            "ENS should be owned by parent"
        );
    }

    function _parentCannotBurnParentControlledFuses(
        TestState memory state
    ) internal {
    ...
    function _ownerIsOwnerWhenExpired(TestState memory state) internal {
        ...
        assertEq(
            nameWrapper.ownerOf(state.childNodeId),
            state.childOwner,
            "Child should still be owned by child owner"
        );
    }
    ...
    function _parentCanBurnParentControlledFusesWithExpiry(
        TestState memory state
    ) internal {
        ...
        if (parentHasCUBurned) {
            ...
            assertEq(
                fusesAfter,
                uint32(1 << 18),
                "Parent-controlled fuse should be set with extended expiry"
            );
        } else {
            ...
            assertEq(
                fuses,
                0,
                "Fuses should remain 0 since operation was prohibited"
            );
            ...
            assertEq(
                fusesAfter,
                0,
                "Fuses should remain 0 as operation was prohibited"
            );
        }
    }
    ...
    function _parentCanSetFusesOnExpiredChild(TestState memory state) internal {
        ...
        nameWrapper.setChildFuses(
            state.parentNode,
            state.childLabelHash,
            uint32(CANNOT_UNWRAP | PARENT_CANNOT_CONTROL | CANNOT_SET_RESOLVER),
            0
        );
        ...
        assertEq(
            fusesAfter,
            0,
            "Fuses should normalize to 0 for expired child"
        );
    }

    function _parentCanBurnFusesWithSetChildFuses(
        TestState memory state
    ) internal {
        ...
        assertEq(
            ...
            "Fuses should be set with setChildFuses"
        );
    }

    function _parentCanBurnFusesWithSetSubnodeOwner(
        TestState memory state
    ) internal {
        ...
        assertEq(
            fuses,
            CANNOT_UNWRAP | PARENT_CANNOT_CONTROL | CANNOT_SET_RESOLVER,
            "Fuses should be set with setSubnodeOwner"
        );
    }

    function _parentCanBurnFusesWithSetSubnodeRecord(
        TestState memory state
    ) internal {
        ...
        assertEq(
            fuses,
            CANNOT_UNWRAP | PARENT_CANNOT_CONTROL | CANNOT_SET_RESOLVER,
            "Fuses should be set with setSubnodeRecord"
        );
    }

    function _parentCanBurnParentControlledFuses(
        TestState memory state
    ) internal {
    ...
    function _ownerResetsToZeroWhenExpired(
        TestState memory state,
        uint32 expectedFuses
    ) internal {
        ...
        assertEq(
            fusesBefore,
            expectedFuses,
            "Fuses should match expected before expiry"
        );
        assertGt(
            expiryBefore,
            block.timestamp,
            "Child should not be expired yet"
        );
    ...
    function testState0000DW_ParentCanBurnParentControlledFusesWithExpiry()
        public
    {
        TestState memory state = setupState0000DW();
    ...
    function testState1111NE_CU_PCC_PCU() public {
        ...
        assertEq(
            ...
            "Additional fuse should be burned"
        );
