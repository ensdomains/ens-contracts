...
contract ERC1155Behavior is Test {
    ...
    bytes32 constant FIRST_NODE = keccak256(abi.encodePacked(ETH_NODE, FIRST_LABEL_HASH));
    ...
    bytes32 constant SECOND_NODE = keccak256(abi.encodePacked(ETH_NODE, SECOND_LABEL_HASH));
    ...
    function _mint(address[] memory addresses) internal {
        ...
        if (addresses.length > 0) {
            ...
            nameWrapper.wrapETH2LD(FIRST_LABEL, addresses[0], uint16(CAN_DO_EVERYTHING), address(0));
        }
        
        // Register and wrap second domain for second holder
        if (addresses.length > 1) {
            baseRegistrar.register(SECOND_LABEL_ID, MINTER, 365 days);
            nameWrapper.wrapETH2LD(SECOND_LABEL, addresses[1], uint16(CAN_DO_EVERYTHING), address(0));
        }
    ...
    function testSupportsInterface() public {
        assertTrue(nameWrapper.supportsInterface(type(IERC1155).interfaceId), "Should support IERC1155");
        assertTrue(nameWrapper.supportsInterface(type(IERC1155MetadataURI).interfaceId), "Should support IERC1155MetadataURI");
        assertTrue(nameWrapper.supportsInterface(type(IERC165).interfaceId), "Should support IERC165");
    }
    ...
    function testBalanceOfWhenAccountsDontOwnTokens() public {
        ...
        assertEq(nameWrapper.balanceOf(state.firstTokenHolder, FIRST_TOKEN_ID), 0, "First holder should have zero balance");
        assertEq(nameWrapper.balanceOf(state.secondTokenHolder, SECOND_TOKEN_ID), 0, "Second holder should have zero balance");
        assertEq(nameWrapper.balanceOf(state.firstTokenHolder, UNKNOWN_TOKEN_ID), 0, "Unknown token should have zero balance");
    }
    ...
    function testBalanceOfWhenAccountsOwnTokens() public {
        ...
        assertEq(nameWrapper.balanceOf(state.firstTokenHolder, uint256(FIRST_NODE)), 1, "First holder should own first token");
        assertEq(nameWrapper.balanceOf(state.secondTokenHolder, uint256(SECOND_NODE)), 1, "Second holder should own second token");
        assertEq(nameWrapper.balanceOf(state.firstTokenHolder, UNKNOWN_TOKEN_ID), 0, "Unknown token should have zero balance");
    }
    ...
    function testSetApprovalForAllSetsApprovalStatusWhichCanBeQueriedViaIsApprovedForAll() public {
        ContractState memory state = _contracts();
        
        vm.prank(state.multiTokenHolder);
        nameWrapper.setApprovalForAll(state.proxy, true);
        
        assertTrue(nameWrapper.isApprovedForAll(state.multiTokenHolder, state.proxy), "Should be approved");
    }
    ...
    function testSetApprovalForAllCanUnsetApprovalForOperator() public {
        ...
        assertFalse(nameWrapper.isApprovedForAll(state.multiTokenHolder, state.proxy), "Should not be approved");
    }
    
    function testSetApprovalForAllRevertsIfAttemptingToApproveSelfAsOperator() public {
        ContractState memory state = _contracts();
    ...
    function testSafeTransferFromRevertsWhenTransferringMoreThanBalance() public {
        ContractState memory state = _mintedToMultiFixture();
        
        vm.prank(state.multiTokenHolder);
        vm.expectRevert("ERC1155: insufficient balance for transfer");
        nameWrapper.safeTransferFrom(state.multiTokenHolder, state.recipient, uint256(FIRST_NODE), 2, "");
    }
    ...
    function testSafeTransferFromRevertsWhenTransferringToZeroAddress() public {
        ...
        nameWrapper.safeTransferFrom(state.multiTokenHolder, address(0), uint256(FIRST_NODE), 1, "");
    }
    ...
    function _validateTransferSuccess(
        ...
        assertEq(nameWrapper.balanceOf(from, id), 0, "Sender balance should be debited");
        assertEq(nameWrapper.balanceOf(to, id), value, "Recipient balance should be credited");
    ...
    function testSafeTransferFromWhenCalledByMultiTokenHolder() public {
        ...
        uint256 balanceBefore = nameWrapper.balanceOf(state.multiTokenHolder, uint256(FIRST_NODE));
        assertEq(balanceBefore, 1, "Should have token before transfer");
        
        vm.prank(state.multiTokenHolder);
        vm.expectEmit(true, true, true, true);
        emit TransferSingle(state.multiTokenHolder, state.multiTokenHolder, state.recipient, uint256(FIRST_NODE), 1);
        nameWrapper.safeTransferFrom(state.multiTokenHolder, state.recipient, uint256(FIRST_NODE), 1, "");
        
        // Validate transfer success
        assertEq(nameWrapper.balanceOf(state.multiTokenHolder, uint256(FIRST_NODE)), 0, "Sender balance should be debited");
        assertEq(nameWrapper.balanceOf(state.recipient, uint256(FIRST_NODE)), 1, "Recipient balance should be credited");
        
        // Validate preserved balances
        assertEq(nameWrapper.balanceOf(state.multiTokenHolder, uint256(SECOND_NODE)), 1, "Other token balance should be preserved");
        assertEq(nameWrapper.balanceOf(state.recipient, uint256(SECOND_NODE)), 0, "Recipient other token balance should be zero");
    }
    ...
    function testSafeTransferFromWhenCalledByOperatorApproved() public {
        ...
        vm.prank(state.proxy);
        vm.expectEmit(true, true, true, true);
        emit TransferSingle(state.proxy, state.multiTokenHolder, state.recipient, uint256(FIRST_NODE), 1);
        nameWrapper.safeTransferFrom(state.multiTokenHolder, state.recipient, uint256(FIRST_NODE), 1, "");
        ...
        assertEq(nameWrapper.balanceOf(state.proxy, uint256(FIRST_NODE)), 0, "Operator balance should remain zero");
        assertEq(nameWrapper.balanceOf(state.proxy, uint256(SECOND_NODE)), 0, "Operator other balance should remain zero");
    }
    ...
    function testSafeTransferFromToValidReceiverWithoutData() public {
        ...
        ERC1155ReceiverMock receiver = new ERC1155ReceiverMock(RECEIVER_SINGLE_MAGIC_VALUE, false, RECEIVER_BATCH_MAGIC_VALUE, false);
        
        vm.prank(state.multiTokenHolder);
        vm.expectEmit(true, true, true, true);
        emit TransferSingle(state.multiTokenHolder, state.multiTokenHolder, address(receiver), uint256(FIRST_NODE), 1);
        nameWrapper.safeTransferFrom(state.multiTokenHolder, address(receiver), uint256(FIRST_NODE), 1, "");
        
        // Validate transfer success
        assertEq(nameWrapper.balanceOf(state.multiTokenHolder, uint256(FIRST_NODE)), 0, "Sender balance should be debited");
        assertEq(nameWrapper.balanceOf(address(receiver), uint256(FIRST_NODE)), 1, "Receiver balance should be credited");
    }
    ...
    function testSafeTransferFromToValidReceiverWithData() public {
        ...
        vm.expectEmit(true, true, true, true);
        emit TransferSingle(state.multiTokenHolder, state.multiTokenHolder, address(receiver), uint256(FIRST_NODE), 1);
        nameWrapper.safeTransferFrom(state.multiTokenHolder, address(receiver), uint256(FIRST_NODE), 1, hex"f00dd00d");
    ...
    function testSafeTransferFromToReceiverReturningUnexpectedValue() public {
        ...
        ERC1155ReceiverMock receiver = new ERC1155ReceiverMock(0x00c0ffee, false, RECEIVER_BATCH_MAGIC_VALUE, false);
    ...
    function testSafeTransferFromToReceiverThatReverts() public {
        ...
        ERC1155ReceiverMock receiver = new ERC1155ReceiverMock(RECEIVER_SINGLE_MAGIC_VALUE, true, RECEIVER_BATCH_MAGIC_VALUE, false);
    ...
    function testSafeTransferFromToContractThatDoesNotImplementRequiredFunction() public {
        ContractState memory state = _mintedToMultiFixture();
        
        vm.prank(state.multiTokenHolder);
        vm.expectRevert("ERC1155: transfer to non ERC1155Receiver implementer");
        nameWrapper.safeTransferFrom(state.multiTokenHolder, address(nameWrapper), uint256(FIRST_NODE), 1, "");
    }
    
    // === safeBatchTransferFrom tests ===
    
    function testSafeBatchTransferFromRevertsWhenTransferringMoreThanBalance() public {
        ContractState memory state = _mintedToMultiFixture();
        ...
        vm.prank(state.multiTokenHolder);
        vm.expectRevert("ERC1155: insufficient balance for transfer");
        nameWrapper.safeBatchTransferFrom(state.multiTokenHolder, state.recipient, ids, amounts, "");
    }
    
    function testSafeBatchTransferFromRevertsWhenIdsArrayLengthDoesntMatchAmountsArrayLength() public {
        ContractState memory state = _mintedToMultiFixture();
        ...
        nameWrapper.safeBatchTransferFrom(state.multiTokenHolder, state.recipient, ids1, amounts1, "");
        ...
        vm.prank(state.multiTokenHolder);
        vm.expectRevert("ERC1155: ids and amounts length mismatch");
        nameWrapper.safeBatchTransferFrom(state.multiTokenHolder, state.recipient, ids2, amounts2, "");
    }
    
    function testSafeBatchTransferFromRevertsWhenTransferringToZeroAddress() public {
        ContractState memory state = _mintedToMultiFixture();
        ...
        vm.prank(state.multiTokenHolder);
        vm.expectRevert("ERC1155: transfer to the zero address");
        nameWrapper.safeBatchTransferFrom(state.multiTokenHolder, address(0), ids, amounts, "");
    }
    ...
    function testSafeBatchTransferFromWhenCalledByMultiTokenHolder() public {
        ...
        vm.prank(state.multiTokenHolder);
        vm.expectEmit(true, true, true, true);
        emit TransferBatch(state.multiTokenHolder, state.multiTokenHolder, state.recipient, ids, amounts);
        nameWrapper.safeBatchTransferFrom(state.multiTokenHolder, state.recipient, ids, amounts, "");
        ...
        uint256[] memory senderBalances = nameWrapper.balanceOfBatch(senderAddresses, ids);
        uint256[] memory recipientBalances = nameWrapper.balanceOfBatch(recipientAddresses, ids);
        
        for (uint256 i = 0; i < ids.length; i++) {
            assertEq(senderBalances[i], 0, "Sender balance should be debited");
            assertEq(recipientBalances[i], amounts[i], "Recipient balance should be credited");
        }
    }
    ...
    function testSafeBatchTransferFromWhenCalledByOperatorApproved() public {
        ...
        vm.prank(state.proxy);
        vm.expectEmit(true, true, true, true);
        emit TransferBatch(state.proxy, state.multiTokenHolder, state.recipient, ids, amounts);
        nameWrapper.safeBatchTransferFrom(state.multiTokenHolder, state.recipient, ids, amounts, "");
    ...
    function testSafeBatchTransferFromToValidReceiverWithoutData() public {
        ...
        vm.prank(state.multiTokenHolder);
        vm.expectEmit(true, true, true, true);
        emit TransferBatch(state.multiTokenHolder, state.multiTokenHolder, address(receiver), ids, amounts);
        nameWrapper.safeBatchTransferFrom(state.multiTokenHolder, address(receiver), ids, amounts, "");
    }
    ...
    function testSafeBatchTransferFromToValidReceiverWithData() public {
        ...
        vm.expectEmit(true, true, true, true);
        emit TransferBatch(state.multiTokenHolder, state.multiTokenHolder, address(receiver), ids, amounts);
        nameWrapper.safeBatchTransferFrom(state.multiTokenHolder, address(receiver), ids, amounts, hex"f00dd00d");
    }
    
    function testSafeBatchTransferFromToReceiverReturningUnexpectedValue() public {
        ContractState memory state = _mintedToMultiFixture();
        
        ERC1155ReceiverMock receiver = new ERC1155ReceiverMock(RECEIVER_SINGLE_MAGIC_VALUE, false, RECEIVER_SINGLE_MAGIC_VALUE, false);
    ...
    function testSafeBatchTransferFromToReceiverThatReverts() public {
        ...
        ERC1155ReceiverMock receiver = new ERC1155ReceiverMock(RECEIVER_SINGLE_MAGIC_VALUE, false, RECEIVER_BATCH_MAGIC_VALUE, true);
    ...
    function testSafeBatchTransferFromToReceiverThatRevertsOnlyOnSingleTransfers() public {
        ContractState memory state = _mintedToMultiFixture();
    ...
    function testSafeBatchTransferFromToContractThatDoesNotImplementRequiredFunction() public {
        ContractState memory state = _mintedToMultiFixture();
        ...
        vm.prank(state.multiTokenHolder);
        vm.expectRevert("ERC1155: transfer to non ERC1155Receiver implementer");
        nameWrapper.safeBatchTransferFrom(state.multiTokenHolder, address(nameWrapper), ids, amounts, "");
    }
}
...
contract ERC1155ReceiverMock {
    ...
    constructor(bytes4 singleRetval, bool singleRevert, bytes4 batchRetval, bool batchRevert) {
    ...
    function onERC1155BatchReceived(
        ...
        require(!_batchRevert, "ERC1155ReceiverMock: reverting on batch receive");
