...
task('seed', 'Creates test subbdomains and wraps them with Namewrapper')
  ...
  .setAction(async ({ name }, hre) => {
    ...
    console.log(
      `Wrapped NFT for sub2.${name}.arb is available at ${getOpenSeaUrl(
        ethers,
        NameWrapper.address,
        namehash(`sub2.${name}.arb`),
      )}`,
    ...
    await (
      await NameWrapper.setFuses(namehash(`${name}.arb`), CANNOT_UNWRAP, {
        gasLimit: 10000000,
      })
    ).wait()
    ...
    await (
      await NameWrapper.setFuses(namehash(`sub2.${name}.arb`), CANNOT_UNWRAP, {
        gasLimit: 10000000,
      })
    ).wait()
    console.log('NameWrapper set CANNOT_UNWRAP fuse successful for sub2')
