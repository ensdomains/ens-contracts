...
function resolveDNS(domain, type) {
  try {
    ...
    const typeName = Object.keys(DNS_TYPES).find(k => DNS_TYPES[k] === typeNum) || type;
...
async function startBatchGateway(port = 8080) {
  try {
    ...
    const child = spawn('node', ['scripts/batch_gateway_server.js', port.toString()], {
      detached: false,
...
async function main() {
  ...
  switch (command) {
    ...
    default:
      ...
      console.error('Available commands: resolve, encode, dnssec, test-oracle, batch, start-gateway');
      process.exit(1);
  }
