...
function main() {
  ...
  if (args.length < 1) {
    console.error(
      'Usage: node generate_dns_fixtures.js <blockTimestamp> [textType]',
    )
  ...
  const currentTime =
    blockTimestamp < 1000000000 ? Math.floor(Date.now() / 1000) : blockTimestamp
  ...
  const txtRRSet = createRRSetWithTexts(
    'test.test',
    texts,
    expiration,
    inception,
