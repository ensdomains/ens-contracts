...
function main() {
  ...
  if (testCase === 'all') {
    console.log('// DNS wire format data for test cases that should trigger NoMatchingProof');
    console.log('// Generated on:', new Date().toISOString());
    console.log('');
    
    console.log('// Test case 1: Root DNSKEY with algorithm 255 (unsupported)');
    const badAlgorithm = createRootKeysWithUnsupportedAlgorithm(expiration, inception);
    console.log('const ROOT_DNSKEY_BAD_ALGORITHM =', hexEncodeSignedSet(badAlgorithm) + ';');
    console.log('');
    
    console.log('// Test case 2: Root DNSKEY with different key data (different keytag)');
    const badKeytag = createRootKeysWithDifferentKey(expiration, inception);
    console.log('const ROOT_DNSKEY_BAD_KEYTAG =', hexEncodeSignedSet(badKeytag) + ';');
    console.log('');
    ...
    console.log('const ROOT_DNSKEY_NO_ZK_BIT =', hexEncodeSignedSet(noZKBit) + ';');
  ...
  switch (testCase) {
    ...
    default:
      ...
      console.error('Valid test cases: bad-algorithm, bad-keytag, no-zk-bit, all');
      process.exit(1);
  }
